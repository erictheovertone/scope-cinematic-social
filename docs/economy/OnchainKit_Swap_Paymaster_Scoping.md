...
- **Migration (existing users):** new SCA address per user. Existing EOAs hold current funds. CORRECTION (verified 2026-06-16): `payoutRecipient` is **NOT immutable** — `updatePayoutRecipient` is a first-class coins-sdk action (owner-signed). So on already-minted coins, earnings **default to the original EOA (won't break — the EOA stays the smart-wallet signer, so funds remain accessible)** AND can be **repointed to the new SCA** if desired. Options: (a) **new users only** get SCAs; (b) **opt-in migrate** (repoint payouts via `updatePayoutRecipient`); (c) **keep EOAs** + onramp. Non-destructive either way.
