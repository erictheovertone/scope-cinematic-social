    ...
    <>
      ...
      <div ref={scrollerRef} style={{
          ...
          return (
            <div>
              <button onClick={() => setDetailKey(null)} style={{ background: 'none', border: 'none', cursor: 'pointer', padding: 0 }}>
                <span style={{ ...BOLD, fontSize: 'var(--fs-10)', color: 'rgb(var(--ink-rgb) / 0.5)', letterSpacing: '0.1em' }}>← BACK</span>
              </button>
              ...
              {/* Divider */}
              <div style={{ height: 1, backgroundColor: 'rgb(var(--ink-rgb) / 0.06)', margin: '0 0 32px' }} />
              ...
              {d.sections.map((section, i) => (
                <div key={i} style={{ padding: '0 0 32px' }}>
                  ...
                  <p style={{ ...REG, fontSize: 'var(--fs-13)', color: 'rgb(var(--ink-rgb) / 0.75)', lineHeight: 1.4, margin: 0 }}>{section.body}</p>
                  {i < d.sections.length - 1 && <div style={{ height: 1, backgroundColor: 'rgb(var(--ink-rgb) / 0.05)', marginTop: 28 }} />}
                </div>
              ))}

              {(detailKey === 'free' || detailKey === 'pro') && (
                <button onClick={onJoinPress} style={{ width: '100%', background: 'var(--ink-100)', border: 'none', cursor: 'pointer', padding: '14px 0' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '10px 0', borderBottom: '1px solid rgb(var(--ink-rgb) / 0.08)' }}>
          ...
          >
            <span style={{ ...BOLD, fontSize: 'var(--fs-8)', color: 'rgb(var(--ink-rgb) / 0.5)', textTransform: 'uppercase', letterSpacing: '0.1em' }}>
        ...
        {(() => {
          ...
          return (
            <div style={{ padding: '14px 0 18px', borderBottom: '1px solid rgb(var(--ink-rgb) / 0.08)', marginBottom: 20 }}>
              <span style={{ ...BOLD, fontSize: 'var(--fs-7)', color: 'rgb(var(--ink-rgb) / 0.4)', textTransform: 'uppercase', letterSpacing: '0.15em', display: 'block', marginBottom: 14 }}>BADGES EARNED</span>
              <div style={{ display: 'flex', flexWrap: 'wrap', gap: 18 }}>
                {earned.map((b, i) => (
                  <div key={b.key} style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 6, width: 52 }}>
                    ...
                    />
                    <span style={{ ...BOLD, fontSize: 'var(--fs-8)', letterSpacing: '0.04em', color: 'var(--ink-100)', textTransform: 'uppercase', lineHeight: 1.1, textAlign: 'center' }}>{b.title}</span>
                  </div>
                ))}
              </div>
            </div>
        ...
        {[...tiers].sort((a, b) => {
          ...
          <div key={tier.key} id={`badge-tier-${tier.key}`}>
            <div onClick={() => { if (tier.key === 'composer' && vComposerTrackCount > 0 && viewerHandle) { onClose(); router.push(`/composer/${viewerHandle}`); } else setDetailKey(tier.key); }} style={{ display: 'flex', gap: 16, marginBottom: 24, alignItems: 'flex-start', cursor: 'pointer', animation: visible ? `badgeRippleIn 300ms ease-out ${i * 45}ms both` : undefined }}>
              ...
              <div style={{ flex: 1 }}>
                <p style={{ ...BOLD, fontSize: 'var(--fs-12)', color: 'var(--ink-100)', textTransform: 'uppercase', letterSpacing: '0.06em', margin: '0 0 6px', display: 'flex', alignItems: 'center', gap: 8 }/* Brief M14 — badge titles at full ivory (--ink-100), no accent-color/opacity step. Was tier.color (gold/pink/blue/0.6-ivory per badge). Matches desktop's var(--ink-100). Descriptions / YOURS-NOT-YET labels / LEARN MORE / art keep their values. */}>
                  <span>{tier.title}</span>
                  {/* Real earned status per the viewer's flags — every tier they
                      hold reads YOURS; the rest read NOT YET YOURS. */}
                  {tierEarned(tier.key, vTiers, vIsPaid, vComposerTrackCount) ? (
                    <span style={{ ...BOLD, fontSize: 'var(--fs-7)', color: 'var(--ink-100)', letterSpacing: '0.12em', border: '1px solid rgb(var(--ink-rgb) / 0.55)', padding: '1px 4px' }}>YOURS</span>
                  ) : (
                    <span style={{ ...BOLD, fontSize: 'var(--fs-7)', color: 'rgb(var(--ink-rgb) / 0.3)', letterSpacing: '0.12em' }}>NOT YET YOURS</span>
                  )}
                </p>
                <p style={{ ...REG, fontSize: 'var(--fs-11)', color: 'rgb(var(--ink-rgb) / 0.65)', lineHeight: 1.3, margin: '0 0 6px' }}>
                  {tier.description}
                </p>
                ...
                {tier.key === 'pro' && !vIsPaid && (
                  <button onClick={(e) => { e.stopPropagation(); onJoinPress(); }} style={{ background: 'transparent', border: 'none', cursor: 'pointer', padding: '0 0 6px', display: 'block' }}>
                    <span style={{ ...BOLD, fontSize: 'var(--fs-9)', color: 'var(--ink-100)', textTransform: 'uppercase', letterSpacing: '0.1em' }}>GET PRO →</span>
                  </button>
                )}
                <button
                ...
                >
                  <span style={{ fontFamily: "'SK-Modernist', sans-serif", fontWeight: 700, fontSize: 'var(--fs-8)', color: 'var(--ink-100)', letterSpacing: '0.1em', textTransform: 'uppercase' }}>
                    LEARN MORE →
                  </span>
                </button>
                <p style={{ ...BOLD, fontSize: 'var(--fs-8)', color: 'rgb(var(--ink-rgb) / 0.25)', textTransform: 'uppercase', letterSpacing: '0.1em', margin: 0 }}>
                  {tier.sub}
                </p>
              </div>
            </div>
            {i < tiers.length - 1 && (
              <div style={{ height: 1, backgroundColor: 'rgb(var(--ink-rgb) / 0.05)', marginBottom: 24 }} />
            )}
          </div>
        ))}
        ...
        <button
          onClick={onJoinPress}
          style={{ width: '100%', background: 'var(--ink-100)', border: 'none', cursor: 'pointer', padding: '14px 0', marginTop: 8 }}
        >
        ...
        <button
          onClick={onClose}
          style={{ width: '100%', background: 'transparent', border: '1px solid rgb(var(--ink-rgb) / 0.12)', cursor: 'pointer', padding: '12px 0', marginTop: 8 }}
        >
          <span style={{ ...BOLD, fontSize: 'var(--fs-10)', color: 'rgb(var(--ink-rgb) / 0.4)', textTransform: 'uppercase', letterSpacing: '0.08em' }}>
            CLOSE
          </span>
        </button>
        </>)}
