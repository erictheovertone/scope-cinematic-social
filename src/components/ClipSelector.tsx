  ...
  return (
    <div style={{ marginTop: 10 }}>
      ...
      <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
        <button onClick={play} aria-label={playing ? "Pause" : "Play"} style={{ flexShrink: 0, width: 34, height: 34, borderRadius: "50%", border: "1px solid rgb(var(--ink-rgb) / 0.2)", background: "transparent", cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center", padding: 0 }}>
          {playing
        ...
        >
          ...
          <div style={{ position: "absolute", top: 0, bottom: 0, left: `${startFrac * 100}%`, width: `${winFrac * 100}%`, background: "rgb(var(--ink-rgb) / 0.14)", borderLeft: "2px solid var(--ink-100)", borderRight: "2px solid var(--ink-100)", pointerEvents: "none" }} />
        </div>
      </div>
      <p style={{ ...SKR, fontSize: "var(--fs-7)", color: "rgb(var(--ink-rgb) / 0.4)", margin: "6px 0 0" }}>
        <span style={{ ...SKB, color: "rgb(var(--ink-rgb) / 0.6)" }}>{fmt(start)}</span> – {fmt(start + winSec)} · drag to choose the section
      </p>
    </div>
  );
}
