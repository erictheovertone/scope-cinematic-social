    ...
    <div style={{ position: "fixed", inset: 0, zIndex: 1300, background: "var(--black)", display: "flex", flexDirection: "column", maxWidth: "30rem", margin: "0 auto" }}>
      ...
      <div style={{ padding: "calc(16px + var(--safe-top)) 18px 12px", borderBottom: `1px solid ${HAIR}` }}>
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 12 }}>
          <span style={{ ...SKB, fontSize: "var(--fs-11)", color: "var(--ink-100)", textTransform: "uppercase", letterSpacing: "0.1em" }}>Add Music</span>
          <button onClick={onClose} aria-label="Close" style={{ background: "none", border: "none", cursor: "pointer", ...SKR, fontSize: 20, color: "rgb(var(--ink-rgb) / 0.55)", lineHeight: 1, padding: 4 }}>✕</button>
        </div>
        <input
          value={q}
          onChange={(e) => setQ(e.target.value)}
          placeholder="search title or composer…"
          style={{ width: "100%", boxSizing: "border-box", background: "rgb(var(--ink-rgb) / 0.05)", border: "none", outline: "none", ...SKR, fontSize: "max(16px, var(--fs-9))", color: "var(--ink-100)", padding: "9px 11px" }}
        />
        {/* keyword chips */}
      ...
      <div style={{ flex: 1, overflowY: "auto", padding: "6px 0 40px" }}>
        {tracks === null && <p style={{ ...SKR, fontSize: "var(--fs-8)", color: "rgb(var(--ink-rgb) / 0.4)", padding: "20px 18px" }}>Loading…</p>}
        {tracks !== null && results.length === 0 && <p style={{ ...SKR, fontSize: "var(--fs-8)", color: "rgb(var(--ink-rgb) / 0.4)", padding: "20px 18px" }}>No tracks match.</p>}
        {results.map((t) => {
          const isPlaying = playing === t.id;
          const isCurrent = currentTrackId === t.id;
          return (
            <div key={t.id} style={{ display: "flex", alignItems: "center", gap: 12, padding: "12px 18px", borderBottom: `1px solid rgb(var(--ink-rgb) / 0.06)` }}>
              ...
              <button onClick={() => togglePlay(t)} aria-label={isPlaying ? "Pause" : "Play"} style={{ flexShrink: 0, width: 32, height: 32, borderRadius: "50%", border: `1px solid ${HAIR}`, background: "transparent", cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center", padding: 0 }}>
                {isPlaying ? (
                  <svg width="11" height="11" viewBox="0 0 24 24" fill="var(--ink-100)"><rect x="6" y="5" width="4" height="14" /><rect x="14" y="5" width="4" height="14" /></svg>
                ) : (
                  <svg width="11" height="11" viewBox="0 0 24 24" fill="var(--ink-100)"><path d="M7 5v14l12-7z" /></svg>
                )}
              </button>
              {/* info */}
              <div style={{ flex: 1, minWidth: 0 }}>
                <div style={{ display: "flex", alignItems: "baseline", gap: 8 }}>
                  <span style={{ ...SKB, fontSize: "var(--fs-9)", color: "var(--ink-100)", whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis" }}>{t.title}</span>
                  <span style={{ ...SKR, fontSize: "var(--fs-7)", color: "rgb(var(--ink-rgb) / 0.4)", flexShrink: 0 }}>{fmt(t.duration_seconds)}</span>
                </div>
                ...
                <div style={{ display: "flex", alignItems: "center", gap: 6 }}>
                  <span style={{ ...SKR, fontSize: "var(--fs-7)", color: "rgb(var(--ink-rgb) / 0.45)" }}>{t.composer_handle ? `@${t.composer_handle}` : ""}</span>
                  <span style={{ ...SKR, fontSize: "var(--fs-7)", color: "rgb(var(--ink-rgb) / 0.28)", whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis" }}>{t.keywords.slice(0, 3).join(" · ")}</span>
                </div>
              </div>
              {/* select */}
              <button onClick={() => pick(t)} style={{ flexShrink: 0, ...SKB, fontSize: "var(--fs-8)", color: isCurrent ? "var(--ink-100)" : "var(--ink-100)", textTransform: "uppercase", letterSpacing: "0.06em", background: "transparent", border: `1px solid ${isCurrent ? "rgb(var(--ink-rgb) / 0.6)" : "rgb(var(--ink-rgb) / 0.4)"}`, cursor: "pointer", padding: "7px 12px" }}>
