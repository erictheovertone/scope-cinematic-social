  ...
  return (
    ...
    >
      <svg width={cfg.heart} height={cfg.heart} viewBox="0 0 24 24"
        fill={liked ? "var(--ink-100)" : "none"}
        stroke={liked ? "var(--ink-100)" : "rgb(var(--ink-rgb) / 0.5)"} strokeWidth={2}>
        <path d="M20.8 4.6a5.5 5.5 0 0 0-7.8 0L12 5.7l-1-1.1a5.5 5.5 0 0 0-7.8 7.8l1 1.1L12 21l7.8-7.8 1-1.1a5.5 5.5 0 0 0 0-7.8z" />
      </svg>
      {state.count > 0 && (
        <span style={{ ...SKR, fontSize: scale(cfg.meta, 1), color: liked ? "var(--ink-100)" : "rgb(var(--ink-rgb) / 0.4)", fontVariantNumeric: "tabular-nums", lineHeight: 1 }}>
          {state.count}
        </span>
      )}
    </button>
      ...
      {cfg.avatar && (
        ...
        >
          {avatarUrl ? (
            ...
            <span style={{ ...SKB, fontSize: scale(handleSize, 0.85), color: "var(--ink-100)", textTransform: "uppercase", lineHeight: 1 }}>
              {c.username?.[0] ?? "?"}
            </span>
          )}
        </div>
      ...
      <div style={{ flex: 1, minWidth: 0 }}>
        <div style={{ lineHeight: 1.3 }}>
          <span
            ref={handleRef}
            onClick={canProfile ? (e) => { e.stopPropagation(); onProfile!(c.username as string); } : undefined}
            style={{ ...SKB, fontSize: handleSize, color: `rgb(var(--ink-rgb) / ${cfg.hOp})`, marginRight: 5, textTransform: "uppercase", cursor: canProfile ? "pointer" : "default", display: "inline-block" }}
          >
            @{c.username}
          </span>
          <span style={{ ...SKR, fontSize: textSize, color: `rgb(var(--ink-rgb) / ${cfg.tOp})` }}>{c.content}</span>
        </div>
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 12, marginTop: 3 }}>
            {!isReply && onReply && viewerDid && (
              <button
                ...
                style={{ background: "none", border: "none", padding: 0, cursor: "pointer", ...SKR, fontSize: cfg.meta, color: "rgb(var(--ink-rgb) / 0.4)", textTransform: "uppercase", letterSpacing: "0.04em" }}
              >
                reply
              </button>
            )}
            {cfg.showTime && c.created_at && (
              <span style={{ ...SKR, fontSize: cfg.meta, color: "rgb(var(--ink-rgb) / 0.3)" }}>{timeAgo(c.created_at)}</span>
            )}
          </div>
        )}
      </div>
          ...
          <div key={parent.id}>
            ...
            {collapsed && (
              <button
                ...
                style={{ marginLeft: cfg.indent, marginBottom: 8, background: "none", border: "none", padding: 0, cursor: "pointer", ...SKR, fontSize: cfg.meta, color: "rgb(var(--ink-rgb) / 0.4)", textTransform: "uppercase", letterSpacing: "0.04em" }}
              >
  ...
  const body = (
    <>
      ...
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", gap: 10 }}>
        <span style={{ ...SKB, fontSize: 10, color: "rgb(var(--ink-rgb) / 0.5)", textTransform: "uppercase", letterSpacing: "0.12em" }}>
          Replying to @{parent.username}
        </span>
        <button onClick={attemptClose} aria-label="Cancel reply" style={{ background: "none", border: "none", padding: 0, cursor: "pointer", ...SKR, fontSize: 18, color: "rgb(var(--ink-rgb) / 0.55)", lineHeight: 1 }}>✕</button>
      </div>

      {/* Parent comment quoted for context (muted, clamped) */}
      <div style={{ borderLeft: "2px solid rgb(var(--ink-rgb) / 0.14)", paddingLeft: 10 }}>
        <span style={{ ...SKB, fontSize: "var(--fs-8)", color: "rgb(var(--ink-rgb) / 0.6)", textTransform: "uppercase", marginRight: 5 }}>@{parent.username}</span>
        <span style={{ ...SKR, fontSize: "var(--fs-8)", color: "rgb(var(--ink-rgb) / 0.4)", display: "-webkit-box", WebkitLineClamp: 2, WebkitBoxOrient: "vertical", overflow: "hidden" }}>{parent.content}</span>
      </div>
      ...
      <input
        ...
        style={{
          ...
          borderBottom: "1px solid rgb(var(--ink-rgb) / 0.18)", outline: "none",
          ...SKR, fontSize: "max(16px, var(--fs-10))", color: "var(--ink-100)", padding: "6px 0",
        }}
      />
      ...
      {askDiscard ? (
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", gap: 12 }}>
          <span style={{ ...SKR, fontSize: "var(--fs-8)", color: "rgb(var(--ink-rgb) / 0.5)", textTransform: "uppercase", letterSpacing: "0.04em" }}>Discard reply?</span>
          <div style={{ display: "flex", gap: 16 }}>
            <button onClick={() => setAskDiscard(false)} style={{ background: "none", border: "none", padding: 0, cursor: "pointer", ...SKB, fontSize: "var(--fs-9)", color: "rgb(var(--ink-rgb) / 0.55)", textTransform: "uppercase" }}>keep</button>
            <button onClick={finish} style={{ background: "none", border: "none", padding: 0, cursor: "pointer", ...SKB, fontSize: "var(--fs-9)", color: "var(--ink-100)", textTransform: "uppercase" }}>discard</button>
          </div>
        </div>
      ) : (
        <div style={{ display: "flex", justifyContent: "flex-end" }}>
          <button
            ...
            style={{ background: "none", border: "none", padding: 0, cursor: text.trim() ? "pointer" : "default", ...SKB, fontSize: "var(--fs-10)", color: text.trim() ? "var(--ink-100)" : "rgb(var(--ink-rgb) / 0.25)", textTransform: "uppercase", letterSpacing: "0.04em" }}
          >
    ...
    >
      ...
      >
        ...
        <div
          ...
          style={{ alignSelf: "center", width: 36, height: 4, borderRadius: 2, background: "rgb(var(--ink-rgb) / 0.25)", margin: "0 0 6px", cursor: "grab", touchAction: "none" }}
        />
        {body}
      </div>
    </div>,
