...
export default function NotificationsPanel({ onClose }: Props) {
  ...
  return (
    <div
      ...
      style={{ background: "var(--surface-1)", maxWidth: '30rem', margin: "0 auto" }}
    >
      {/* Header */}
      <div className="relative flex items-center px-[4px] pt-[12px] pb-[10px]">
        <button onClick={onClose} className="bg-transparent border-none cursor-pointer p-0">
          <span style={{ ...MONO, fontSize: 'var(--fs-9)', color: "var(--ink-100)", textTransform: "uppercase", letterSpacing: "0.06em" }}>
            ← Back
          </span>
        </button>
        <span
          className="absolute left-1/2"
          style={{ ...MONO, fontSize: 'var(--fs-9)', color: "var(--ink-100)", textTransform: "uppercase", letterSpacing: "0.08em", transform: "translateX(-50%)" }}
        >
          NOTIFICATIONS
        </span>
      </div>
      ...
      <div className="flex-1 overflow-y-auto">
        {loading ? (
          ...
          <div className="flex items-center justify-center mt-12">
            <p style={{ ...MONO, fontSize: 'var(--fs-9)', color: "rgb(var(--ink-rgb) / 0.4)", textTransform: "uppercase", letterSpacing: "0.06em" }}>
          ...
          notifications.map((n) => (
            ...
            >
              ...
              <div className="flex-1 min-w-0">
                <p style={{ ...MONO, fontSize: 'var(--fs-8)', color: "var(--ink-100)", textTransform: "uppercase", letterSpacing: "0.04em", lineHeight: 1.4, margin: 0, wordBreak: "break-word" }}>
                  <NotificationActorMessage handle={n.actor_handle ?? null} type={n.type} onNavigate={goToActor} />
                </p>
                <p style={{ ...MONO, fontSize: 'var(--fs-6)', color: "rgb(var(--ink-rgb) / 0.4)", textTransform: "uppercase", letterSpacing: "0.06em", margin: "2px 0 0" }}>
