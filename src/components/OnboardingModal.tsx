...
export default function OnboardingModal({ onComplete }: Props) {
  ...
  return (
    <div style={{
        ...
        <div style={{ display: 'flex', gap: 6 }}>
          {SCREENS.map((_, i) => (
            <div key={i} style={{
              width: i === screen ? 20 : 6,
              height: 2,
              backgroundColor: i === screen ? 'var(--ink-100)' : 'rgb(var(--ink-rgb) / 0.2)',
      ...
      {/* Content */}
      <div>
        <p style={{ ...SKB, fontSize: 'var(--fs-9)', color: 'rgb(var(--ink-rgb) / 0.35)', letterSpacing: '0.15em', margin: '0 0 24px' }}>
          {current.label}
        </p>
        <p style={{
          ...SKB, fontSize: 'var(--fs-36)', color: 'var(--ink-100)',
          lineHeight: 1.1, letterSpacing: '-0.03em',
          margin: '0 0 28px', whiteSpace: 'pre-line',
        }}>
          {current.title}
        </p>
        <p style={{ ...SKR, fontSize: 'var(--fs-14)', color: 'rgb(var(--ink-rgb) / 0.6)', lineHeight: 1.65, margin: 0 }}>
          {current.body}
        </p>
      </div>
      ...
      <div>
        <button onClick={handleNext} style={{
          width: '100%', background: 'var(--ink-100)', border: 'none',
        ...
        {screen < SCREENS.length - 1 && (
          <button onClick={handleDone} style={{
            ...
            <span style={{ ...SKB, fontSize: 'var(--fs-9)', color: 'rgb(var(--ink-rgb) / 0.3)', textTransform: 'uppercase', letterSpacing: '0.1em' }}>
              SKIP
            </span>
          </button>
        )}
