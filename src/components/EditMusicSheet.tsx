...
export default function EditMusicSheet({
  ...
  return createPortal(
    <>
      ...
      <div style={{ position: "fixed", left: 0, right: 0, bottom: 0, zIndex: 561, maxWidth: "30rem", margin: "0 auto", background: "var(--surface-1)", borderTop: `1px solid ${HAIR}`, padding: "16px 20px calc(28px + env(safe-area-inset-bottom))", transform: visible ? "translateY(0)" : "translateY(100%)", transition: "transform 220ms cubic-bezier(0.32,0.72,0,1)" }}>
        ...
        {trackId ? (
          <>
            <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", gap: 10, marginBottom: 14 }}>
              <span style={{ ...SKR, fontSize: "var(--fs-9)", color: "var(--ink-100)", whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis" }}>
                {meta?.title ?? "Track attached"}{meta?.handle ? ` · @${meta.handle}` : ""}
              </span>
              <button onClick={() => persist(null, null)} disabled={saving} style={{ flexShrink: 0, ...SKB, fontSize: "var(--fs-8)", color: "var(--ink-100)", textTransform: "uppercase", letterSpacing: "0.06em", background: "transparent", border: "none", cursor: "pointer", padding: 0 }}>Remove</button>
            </div>
            {isVideo && (
          ...
          <p style={{ ...SKR, fontSize: "var(--fs-8)", color: "rgb(var(--ink-rgb) / 0.4)", margin: "0 0 14px" }}>No music on this post.</p>
        )}

        <button onClick={() => setShowPicker(true)} style={{ width: "100%", background: "transparent", border: `1px solid ${HAIR}`, cursor: "pointer", padding: "12px 0", ...SKB, fontSize: "var(--fs-9)", color: "var(--ink-100)", textTransform: "uppercase", letterSpacing: "0.08em" }}>
          {trackId ? "Change track" : "Add music"}
        </button>
        {error && <p style={{ ...SKR, fontSize: "var(--fs-8)", color: "var(--ink-100)", margin: "10px 0 0" }}>{error}</p>}
      </div>
