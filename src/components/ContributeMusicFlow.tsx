    ...
    <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
      ...
      {step === "upload" && (
        <>
          <div
            ...
            style={{ border: `1px dashed ${HAIR}`, padding: "26px 18px", textAlign: "center", cursor: rows.length < BATCH_CAP ? "pointer" : "default", background: "rgb(var(--ink-rgb) / 0.02)" }}
          >
            <p style={{ ...SKB, fontSize: "var(--fs-10)", color: "var(--ink-100)", margin: "0 0 6px", letterSpacing: "0.06em" }}>DROP AUDIO OR TAP TO CHOOSE</p>
            <p style={{ ...SKR, fontSize: "var(--fs-8)", color: "rgb(var(--ink-rgb) / 0.4)", margin: 0 }}>MP3 · AAC · WAV — up to 15MB, ~6 min · {rows.length}/{BATCH_CAP}</p>
          </div>
          ...
          {rows.length > 0 && (
            <div style={{ display: "flex", flexDirection: "column", gap: 8, maxHeight: isDesktop ? 300 : undefined, overflowY: isDesktop ? "auto" : "visible" }}>
              {rows.map((r) => (
                <div key={r.localId} style={{ display: "flex", alignItems: "center", gap: 10, padding: "8px 10px", border: `1px solid ${r.status === "failed" ? "rgb(var(--ink-rgb) / 0.4)" : HAIR}` }}>
                  <div style={{ flex: 1, minWidth: 0 }}>
                    <p style={{ ...SKR, fontSize: "var(--fs-8)", color: "var(--ink-100)", margin: "0 0 5px", whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis" }}>{r.fileName}</p>
                    {r.status === "uploading" && (
                      <div style={{ height: 3, background: "rgb(var(--ink-rgb) / 0.12)" }}><div style={{ height: "100%", width: `${r.progress}%`, background: RED, transition: "width 120ms linear" }} /></div>
                    )}
                    {r.status === "done" && <span style={{ ...SKR, fontSize: "var(--fs-7)", color: "rgb(var(--ink-rgb) / 0.4)" }}>DONE · {fmtDuration(r.duration)}</span>}
                    {r.status === "failed" && <span style={{ ...SKR, fontSize: "var(--fs-7)", color: RED }}>FAILED</span>}
                  </div>
                  {r.status === "failed" && (
                    <button onClick={() => void startUpload(r)} style={{ ...SKB, fontSize: "var(--fs-7)", color: "var(--ink-100)", background: "transparent", border: `1px solid ${HAIR}`, cursor: "pointer", padding: "4px 9px", textTransform: "uppercase" }}>Retry</button>
                  )}
                  <button onClick={() => removeRow(r.localId)} aria-label="Remove" style={{ background: "none", border: "none", cursor: "pointer", ...SKR, fontSize: 16, color: "rgb(var(--ink-rgb) / 0.4)", lineHeight: 1, padding: 2 }}>✕</button>
                </div>
              ))}
            </div>
          )}
          ...
          {rows.length > 0 && (
            <div style={{ display: "flex", justifyContent: "flex-end" }}>
              <button
                ...
                style={{ ...SKB, fontSize: "var(--fs-9)", color: (uploading || doneRows.length === 0) ? "rgb(var(--ink-rgb) / 0.25)" : "var(--ink-100)", textTransform: "uppercase", letterSpacing: "0.08em", background: "none", border: "none", cursor: (uploading || doneRows.length === 0) ? "default" : "pointer", padding: 0 }}
              >
                {uploading ? "Uploading…" : `Details (${doneRows.length}) →`}
              </button>
            </div>
          )}
          {failedRows.length > 0 && !uploading && <p style={{ ...SKR, fontSize: "var(--fs-7)", color: "rgb(var(--ink-rgb) / 0.4)", margin: 0 }}>Failed files are skipped unless retried.</p>}
        </>
      )}

      {step === "table" && (
        <>
          <p style={{ ...SKR, fontSize: "var(--fs-8)", color: "rgb(var(--ink-rgb) / 0.4)", margin: 0 }}>Title each track and pick {KEYWORDS_MIN}–{KEYWORDS_MAX} keywords.</p>
          <div style={{ display: "flex", flexDirection: "column", gap: 10, maxHeight: isDesktop ? 380 : undefined, overflowY: isDesktop ? "auto" : "visible" }}>
            {doneRows.map((r) => {
              ...
              const ok = rowReady(r);
              return (
                <div key={r.localId} style={{ border: `1px solid ${ok ? HAIR : "rgb(var(--ink-rgb) / 0.35)"}`, padding: "10px 12px" }}>
                  <div style={{ display: "flex", gap: 10, alignItems: "flex-start" }}>
                    ...
                    <div style={{ flexShrink: 0, display: "flex", flexDirection: "column", alignItems: "center", gap: 4 }}>
                      ...
                      {doneRows.length > 1 && r.artUrl && (
                        <button onClick={() => applyArtToAll(r.localId)} style={{ ...SKR, fontSize: 9, color: "rgb(var(--ink-rgb) / 0.4)", background: "none", border: "none", cursor: "pointer", padding: 0, textTransform: "uppercase", letterSpacing: "0.04em" }}>All</button>
                      )}
                    </div>
                    <input
                      value={r.title}
                      ...
                      style={{ flex: 1, minWidth: 0, boxSizing: "border-box", background: "transparent", border: "none", borderBottom: `1px solid rgb(var(--ink-rgb) / 0.18)`, outline: "none", ...SKR, fontSize: "max(16px, var(--fs-9))", color: "var(--ink-100)", padding: "4px 0", alignSelf: "center" }}
                    />
                  </div>
                  <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", gap: 10, marginTop: 8 }}>
                    <span style={{ ...SKR, fontSize: "var(--fs-7)", color: r.keywords.length >= KEYWORDS_MIN ? "rgb(var(--ink-rgb) / 0.5)" : "rgb(var(--ink-rgb) / 0.7)", whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis" }}>
                      {r.keywords.length ? r.keywords.join(" · ") : "no keywords yet"} ({r.keywords.length}/{KEYWORDS_MAX})
                    </span>
                    <button onClick={() => setExpanded(open ? null : r.localId)} style={{ flexShrink: 0, ...SKB, fontSize: "var(--fs-7)", color: "var(--ink-100)", background: "transparent", border: `1px solid ${HAIR}`, cursor: "pointer", padding: "4px 9px", textTransform: "uppercase" }}>{open ? "Done" : "Keywords"}</button>
                  </div>
                  {open && (
                    <div style={{ marginTop: 10 }}>
                      {MUSIC_TAXONOMY.map((group) => (
                        <div key={group.label} style={{ marginBottom: 10 }}>
                          <p style={{ ...SKB, fontSize: "var(--fs-7)", color: "rgb(var(--ink-rgb) / 0.35)", textTransform: "uppercase", letterSpacing: "0.16em", margin: "0 0 6px" }}>{group.label}</p>
                      ...
                      <button onClick={() => applyKeywordsToAll(r.localId)} style={{ ...SKB, fontSize: "var(--fs-7)", color: "var(--ink-100)", background: "transparent", border: `1px solid ${HAIR}`, cursor: "pointer", padding: "6px 12px", textTransform: "uppercase", letterSpacing: "0.06em" }}>Apply keywords to all</button>
                    </div>
                  )}
                </div>
              );
      ...
      {step === "done" && (
        <div style={{ padding: "8px 0 6px" }}>
          <p style={{ ...SKB, fontSize: "var(--fs-11)", color: "var(--ink-100)", margin: "0 0 8px", letterSpacing: "0.04em" }}>Submitted for review</p>
          <p style={{ ...SKR, fontSize: "var(--fs-9)", color: "rgb(var(--ink-rgb) / 0.5)", lineHeight: 1.5, margin: "0 0 20px" }}>You&rsquo;ll be notified when {doneRows.length > 1 ? "they&rsquo;re" : "it&rsquo;s"} approved. Approved contributors earn the COMPOSER badge.</p>
          <button onClick={close} style={{ ...SKB, width: "100%", fontSize: "var(--fs-10)", color: "var(--ink-100)", textTransform: "uppercase", letterSpacing: "0.08em", background: "transparent", border: `1px solid ${HAIR}`, cursor: "pointer", padding: "13px 0" }}>Done</button>
        </div>
      )}
...
function StepNav({ onBack, onNext, nextLabel, nextDisabled }: { onBack: () => void; onNext: () => void; nextLabel: string; nextDisabled?: boolean }) {
  return (
    <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", gap: 12, marginTop: 4 }}>
      <button onClick={onBack} style={{ ...SKB, fontSize: "var(--fs-9)", color: "rgb(var(--ink-rgb) / 0.5)", textTransform: "uppercase", letterSpacing: "0.08em", background: "none", border: "none", cursor: "pointer", padding: 0 }}>← Back</button>
      <button onClick={onNext} disabled={nextDisabled} style={{ ...SKB, fontSize: "var(--fs-9)", color: nextDisabled ? "rgb(var(--ink-rgb) / 0.25)" : "var(--ink-100)", textTransform: "uppercase", letterSpacing: "0.08em", background: "none", border: "none", cursor: nextDisabled ? "default" : "pointer", padding: 0 }}>{nextLabel} →</button>
    </div>
  );
}
