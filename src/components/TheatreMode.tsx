...
//
// Design system: pure black, var(--ink-100), SK-Modernist, sharp corners, no shadow/blur,
    ...
    return (
      <div onClick={() => handleClose()} style={{ position: 'fixed', inset: 0, zIndex: 900, background: 'var(--black)', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: 22, padding: '0 40px' }}>
        ...
        <button
          ...
          style={{ ...SKB, fontSize: 'var(--fs-9)', color: 'var(--ink-100)', textTransform: 'uppercase', letterSpacing: '0.12em', background: 'transparent', border: '1px solid rgb(var(--ink-rgb) / 0.5)', cursor: 'pointer', padding: '12px 26px', touchAction: 'manipulation' }}
        >
          Back to profile
        </button>
      </div>
  ...
  return (
    ...
    <div data-swipe-exclude data-force-dark style={{ ...stageStyle, zIndex: zBase }}>{/* Brief T1 Stage 2 — theatre is a VIEWING SURFACE: forces the dark token set in light theme. */}
      ...
      >
        ...
        >
          <span style={{ ...SKB, fontSize: 'var(--fs-11)', color: 'var(--ink-100)', letterSpacing: '0.02em', textTransform: 'uppercase' }}>Back</span>
        </button>
        ...
        {!showData && (
          <div style={{ position: 'absolute', top: stageH / 2 + boxH / 2 + Math.max(2, (stageH / 2 - boxH / 2 - 34) / 2), left: 0, right: 0, display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 6, zIndex: 3, height: 34 }}>
            ...
            {isScreening && ranks?.[index] != null && (
              <span style={{ position: 'absolute', left: 16, top: '50%', transform: 'translateY(-50%)', fontFamily: 'var(--font-display)', fontWeight: 700, fontSize: 12, letterSpacing: 'var(--track-wide)', color: 'rgb(var(--ink-rgb) / 0.5)', pointerEvents: 'none' }}>
            ...
            >
              ...
              <span style={{ ...SKL, fontSize: 'var(--fs-30)', lineHeight: 1, color: 'var(--ink-100)', display: 'block', transform: 'translateY(-3px)' }}>+</span>
            </button>
            {isFeed && f(post, 'username') && (
              <span style={{ ...SKB, fontSize: 'var(--fs-10)', color: 'rgb(var(--ink-rgb) / 0.85)', letterSpacing: '0.02em', textTransform: 'uppercase', pointerEvents: 'none', marginRight: 6 }}>
                @{f(post, 'username')}
              </span>
            )}
            {isFeed && (
              <div style={{ display: 'flex', alignItems: 'center', gap: 4 }}>
                ...
                <button onClick={(e) => { stop(e); handleLike(); }} aria-label="Like" style={{ display: 'inline-flex', alignItems: 'center', justifyContent: 'center', gap: 4, background: 'transparent', border: 'none', cursor: 'pointer', minWidth: 44, minHeight: 44, padding: 0 }}>
                  <svg width="20.5" height="20.5" viewBox="0 0 24 24" fill={isLiked ? 'var(--ink-100)' : 'none'} stroke={isLiked ? 'var(--ink-100)' : 'rgb(var(--ink-rgb) / 0.85)'} strokeWidth="2" strokeLinejoin="round"><path d="M12 21s-7-4.5-9.5-9C1 9 2.5 5.5 6 5.5c2 0 3.2 1.2 4 2.3.8-1.1 2-2.3 4-2.3 3.5 0 5 3.5 3.5 6.5C19 16.5 12 21 12 21z"/></svg>
                  <span style={{ ...SKB, fontSize: 'var(--fs-9)', color: 'var(--ink-100)', fontVariantNumeric: 'tabular-nums' }}>{likes.length}</span>
                </button>
                <button onClick={(e) => { stop(e); setShowData(true); setShowComments(true); }} aria-label="Comments" style={{ display: 'inline-flex', alignItems: 'center', justifyContent: 'center', gap: 4, background: 'transparent', border: 'none', cursor: 'pointer', minWidth: 44, minHeight: 44, padding: 0 }}>
                  <svg width="20.5" height="20.5" viewBox="0 0 24 24" fill="none" stroke="rgb(var(--ink-rgb) / 0.85)" strokeWidth="2" strokeLinejoin="round"><path d="M21 11.5a8.5 8.5 0 0 1-12.5 7.5L3 21l2-5.5A8.5 8.5 0 1 1 21 11.5z"/></svg>
                  <span style={{ ...SKB, fontSize: 'var(--fs-9)', color: 'var(--ink-100)', fontVariantNumeric: 'tabular-nums' }}>{comments.length}</span>
                </button>
                {coinAddr && (
                  <span style={{ display: 'inline-flex', alignItems: 'center', gap: 4, minHeight: 44 }}>
                    ...
                    <span style={{ ...SKB, fontSize: 'var(--fs-9)', color: fcCount > 0 ? 'var(--ink-100)' : 'var(--ink-100)', fontVariantNumeric: 'tabular-nums' }}>{fcCount}/{FIRST_CUT_SLOTS}</span>
                  </span>
                )}
              </div>
            )}
        ...
        >
          ...
          <div style={{ display: 'flex', alignItems: 'baseline', gap: 10, marginBottom: 8 }}>
            <span style={{ ...SKB, fontSize: 'var(--fs-12)', color: 'var(--ink-100)', textTransform: 'uppercase', letterSpacing: '0.02em' }}>@{f(post, 'username') ?? '—'}</span>
            {f(post, 'caption') && (
              <span style={{ ...SKR, fontSize: 'var(--fs-10)', color: 'rgb(var(--ink-rgb) / 0.55)', lineHeight: 1.3, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap', flex: 1, minWidth: 0 }}>{f(post, 'caption')}</span>
            )}
            {isCoin && (
              <button
                onClick={(e) => { stop(e); setShowCollect(true); }}
                style={{ ...SKB, fontSize: 'var(--fs-9)', letterSpacing: '0.08em', color: 'var(--ink-100)', textTransform: 'uppercase', background: 'transparent', border: '1px solid var(--ink-100)', cursor: 'pointer', padding: '4px 12px', flexShrink: 0 }}
              >
                Collect
              </button>
            )}
          ...
          {showComments && (
            <div className="fc-slot" style={{ marginTop: 9, animation: reduceMotion.current ? 'none' : `fcSlotRipple 0.32s ${EASE} both` }}>
              <div style={{ maxHeight: 96, overflowY: 'auto', display: 'flex', flexDirection: 'column', gap: 7 }}>
                {comments.length === 0 ? (
                  <span style={{ ...SKR, fontSize: 'var(--fs-10)', color: 'rgb(var(--ink-rgb) / 0.35)', textTransform: 'uppercase', letterSpacing: '0.1em' }}>No comments yet</span>
                ) : comments.map((c, i) => (
                  <div key={c.id ?? i} style={{ display: 'flex', gap: 7, alignItems: 'baseline' }}>
                    <span style={{ ...SKB, fontSize: 'var(--fs-10)', color: 'var(--ink-100)', textTransform: 'uppercase', flexShrink: 0 }}>@{c.username ?? '—'}</span>
                    <span style={{ ...SKR, fontSize: 'var(--fs-11)', color: 'rgb(var(--ink-rgb) / 0.7)', lineHeight: 1.35 }}>{c.content}</span>
                  </div>
                ))}
              </div>
              {user && (
                <div style={{ display: 'flex', gap: 8, marginTop: 10, alignItems: 'center' }}>
                  <input
                    ...
                    style={{ ...SKR, flex: 1, fontSize: 'max(16px, var(--fs-11))', color: 'var(--ink-100)', background: 'transparent', border: 'none', borderBottom: '1px solid rgb(var(--ink-rgb) / 0.18)', outline: 'none', padding: '6px 0' }}
                  />
                  <button onClick={(e) => { stop(e); handleAddComment(); }} disabled={!newComment.trim()} style={{ ...SKB, fontSize: 'var(--fs-9)', letterSpacing: '0.1em', color: newComment.trim() ? 'var(--ink-100)' : 'rgb(var(--ink-rgb) / 0.3)', textTransform: 'uppercase', background: 'transparent', border: 'none', cursor: newComment.trim() ? 'pointer' : 'default', flexShrink: 0 }}>Post</button>
                </div>
              )}
            </div>
          )}
        ...
        {source === 'profile' && (
          <div style={{ position: 'absolute', right: 16, bottom: 14, ...SKB, fontSize: 'var(--fs-9)', color: 'rgb(var(--ink-rgb) / 0.45)', letterSpacing: '0.08em', pointerEvents: 'none' }}>
