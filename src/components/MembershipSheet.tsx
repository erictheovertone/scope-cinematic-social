  ...
  return (
    <>
      ...
      {embeddedOpen && typeof document !== "undefined" && createPortal(
        <div data-swipe-exclude style={{ position: "fixed", inset: 0, width: "100dvw", height: "100dvh", zIndex: 600, backgroundColor: "var(--surface-1)", display: "flex", flexDirection: "column", overflow: "hidden", paddingTop: "env(safe-area-inset-top, 0px)" }}>
          <div style={{ flexShrink: 0, display: "flex", alignItems: "center", justifyContent: "space-between", padding: "14px 18px", borderBottom: "1px solid rgb(var(--ink-rgb) / 0.08)", background: "var(--surface-1)" }}>
      ...
      <div data-swipe-exclude style={{
        ...
        padding: "32px 24px calc(48px + var(--safe-bottom))", /* X3 §3 — bottom sheet: clear the home indicator */
        maxHeight: "90vh",
        overflowY: "auto",
      }}>
        {/* Drag handle */}
        ...
        {isPaidMember && paidMemberUntil && (
          <div style={{ padding: '20px 0', textAlign: 'center', marginBottom: 16 }}>
            <div style={{ height: 1, backgroundColor: 'rgb(var(--ink-rgb) / 0.06)', marginBottom: 20 }} />
            <p style={{ ...BOLD, fontSize: 'var(--fs-10)', color: 'var(--ink-100)', textTransform: 'uppercase', letterSpacing: '0.1em', margin: '0 0 8px' }}>
              ACTIVE MEMBER
            </p>
            <p style={{ ...REG, fontSize: 'var(--fs-11)', color: 'rgb(var(--ink-rgb) / 0.5)', margin: '0 0 4px' }}>
              Your membership is active until
            </p>
            <p style={{ ...BOLD, fontSize: 'var(--fs-13)', color: 'var(--ink-100)', margin: '0 0 16px' }}>
              {paidMemberUntil.toLocaleDateString('en-US', { month: 'long', day: 'numeric', year: 'numeric' })}
            </p>
            <button
              onClick={onClose}
              style={{ background: 'transparent', border: '1px solid rgb(var(--ink-rgb) / 0.2)', cursor: 'pointer', padding: '10px 32px' }}
            >
              <span style={{ ...BOLD, fontSize: 'var(--fs-10)', color: 'rgb(var(--ink-rgb) / 0.5)', textTransform: 'uppercase', letterSpacing: '0.08em' }}>CLOSE</span>
            </button>
            <div style={{ height: 1, backgroundColor: 'rgb(var(--ink-rgb) / 0.06)', marginTop: 20 }} />
          </div>
        )}

        {/* Header */}
        <div style={{ marginBottom: 28 }}>
          <p style={{ ...BOLD, fontSize: 'var(--fs-18)', color: "var(--ink-100)", textTransform: "uppercase", letterSpacing: "-0.02em", margin: "0 0 8px" }}>
            BECOME A SCOPE MEMBER
          </p>
          <p style={{ ...REG, fontSize: 'var(--fs-11)', color: "rgb(var(--ink-rgb) / 0.5)", lineHeight: 1.6, margin: 0 }}>
        ...
        {/* What you get */}
        <div style={{ marginBottom: 28, borderTop: "1px solid rgb(var(--ink-rgb) / 0.06)", paddingTop: 20 }}>
          {[
            "UNLIMITED POSTS",
            "UNLIMITED DECKS",
            "UP TO 5 LINKS",
            ...
            <div key={benefit} style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 10 }}>
              <div style={{ width: 4, height: 4, borderRadius: "50%", backgroundColor: "var(--ink-100)", flexShrink: 0 }} />
              <p style={{ ...BOLD, fontSize: 'var(--fs-9)', color: "var(--ink-100)", textTransform: "uppercase", letterSpacing: "0.06em", margin: 0 }}>{benefit}</p>
            </div>
          ))}
        </div>
          ...
          {plans.map(plan => (
            <button
              ...
              style={{
                ...
                border: `1px solid ${selectedPlan === plan.id ? 'var(--ink-100)' : 'rgb(var(--ink-rgb) / 0.15)'}`,
                padding: '10px 8px',
                cursor: 'pointer',
                textAlign: 'center',
              }}
            >
              <p style={{ ...BOLD, fontSize: 'var(--fs-8)', color: selectedPlan === plan.id ? 'var(--ink-100)' : 'rgb(var(--ink-rgb) / 0.4)', textTransform: 'uppercase', letterSpacing: '0.06em', margin: '0 0 4px' }}>
                {plan.label}
              </p>
              <p style={{ ...BOLD, fontSize: 'var(--fs-13)', color: selectedPlan === plan.id ? 'var(--ink-100)' : 'rgb(var(--ink-rgb) / 0.4)', margin: '0 0 4px' }}>
                {plan.price}
              </p>
              <p style={{ ...REG, fontSize: 'var(--fs-7)', color: 'rgb(var(--ink-rgb) / 0.35)', textTransform: 'uppercase', letterSpacing: '0.04em', margin: 0 }}>
                {plan.sub}
              </p>
            </button>
          ))}
        ...
        {txStatus === "error" && txError && (
          <p style={{ ...REG, fontSize: 'var(--fs-9)', color: "var(--ink-100)", textAlign: "center", margin: "0 0 12px", lineHeight: 1.4 }}>
            {txError.slice(0, 80)}
          </p>
        )}
        ...
        <button
          ...
          style={{
            width: "100%",
            background: working ? "rgb(var(--ink-rgb) / 0.4)" : "var(--ink-100)",
            border: "none",
            padding: "16px 0",
            cursor: working ? "default" : "pointer",
          }}
        >
          <span style={{ ...BOLD, fontSize: 'var(--fs-12)', color: working ? "var(--ink-100)" : "var(--on-ink)", textTransform: "uppercase", letterSpacing: "0.06em" }/* X2c — idle state fills solid ivory → --on-ink; working state is translucent-grey so ivory stays legible */}>
            {working ? "PROCESSING..." : txStatus === "confirming" ? "CONFIRMING..." : "JOIN SCOPE · " + plans.find(p => p.id === selectedPlan)?.price}
          </span>
        </button>

        <p style={{ ...REG, fontSize: 'var(--fs-8)', color: "rgb(var(--ink-rgb) / 0.25)", textAlign: "center", margin: "12px 0 0", lineHeight: 1.5 }}>
