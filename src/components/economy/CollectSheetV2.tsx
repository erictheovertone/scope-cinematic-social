...
export default function CollectSheetV2({ post, visible, onClose, tradeable = true }: Props) {
  ...
  const tabStyle = (active: boolean): React.CSSProperties => ({
    ...SKB, flex: 1, fontSize: 'var(--fs-11)', letterSpacing: '0.12em', textTransform: 'uppercase',
    padding: '10px 0', textAlign: 'center', cursor: 'pointer',
    color: active ? 'var(--ink-100)' : 'rgb(var(--ink-rgb) / 0.4)',
    background: 'transparent',
    borderTop: 'none', borderLeft: 'none', borderRight: 'none',
    borderBottomWidth: 1,
    borderBottomStyle: 'solid',
    borderBottomColor: active ? 'var(--ink-100)' : 'rgb(var(--ink-rgb) / 0.1)',
  });
  ...
  return (
    <>
      ...
      <div
        ...
        {/* Brief F3: X doubled (fs-16→32px) + opacity 0.5→0.85 (more visible); padding
            keeps the hit ≥44px. */}
        <button onClick={onClose} aria-label="Close" style={{ position: 'absolute', top: 8, right: 10, zIndex: 5, background: 'transparent', border: 'none', cursor: 'pointer', padding: 8, display: 'flex', alignItems: 'center', justifyContent: 'center', lineHeight: 0 }}>
          <span style={{ fontFamily: "'SK-Modernist', sans-serif", fontWeight: 400, fontSize: 32, color: 'rgb(var(--ink-rgb) / 0.85)', lineHeight: 1 }}>×</span>
        </button>

        {/* Banner ONLY for mock data — real coin reads (live) carry no banner. */}
        {market && !market.live && (
          <div style={{ ...SKB, fontSize: 'var(--fs-7)', letterSpacing: '0.2em', color: 'var(--ink-100)', textTransform: 'uppercase', marginBottom: 14 }}>
          ...
          const text = (
            <div style={{ flex: 1, minWidth: 0 }}>
              <div style={{ display: 'flex', alignItems: 'baseline', gap: 8, margin: '0 0 3px' }}>
                <p style={{ ...SKB, fontSize: 'var(--fs-11)', color: 'var(--ink-100)', textTransform: 'uppercase', letterSpacing: '0.02em', margin: 0 }}>@{post.username}</p>
                {post.ticker && <TickerMark ticker={post.ticker} size={13.5} />}
              </div>
              <p style={{ ...SKR, fontSize: 'var(--fs-10)', color: 'rgb(var(--ink-rgb) / 0.5)', margin: 0, lineHeight: 1.35, overflow: 'hidden', display: '-webkit-box', WebkitLineClamp: 2, WebkitBoxOrient: 'vertical' as const }}>{post.caption || ''}</p>
            </div>
          );
          const media = (
            <div
            ...
            >
              ...
              {captured && (
                <>
                  <div style={{ position: 'absolute', top: 6, left: 6, width: 22, height: 22, borderTop: '2px solid var(--ink-100)', borderLeft: '2px solid var(--ink-100)', animation: 'cornerReveal 0.6s ease forwards', opacity: 0 }} />
                  <div style={{ position: 'absolute', top: 6, right: 6, width: 22, height: 22, borderTop: '2px solid var(--ink-100)', borderRight: '2px solid var(--ink-100)', animation: 'cornerReveal 0.6s ease 0.1s forwards', opacity: 0 }} />
                  <div style={{ position: 'absolute', bottom: 6, left: 6, width: 22, height: 22, borderBottom: '2px solid var(--ink-100)', borderLeft: '2px solid var(--ink-100)', animation: 'cornerReveal 0.6s ease 0.2s forwards', opacity: 0 }} />
                  <div style={{ position: 'absolute', bottom: 6, right: 6, width: 22, height: 22, borderBottom: '2px solid var(--ink-100)', borderRight: '2px solid var(--ink-100)', animation: 'cornerReveal 0.6s ease 0.3s forwards', opacity: 0 }} />
        ...
        <div style={{ display: 'flex', gap: 1, marginBottom: 18, background: 'rgb(var(--ink-rgb) / 0.08)' }}>
          {[
          ...
          <div style={{ display: 'flex', alignItems: 'baseline', justifyContent: 'space-between', marginBottom: 16 }}>
            ...
            <span style={{ ...SKB, fontSize: 'var(--fs-13)', color: 'var(--ink-100)', fontVariantNumeric: 'tabular-nums' }}>
              ...
              {market?.priceUsd != null && (
                <span style={{ ...SKR, fontSize: 'var(--fs-9)', color: 'rgb(var(--ink-rgb) / 0.45)', marginLeft: 7 }}>· {usd(held * market.priceUsd)}</span>
              )}
            </span>
          </div>
        )}
        ...
        {done ? (
          <div style={{ border: '1px solid rgb(var(--ink-rgb) / 0.12)', padding: '18px 14px', textAlign: 'center', marginBottom: 18 }}>
            <p style={{ ...SKB, fontSize: 'var(--fs-12)', color: done.startsWith('[') ? 'var(--ink-100)' : 'var(--ink-100)', textTransform: 'uppercase', letterSpacing: '0.06em', margin: 0 }}>{done}</p>
            {market && !market.live && (
              <p style={{ ...SKR, fontSize: 'var(--fs-8)', color: 'rgb(var(--ink-rgb) / 0.3)', textTransform: 'uppercase', letterSpacing: '0.08em', margin: '8px 0 0' }}>PREVIEW ONLY · NO REAL TRANSACTION</p>
            )}
          </div>
        ) : (
          <>
            ...
            {mode === 'buy' ? (
              ...
              <>
                {/* Large amount input */}
                <div style={{ display: 'flex', alignItems: 'center', borderBottom: '1px solid rgb(var(--ink-rgb) / 0.15)', paddingBottom: 8, marginBottom: 12 }}>
                  <span style={{ ...SKB, fontSize: 'var(--fs-34)', color: buyUsd ? 'var(--ink-100)' : 'rgb(var(--ink-rgb) / 0.25)' }}>$</span>
                  <input
                    inputMode="decimal"
                    placeholder="0"
                    value={buyUsd}
                    onChange={(e) => setBuyUsd(e.target.value.replace(/[^0-9.]/g, ''))}
                    style={{ ...SKB, fontSize: 'max(16px, var(--fs-34))', color: 'var(--ink-100)', WebkitTextFillColor: 'var(--ink-100)', caretColor: 'var(--ink-100)', background: 'transparent', WebkitAppearance: 'none', appearance: 'none', border: 'none', outline: 'none', width: '100%', padding: 0, fontVariantNumeric: 'tabular-nums' }}
                  />
                </div>
                ...
                <div style={{ display: 'flex', gap: 6, marginBottom: 14 }}>
                  {BUY_CHIPS.map((c) => (
                    <button
                      ...
                      style={{ ...SKB, flex: 1, fontSize: 'var(--fs-11)', color: 'var(--ink-100)', background: 'transparent', border: '1px solid rgb(var(--ink-rgb) / 0.18)', padding: '8px 0', cursor: 'pointer' }}
                    >
                      ${c}
                    </button>
                  ))}
                ...
                <div style={{ border: '1px solid rgb(var(--ink-rgb) / 0.1)', padding: '12px 12px', marginBottom: 14 }}>
                  ...
                  <div style={{ display: 'flex', alignItems: 'baseline', justifyContent: 'space-between', marginTop: 6 }}>
                    <span style={{ ...SKR, fontSize: 'var(--fs-8)', color: 'rgb(var(--ink-rgb) / 0.3)', textTransform: 'uppercase', letterSpacing: '0.08em' }}>1 FRAGMENT = {TOKENS_PER_PIECE.toLocaleString()} TOKENS</span>
                    <span style={{ ...SKR, fontSize: 'var(--fs-9)', color: 'rgb(var(--ink-rgb) / 0.4)' }}>
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 16 }}>
                  <span style={{ ...SKR, fontSize: 'var(--fs-9)', color: 'rgb(var(--ink-rgb) / 0.4)', textTransform: 'uppercase', letterSpacing: '0.1em' }}>PAY WITH</span>
                  <div style={{ display: 'flex', gap: 1, background: 'rgb(var(--ink-rgb) / 0.12)' }}>
                ...
                {busy ? (
                  ...
                  <div style={{ width: '100%', border: '1px solid rgb(var(--ink-rgb) / 0.55)', padding: '13px 0', marginBottom: 8, display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 10, minHeight: 46 }}>
                    <ScopeLoader size="sm" />
                  ...
                  <button
                    onClick={doBuy}
                    disabled={!buyQuote}
                    style={{ width: '100%', background: !buyQuote ? 'rgb(var(--ink-rgb) / 0.4)' : 'var(--ink-100)', border: 'none', cursor: !buyQuote ? 'default' : 'pointer', padding: '14px 0', marginBottom: 8 }}
                  >
                    <span style={{ ...SKB, fontSize: 'var(--fs-12)', color: buyQuote ? 'var(--on-ink)' : 'var(--ink-100)', textTransform: 'uppercase', letterSpacing: '0.08em' }/* X2c — enabled state fills solid ivory; text → --on-ink (charcoal) so it isn't ivory-on-ivory. Disabled bg is translucent over dark, so ivory stays legible there. */}>
                ...
                <button
                  ...
                  style={{ width: '100%', background: 'transparent', border: '1px solid rgb(var(--ink-rgb) / 0.15)', cursor: 'pointer', padding: '11px 0' }}
                >
                  <span style={{ ...SKB, fontSize: 'var(--fs-10)', color: 'rgb(var(--ink-rgb) / 0.6)', textTransform: 'uppercase', letterSpacing: '0.08em' }}>FUND WALLET</span>
                </button>
              </>
            ) : (
              /* ── SELL — position-led ── */
                ...
                <div style={{ display: 'flex', alignItems: 'baseline', justifyContent: 'space-between', marginBottom: 12 }}>
                  ...
                  <span style={{ ...SKB, fontSize: 'var(--fs-18)', color: 'var(--ink-100)', fontVariantNumeric: 'tabular-nums' }}>
                    ...
                    {market?.priceUsd != null && held > 0 && (
                      <span style={{ ...SKR, fontSize: 'var(--fs-11)', color: 'rgb(var(--ink-rgb) / 0.45)', marginLeft: 7 }}>· {usd(held * market.priceUsd)}</span>
                    )}
                  </span>
                </div>

                {held <= 0 ? (
                  <p style={{ ...SKR, fontSize: 'var(--fs-11)', color: 'rgb(var(--ink-rgb) / 0.4)', textTransform: 'uppercase', letterSpacing: '0.06em', padding: '16px 0' }}>
                  ...
                  <>
                    ...
                    <div style={{ display: 'flex', alignItems: 'center', borderBottom: '1px solid rgb(var(--ink-rgb) / 0.15)', paddingBottom: 8, marginBottom: 12 }}>
                      <input
                        ...
                        style={{ ...SKB, fontSize: 'max(16px, var(--fs-30))', color: 'var(--ink-100)', WebkitTextFillColor: 'var(--ink-100)', caretColor: 'var(--ink-100)', background: 'transparent', WebkitAppearance: 'none', appearance: 'none', border: 'none', outline: 'none', width: '100%', padding: 0, fontVariantNumeric: 'tabular-nums' }}
                      />
                      <span style={{ ...SKB, fontSize: 'var(--fs-11)', color: 'rgb(var(--ink-rgb) / 0.4)', textTransform: 'uppercase', letterSpacing: '0.1em', whiteSpace: 'nowrap' }}>FRAGMENTS</span>
                    </div>
                    ...
                    <div style={{ border: '1px solid rgb(var(--ink-rgb) / 0.1)', padding: '12px 12px', marginBottom: 14 }}>
                      <div style={{ display: 'flex', alignItems: 'baseline', justifyContent: 'space-between' }}>
                        ...
                        <span style={{ ...SKB, fontSize: 'var(--fs-18)', color: 'var(--ink-100)', fontVariantNumeric: 'tabular-nums' }}>
                          ...
                          <span style={{ ...SKR, fontSize: 'var(--fs-9)', color: 'rgb(var(--ink-rgb) / 0.4)', marginLeft: 6 }}>{sellQuote && sellCurrency === 'ETH' ? `(${eth(sellQuote.ethAmount)})` : sellQuote ? '(USDC)' : ''}</span>
                        </span>
                      </div>
                      ...
                      {(() => {
                        ...
                        return (
                          <p style={{ ...SKB, fontSize: 'var(--fs-8)', color: impact >= 5 ? 'var(--ink-100)' : 'rgb(var(--ink-rgb) / 0.45)', textTransform: 'uppercase', letterSpacing: '0.1em', margin: '8px 0 0' }}>
                    ...
                    <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 14 }}>
                      <span style={{ ...SKR, fontSize: 'var(--fs-9)', color: 'rgb(var(--ink-rgb) / 0.4)', textTransform: 'uppercase', letterSpacing: '0.1em' }}>RECEIVE</span>
                      <div style={{ display: 'flex', gap: 1, background: 'rgb(var(--ink-rgb) / 0.12)' }}>
                        {(['USDC', 'ETH'] as TradeCurrency[]).map((c) => (
                          <button
                            key={c}
                    ...
                    {/* FIRST CUT guard */}
                    {sellEndsFirstCut && (
                      <div style={{ border: '1px solid var(--ink-100)', padding: '12px 12px', marginBottom: 14, display: 'flex', gap: 10, alignItems: 'flex-start' }}>
                        <ApertureMark size={15.5} />
                        <p style={{ ...SKR, fontSize: 'var(--fs-10)', color: 'var(--ink-100)', lineHeight: 1.4, margin: 0 }}>
                          This sale ends your First Cut on this post — <span style={{ color: 'var(--ink-100)', ...SKB }}>permanently.</span>
                        </p>
                      </div>
                    )}
                    ...
                    {busy ? (
                      /* THE WHEEL — pressed button transformed into narration. */
                      <div style={{ width: '100%', border: '1px solid rgb(var(--ink-rgb) / 0.55)', padding: '13px 0', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 10, minHeight: 46 }}>
                        <ScopeLoader size="sm" />
                        <span style={{ ...SKB, fontSize: 'var(--fs-11)', color: 'var(--ink-100)', textTransform: 'uppercase', letterSpacing: '0.1em' }}>
                      ...
                      <button
                        ...
                        style={{ width: '100%', background: sellPieces <= 0 ? 'rgb(var(--ink-rgb) / 0.08)' : (sellEndsFirstCut && confirmEndFirstCut ? 'var(--ink-100)' : 'transparent'), border: sellPieces <= 0 ? 'none' : '1px solid var(--ink-100)', cursor: sellPieces <= 0 ? 'default' : 'pointer', padding: '14px 0' }}
                      >
                        <span style={{ ...SKB, fontSize: 'var(--fs-12)', color: (sellEndsFirstCut && confirmEndFirstCut) ? 'var(--on-ink)' : 'var(--ink-100)', textTransform: 'uppercase', letterSpacing: '0.08em' }/* X2c — the CONFIRM—END FIRST CUT state fills solid ivory; text → --on-ink there. Other states are transparent/faint over dark → ivory stays legible. */}>
        ...
        <div
          ...
          style={{ border: '1px solid rgb(var(--ink-rgb) / 0.1)', padding: '12px 12px', marginTop: 18, cursor: 'pointer' }}
        >
          <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 10 }}>
            <ApertureMark size={15.5} />
            <span style={{ ...SKB, fontSize: 'var(--fs-9)', color: 'var(--ink-100)', textTransform: 'uppercase', letterSpacing: '0.12em' }}>FIRST CUT</span>
            <span style={{ marginLeft: 'auto', display: 'flex', alignItems: 'center', gap: 6 }}>
              {openCount > 0 ? (
                <span style={{ ...SKB, fontSize: 'var(--fs-8)', color: 'var(--ink-100)', textTransform: 'uppercase', letterSpacing: '0.1em' }}>{openCount} OF 10 SLOTS OPEN</span>
              ) : (
                <span style={{ ...SKR, fontSize: 'var(--fs-8)', color: 'rgb(var(--ink-rgb) / 0.45)', textTransform: 'uppercase', letterSpacing: '0.08em' }}>{holdingCount} HOLDING</span>
              )}
              <span style={{ ...SKB, fontSize: 'var(--fs-11)', color: 'rgb(var(--ink-rgb) / 0.5)', lineHeight: 1, transform: showSlots ? 'rotate(90deg)' : 'none', transition: 'transform 0.2s ease' }}>›</span>
            </span>
          </div>
          ...
          {showSlots && (
            <div style={{ marginTop: 12, borderTop: '1px solid rgb(var(--ink-rgb) / 0.08)', paddingTop: 12 }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 10 }}>
                <ApertureMark size={14.5} />
                <span style={{ ...SKB, fontSize: 'var(--fs-9)', color: 'var(--ink-100)', textTransform: 'uppercase', letterSpacing: '0.12em' }}>FIRST CUT</span>
                <span style={{ ...SKB, fontSize: 'var(--fs-9)', color: 'rgb(var(--ink-rgb) / 0.4)', marginLeft: 'auto', textTransform: 'uppercase', letterSpacing: '0.08em' }}>THE FIRST 10 · PERMANENT</span>
              </div>
              {Array.from({ length: 10 }).map((_, idx) => {
                ...
                return (
                  <div key={pos} style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '6px 0', opacity: !isOpen && !lit ? 0.38 : 1 }}>
                    <span style={{ ...SKB, fontSize: 'var(--fs-10)', width: 18, color: lit ? 'var(--ink-100)' : 'rgb(var(--ink-rgb) / 0.35)', fontVariantNumeric: 'tabular-nums' }}>{String(pos).padStart(2, '0')}</span>
                    {isOpen ? (
                      <span style={{ ...SKR, fontSize: 'var(--fs-9)', color: 'rgb(var(--ink-rgb) / 0.3)', textTransform: 'uppercase', letterSpacing: '0.1em' }}>OPEN</span>
                    ) : (
                      <>
                        {slot!.avatarUrl && <img src={feedImage(slot!.avatarUrl, 96)} alt="" style={{ width: 24, height: 24, borderRadius: '50%' }} />}
                        <span style={{ ...SKR, fontSize: 'var(--fs-10)', color: 'var(--ink-100)', textTransform: 'uppercase', letterSpacing: '0.04em' }}>@{slot!.handle}</span>
                        <span style={{ ...SKB, fontSize: 'var(--fs-8)', marginLeft: 'auto', color: lit ? 'var(--ink-100)' : 'rgb(var(--ink-rgb) / 0.6)', textTransform: 'uppercase', letterSpacing: '0.12em' }}>{lit ? 'HOLDING' : 'DEPARTED'}</span>
                      </>
                    )}
                  </div>
                );
          ...
          <div style={{ border: '1px solid rgb(var(--ink-rgb) / 0.4)', padding: '24px 16px', textAlign: 'center', marginTop: 2 }}>
            <p style={{ ...SKB, fontSize: 'var(--fs-12)', color: 'var(--ink-100)', textTransform: 'uppercase', letterSpacing: '0.16em', margin: '0 0 12px' }}>[ NOT TRADEABLE ]</p>
            <p style={{ ...SKR, fontSize: 'var(--fs-11)', color: 'rgb(var(--ink-rgb) / 0.6)', lineHeight: 1.6, margin: '0 auto', maxWidth: 240 }}>
        ...
        {market && !market.live && (
          <p style={{ ...SKR, fontSize: 'var(--fs-8)', color: 'rgb(var(--ink-rgb) / 0.25)', textAlign: 'center', margin: '14px 0 0', textTransform: 'uppercase', letterSpacing: '0.08em' }}>
