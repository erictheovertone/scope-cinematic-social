  ...
  if (rows.length === 0 && visibleStacks.length === 0) {
    return isOwn ? (
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', minHeight: '30vh', padding: '0 32px' }}>
        <p style={{ ...SKB, fontSize: 'var(--fs-10)', color: 'rgb(var(--ink-rgb) / 0.4)', textTransform: 'uppercase', textAlign: 'center', letterSpacing: '0.08em', lineHeight: 1.8 }}>
          NOTHING COLLECTED YET.<br />COLLECT IS HOW YOU KEEP THINGS ON SCOPE.
        </p>
      </div>
    ) : (
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', minHeight: '30vh', padding: '0 32px' }}>
        <p style={{ ...SKB, fontSize: 'var(--fs-10)', color: 'rgb(var(--ink-rgb) / 0.35)', textTransform: 'uppercase', textAlign: 'center', letterSpacing: '0.08em' }}>
    ...
    <>
      ...
      {(visibleStacks.length > 0 || isOwn) && (
        <div style={{ padding: '2px 0 38px' }}>
          ...
          {visibleStacks.length === 0 && isOwn && (
            <button onClick={() => setCreateOpen(true)} style={{ display: 'block', width: '100%', background: 'transparent', border: '1px dashed rgb(var(--ink-rgb) / 0.18)', cursor: 'pointer', padding: '18px 0', margin: '0 0 2px' }}>
              <span style={{ ...SKR, fontSize: 'var(--fs-8)', color: 'rgb(var(--ink-rgb) / 0.45)', textTransform: 'uppercase', letterSpacing: '0.1em' }}>CURATE YOUR FIRST PROGRAM</span>
            </button>
          )}
            ...
            return (
              ...
              >
                ...
                <span style={{ position: 'absolute', right: 12, top: '50%', transform: 'translateY(-50%)', display: 'flex', alignItems: 'center', gap: 8 }}>
                  <span style={{ ...SKB, fontSize: 'var(--fs-7)', color: 'rgb(var(--ink-rgb) / 0.7)', border: '1px solid rgb(var(--ink-rgb) / 0.3)', padding: '2px 6px', fontVariantNumeric: 'tabular-nums' }}>{items.length}</span>
                  <span style={{ ...SKR, fontSize: 'var(--fs-12)', color: 'rgb(var(--ink-rgb) / 0.7)' }}>›</span>
                </span>
              </button>
            );
          })}
      ...
      {rows.length > 0 && (
        <div>
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '6px 10px 10px', borderTop: '1px solid rgb(var(--ink-rgb) / 0.08)', whiteSpace: 'nowrap' }}>
            {activeStack ? (
              <span style={{ ...SKB, fontSize: 'var(--fs-10)', color: 'var(--ink-100)', textTransform: 'uppercase', letterSpacing: '0.16em', overflow: 'hidden', textOverflow: 'ellipsis' }}>{activeStack.title}</span>
            ) : (
              <span style={{ ...SKB, fontSize: 'var(--fs-10)', color: 'rgb(var(--ink-rgb) / 0.85)', textTransform: 'uppercase', letterSpacing: '0.16em' }}>COLLECTED ITEMS</span>
            )}
            <span style={{ display: 'flex', alignItems: 'center', gap: 14, flexShrink: 0 }}>
              {activeStack && isOwn && (
                <button onClick={() => setEditStackId(activeStack.id)} style={{ ...SKB, fontSize: 'var(--fs-7)', color: 'var(--ink-100)', background: 'transparent', border: 'none', cursor: 'pointer', textTransform: 'uppercase', letterSpacing: '0.08em', padding: 0 }}>EDIT</button>
              )}
              {activeStack && (
                <button onClick={() => setFullscreen('open')} aria-label="Full-screen program" style={{ ...SKR, fontSize: 'var(--fs-11)', color: 'rgb(var(--ink-rgb) / 0.6)', background: 'transparent', border: 'none', cursor: 'pointer', lineHeight: 1, padding: 0 }}>⤢</button>
              )}
              {activeStack && (
                <button onClick={() => switchTo(null)} aria-label="Show all items" style={{ ...SKR, fontSize: 'var(--fs-12)', color: 'rgb(var(--ink-rgb) / 0.6)', background: 'transparent', border: 'none', cursor: 'pointer', lineHeight: 1, padding: 0 }}>×</button>
              )}
              ...
              <button onClick={() => setViewMode('grid')} aria-label="Grid view" style={{ background: 'transparent', border: 'none', cursor: 'pointer', padding: 0, lineHeight: 0, opacity: viewMode === 'grid' ? 1 : 0.4 }}>
                <svg width="15" height="15" viewBox="0 0 13 13"><rect x="0" y="0" width="5.5" height="5.5" fill="var(--ink-100)"/><rect x="7.5" y="0" width="5.5" height="5.5" fill="var(--ink-100)"/><rect x="0" y="7.5" width="5.5" height="5.5" fill="var(--ink-100)"/><rect x="7.5" y="7.5" width="5.5" height="5.5" fill="var(--ink-100)"/></svg>
              </button>
              <button onClick={() => setViewMode('list')} aria-label="List view" style={{ background: 'transparent', border: 'none', cursor: 'pointer', padding: 0, lineHeight: 0, opacity: viewMode === 'list' ? 1 : 0.4 }}>
                <svg width="15" height="15" viewBox="0 0 13 13"><rect x="0" y="1" width="13" height="2" fill="var(--ink-100)"/><rect x="0" y="5.5" width="13" height="2" fill="var(--ink-100)"/><rect x="0" y="10" width="13" height="2" fill="var(--ink-100)"/></svg>
              </button>
            </span>
          </div>
          ...
          {viewMode === 'grid' ? (
            ...
            <div className={swapPhase === 'out' ? 'collage-swap-out' : ''}>
              {collage.slice(0, shownCount).map((h) => (
                <button key={h.postId} onClick={() => openLightbox(h.postId)} style={{ display: 'flex', width: '100%', alignItems: 'center', gap: 10, background: 'transparent', border: 'none', borderBottom: '1px solid rgb(var(--ink-rgb) / 0.06)', cursor: 'pointer', padding: '8px 10px', textAlign: 'left' }}>
                  <img src={feedImage(thumbOf(h) ?? '', 96)} alt="" style={{ width: 56, height: 34, objectFit: 'cover', display: 'block', background: '#111', flexShrink: 0 }} />
                  <span style={{ ...SKB, fontSize: 'var(--fs-9)', color: 'var(--ink-100)', textTransform: 'uppercase', flex: 1 }}>{h.ticker ? `[ ${h.ticker} ]` : '—'}</span>
                  ...
                  <span style={{ ...SKR, fontSize: 'var(--fs-8)', color: 'rgb(var(--ink-rgb) / 0.55)', fontVariantNumeric: 'tabular-nums' }}>{mcOf(h)}</span>
                </button>
              ))}
            </div>
          )}
          ...
          {!activeStack && !expanded && collage.length > 4 && (
            <button onClick={() => { setExpanded(true); setVisible(20); }} style={{ display: 'block', width: '100%', background: 'transparent', border: 'none', cursor: 'pointer', padding: '14px 0 18px' }}>
              <span style={{ ...SKR, fontSize: 'var(--fs-8)', color: 'rgb(var(--ink-rgb) / 0.5)', textTransform: 'uppercase', letterSpacing: '0.16em' }}>VIEW ALL · {collage.length}</span>
            </button>
          )}
          {(activeStack ? false : expanded) && collage.length > visible && (
            <button onClick={() => setVisible((v) => v + 20)} style={{ display: 'block', width: '100%', background: 'transparent', border: 'none', cursor: 'pointer', padding: '14px 0 18px' }}>
              <span style={{ ...SKR, fontSize: 'var(--fs-8)', color: 'rgb(var(--ink-rgb) / 0.5)', textTransform: 'uppercase', letterSpacing: '0.12em' }}>LOAD MORE</span>
            </button>
          )}
        </div>
      )}
      ...
      {fullscreen && activeStack && createPortal(
        ...
        >
          <div style={{ flexShrink: 0, display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: 'calc(12px + env(safe-area-inset-top, 0px)) 14px 10px' }}>
            <span style={{ ...SKB, fontSize: 'var(--fs-10)', color: 'var(--ink-100)', textTransform: 'uppercase', letterSpacing: '0.16em', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{activeStack.title}</span>
            <span style={{ display: 'flex', alignItems: 'center', gap: 16, flexShrink: 0 }}>
              ...
              <button onClick={closeFullscreen} aria-label="Collapse" style={{ ...SKR, fontSize: 'var(--fs-12)', color: 'rgb(var(--ink-rgb) / 0.7)', background: 'transparent', border: 'none', cursor: 'pointer', lineHeight: 1, padding: 0 }}>⤡</button>
            </span>
          </div>
      ...
      <div style={{ position: 'absolute', bottom: 0, left: 0, right: 0, maxHeight: '86dvh', overflowY: 'auto', background: '#080808', borderTop: '1px solid rgb(var(--ink-rgb) / 0.1)', padding: '18px 14px calc(22px + env(safe-area-inset-bottom, 0px))' }}>
        <div style={{ display: 'flex', alignItems: 'baseline', justifyContent: 'space-between', marginBottom: 14 }}>
          ...
          <button onClick={onClose} style={{ background: 'transparent', border: 'none', cursor: 'pointer', padding: 6 }}>
            <span style={{ ...SKR, fontSize: 'var(--fs-16)', color: 'rgb(var(--ink-rgb) / 0.5)', lineHeight: 1 }}>×</span>
          </button>
        </div>
        ...
        <div style={{ display: 'flex', alignItems: 'baseline', gap: 10, marginBottom: 4 }}>
          <input
            ...
            style={{ ...SKB, flex: 1, fontSize: 'max(16px, var(--fs-12))', color: 'var(--ink-100)', background: 'transparent', border: 'none', borderBottom: '1px solid rgb(var(--ink-rgb) / 0.25)', outline: 'none', textTransform: 'uppercase', letterSpacing: '0.06em', padding: '6px 0' }}
          />
          <span style={{ ...SKR, fontSize: 'var(--fs-7)', color: 'rgb(var(--ink-rgb) / 0.4)', fontVariantNumeric: 'tabular-nums' }}>{title.length}/{STACK_TITLE_MAX}</span>
        </div>
        <p style={{ ...SKR, fontSize: 'var(--fs-7)', color: 'rgb(var(--ink-rgb) / 0.4)', textTransform: 'uppercase', letterSpacing: '0.08em', margin: '10px 0 8px' }}>
          ...
          {held.map((h) => {
            ...
            return (
              <button
                ...
                style={{ position: 'relative', aspectRatio: '16 / 10', overflow: 'hidden', background: '#111', border: isHero ? '1px solid var(--ink-100)' : sel ? '1px solid rgb(var(--ink-rgb) / 0.8)' : '1px solid transparent', cursor: 'pointer', padding: 0 }}
              >
                <img src={feedImage(thumbOf(h) ?? '', 300)} alt="" style={{ width: '100%', height: '100%', objectFit: 'cover', display: 'block', opacity: sel ? 1 : 0.55 }} />
                {isHero && <span style={{ position: 'absolute', left: 4, bottom: 3, ...SKB, fontSize: 9, color: 'var(--ink-100)', textTransform: 'uppercase' }}>HERO</span>}
              </button>
            );
          })}
        </div>
        ...
        <button
          onClick={create}
          disabled={!canCreate}
          style={{ display: 'block', width: '100%', marginTop: 16, background: canCreate ? 'var(--ink-100)' : 'rgb(var(--ink-rgb) / 0.08)', border: 'none', cursor: canCreate ? 'pointer' : 'default', padding: '13px 0' }}
        >
          <span style={{ ...SKB, fontSize: 'var(--fs-10)', color: canCreate ? 'var(--on-ink)' : 'rgb(var(--ink-rgb) / 0.35)', textTransform: 'uppercase', letterSpacing: '0.1em' }/* X2c — enabled bg fills solid ivory; text → --on-ink (was ivory-on-ivory) */}>
      ...
      <div style={{ position: 'absolute', bottom: 0, left: 0, right: 0, maxHeight: '88dvh', overflowY: 'auto', background: '#080808', borderTop: '1px solid rgb(var(--ink-rgb) / 0.1)', padding: '18px 14px calc(22px + env(safe-area-inset-bottom, 0px))' }}>
          ...
          {renaming ? (
            <span style={{ display: 'flex', alignItems: 'baseline', gap: 8, flex: 1 }}>
              <input
                ...
                maxLength={STACK_TITLE_MAX}
                autoFocus
                style={{ ...SKB, fontSize: 'max(16px, var(--fs-12))', color: 'var(--ink-100)', background: 'transparent', border: 'none', borderBottom: '1px solid rgb(var(--ink-rgb) / 0.3)', outline: 'none', textTransform: 'uppercase', letterSpacing: '0.06em', width: '55%' }}
              />
              <button onClick={async () => { if (await renameStack(stack.id, name)) { setRenaming(false); onChanged(); } }} style={{ ...SKB, fontSize: 'var(--fs-8)', color: 'var(--ink-100)', background: 'transparent', border: 'none', cursor: 'pointer', textTransform: 'uppercase' }}>SAVE</button>
            </span>
          ) : (
            <span style={{ display: 'flex', alignItems: 'baseline', gap: 10 }}>
              <span style={{ ...SKB, fontSize: 'var(--fs-13)', color: 'var(--ink-100)', textTransform: 'uppercase', letterSpacing: '0.1em' }}>{stack.title}</span>
              <span style={{ ...SKR, fontSize: 'var(--fs-8)', color: 'rgb(var(--ink-rgb) / 0.5)', textTransform: 'uppercase' }}>{items.length} ITEMS</span>
            </span>
          )}
          <button onClick={onClose} style={{ background: 'transparent', border: 'none', cursor: 'pointer', padding: 6 }}>
            <span style={{ ...SKR, fontSize: 'var(--fs-16)', color: 'rgb(var(--ink-rgb) / 0.5)', lineHeight: 1 }}>×</span>
        ...
        {isOwn && (
          <div style={{ display: 'flex', gap: 16, margin: '6px 0 12px' }}>
            <button onClick={() => setAdding((a) => !a)} style={{ ...SKR, fontSize: 'var(--fs-8)', color: adding ? 'var(--ink-100)' : 'rgb(var(--ink-rgb) / 0.55)', background: 'transparent', border: 'none', cursor: 'pointer', textTransform: 'uppercase', letterSpacing: '0.06em', padding: 0 }}>+ ADD ITEMS</button>
            <button onClick={() => setRenaming((r) => !r)} style={{ ...SKR, fontSize: 'var(--fs-8)', color: 'rgb(var(--ink-rgb) / 0.55)', background: 'transparent', border: 'none', cursor: 'pointer', textTransform: 'uppercase', letterSpacing: '0.06em', padding: 0 }}>RENAME</button>
            <button onClick={async () => { if (await deleteStack(stack.id)) { onChanged(); onClose(); } }} style={{ ...SKR, fontSize: 'var(--fs-8)', color: 'rgb(var(--ink-rgb) / 0.7)', background: 'transparent', border: 'none', cursor: 'pointer', textTransform: 'uppercase', letterSpacing: '0.06em', padding: 0 }}>DELETE</button>
          </div>
        )}
        ...
        {isOwn && heroSold && items.length > 0 && (
          ...
          <p style={{ ...SKR, fontSize: 'var(--fs-8)', color: 'rgb(var(--ink-rgb) / 0.5)', textTransform: 'uppercase', letterSpacing: '0.08em', margin: '0 0 10px' }}>
            HERO NO LONGER HELD · <span style={{ color: 'var(--ink-100)' }}>SET NEW HERO</span> — tap SET HERO on any item
          </p>
        )}
        {items.length === 0 && (
          <p style={{ ...SKR, fontSize: 'var(--fs-8)', color: 'rgb(var(--ink-rgb) / 0.45)', textTransform: 'uppercase', letterSpacing: '0.08em', margin: '8px 0 12px' }}>
        ...
        {adding && (
          <div style={{ marginBottom: 14 }}>
            <p style={{ ...SKR, fontSize: 'var(--fs-7)', color: 'rgb(var(--ink-rgb) / 0.4)', textTransform: 'uppercase', letterSpacing: '0.08em', margin: '0 0 6px' }}>TAP TO ADD</p>
            <div className="grid grid-cols-4 gap-[2px]">
              {addable.map((h) => (
                <button key={h.postId} onClick={async () => { if (await addStackItems(stack.id, [h.postId], stack.itemPostIds.length)) onChanged(); }} style={{ aspectRatio: '16 / 10', overflow: 'hidden', background: '#111', border: '1px solid rgb(var(--ink-rgb) / 0.15)', cursor: 'pointer', padding: 0 }}>
                  <img src={feedImage(thumbOf(h) ?? '', 300)} alt="" style={{ width: '100%', height: '100%', objectFit: 'cover', display: 'block' }} />
                </button>
              ))}
              {addable.length === 0 && <p style={{ ...SKR, fontSize: 'var(--fs-7)', color: 'rgb(var(--ink-rgb) / 0.35)', textTransform: 'uppercase', gridColumn: 'span 4', padding: '8px 0' }}>EVERYTHING HELD IS ALREADY IN THIS PROGRAM.</p>}
            </div>
          </div>
        )}
