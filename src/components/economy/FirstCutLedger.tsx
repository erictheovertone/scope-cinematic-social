...
// var(--ink-100) (impact only), grey at rest, SK-Modernist Bold, sharp corners.
    ...
    <div style={{ background: 'var(--black)', width: '100%' }}>
      ...
      >
        <div style={{ display: 'flex', alignItems: 'center', gap: 7 }}>
          ...
          />
          <span style={{ ...SKB, fontSize: 'var(--fs-11)', letterSpacing: '0.18em', color: 'var(--ink-100)', textTransform: 'uppercase' }}>First Cut</span>
        </div>
        <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
          <span
            ...
            style={{
              ...
              ...SKB, fontSize: 'var(--fs-11)', letterSpacing: '0.08em', color: 'rgb(var(--ink-rgb) / 0.6)', lineHeight: 1, display: 'inline-block',
          ...
          {!isSheet && (
          <svg
            width="11" height="11" viewBox="0 0 24 24" fill="none" stroke="rgb(var(--ink-rgb) / 0.5)" strokeWidth="3" strokeLinecap="round" strokeLinejoin="round"
      ...
      {expanded && holders !== null && (
        <div style={{ display: 'flex', flexDirection: 'column' }}>
          {filled === 0 && (
            <div
              className="fc-slot"
              style={{ ...SKR, fontSize: 'var(--fs-10)', color: 'var(--ink-100)', letterSpacing: '0.16em', textTransform: 'uppercase', padding: '2px 0 10px', animation: 'fcSlotRipple 0.32s cubic-bezier(0.16,0.84,0.3,1) both' }}
            >
              Be the first
            </div>
          )}
            ...
            return (
              <div
                ...
                style={{
                  ...
                  borderTop: '0.5px solid rgb(var(--ink-rgb) / 0.08)',
              ...
              >
                {/* rank number */}
                <span style={{ ...SKB, fontSize: 'var(--fs-10)', width: 16, color: h ? 'var(--ink-100)' : 'rgb(var(--ink-rgb) / 0.5)', letterSpacing: '0.02em' }}>
                  {rank}
                </span>
                ...
                {h ? (
                  h.avatarUrl
                    ...
                    : <div style={{ width: 22, height: 22, background: 'var(--surface-3)', flexShrink: 0 }} />
                ) : (
                  <div style={{ width: 22, height: 22, borderRadius: '50%', border: '1px solid rgb(var(--ink-rgb) / 0.25)', flexShrink: 0 }} />
                )}
                {/* handle (filled) or OPEN (empty) */}
                {h ? (
                  <span style={{ ...SKB, fontSize: 'var(--fs-11)', color: 'var(--ink-100)', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>
                    @{h.username ?? '—'}
                  </span>
                ) : (
                  <span style={{ ...SKR, fontSize: 'var(--fs-9)', letterSpacing: '0.22em', textTransform: 'uppercase', color: 'rgb(var(--ink-rgb) / 0.35)' }}>
                    Open
                  </span>
                )}
              </div>
      ...
      {fly && typeof document !== 'undefined' && createPortal(
        <img
          ...
          style={{
            ...
            filter: 'drop-shadow(0 0 9px rgb(var(--ink-rgb) / 0.65))',
