...
export default function TrackArt({
  ...
  return (
    <div style={{ ...box, position: "relative", display: "flex", alignItems: "center", justifyContent: "center" }}>
      {WITH_INITIAL && (
        <span style={{ ...SKB, position: "absolute", fontSize: size * 0.42, color: "rgb(var(--ink-rgb) / 0.14)", lineHeight: 1, userSelect: "none" }}>{initial}</span>
      )}
      <svg viewBox="0 0 100 100" width={size} height={size} preserveAspectRatio="none" style={{ position: "absolute", inset: 0 }}>
        <path d={d} fill="none" stroke="rgb(var(--ink-rgb) / 0.42)" strokeWidth={2} strokeLinecap="round" />
      </svg>
    </div>
  );
}
