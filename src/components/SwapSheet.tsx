    ...
    <>
      ...
      >
        ...
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 14 }}>
          <p style={{ ...SKB, fontSize: 'var(--fs-9)', color: 'rgb(var(--ink-rgb) / 0.45)', textTransform: 'uppercase', letterSpacing: '0.18em', margin: 0 }}>
        ...
        {phase === 'done' ? (
          <div style={{ textAlign: 'center', padding: '18px 0 6px' }}>
            ...
            <button onClick={onClose} style={{ width: '100%', background: 'transparent', border: '1px solid rgb(var(--ink-rgb) / 0.15)', cursor: 'pointer', padding: '12px 0' }}>
              <span style={{ ...SKB, fontSize: 'var(--fs-10)', color: 'rgb(var(--ink-rgb) / 0.6)', textTransform: 'uppercase', letterSpacing: '0.08em' }}>DONE</span>
            </button>
          </div>
        ) : (
          <>
            ...
            <div style={panel}>
              <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 8, alignItems: 'center' }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                  <span style={{ ...SKB, fontSize: 'var(--fs-8)', color: 'rgb(var(--ink-rgb) / 0.35)', textTransform: 'uppercase', letterSpacing: '0.14em' }}>YOU PAY</span>
                  ...
                  {sellToken === 'ETH' && buyToken === 'USDC' && spotUsd != null && (
                    <button
                      ...
                      style={{ ...SKB, fontSize: 'var(--fs-7)', letterSpacing: '0.08em', color: usdMode ? 'var(--ink-100)' : 'rgb(var(--ink-rgb) / 0.45)', background: 'transparent', border: '1px solid rgb(var(--ink-rgb) / 0.15)', cursor: 'pointer', padding: '2px 6px', textTransform: 'uppercase' }}
                    >
                      {usdMode ? '$ USD' : 'Ξ ETH'}
                    </button>
                  )}
                </div>
                <span style={{ ...SKR, fontSize: 'var(--fs-8)', color: 'rgb(var(--ink-rgb) / 0.35)', letterSpacing: '0.06em' }}>
                  BALANCE {fmtToken(payBalance, sellToken)} {sellToken}
                </span>
              </div>
              <div style={{ display: 'flex', alignItems: 'baseline', gap: 10 }}>
                {usdEntry && <span style={{ ...SKB, fontSize: 24, color: 'rgb(var(--ink-rgb) / 0.55)' }}>$</span>}
                <input
                  inputMode="decimal"
                  ...
                  placeholder="0"
                  style={{ ...SKB, fontSize: 24, color: 'var(--ink-100)', background: 'transparent', border: 'none', outline: 'none', width: '100%', padding: 0 }}
                />
                <span style={{ ...SKB, fontSize: 'var(--fs-12)', color: 'rgb(var(--ink-rgb) / 0.7)' }}>{usdEntry ? 'USD' : sellToken}</span>
                <button
                  ...
                  style={{ ...SKB, fontSize: 'var(--fs-9)', color: 'var(--ink-100)', background: 'transparent', border: 'none', cursor: 'pointer', letterSpacing: '0.08em', padding: 0 }}
                >
                  MAX
                </button>
              </div>
              ...
              {usdEntry && (
                <p style={{ ...SKR, fontSize: 'var(--fs-8)', color: 'rgb(var(--ink-rgb) / 0.4)', margin: '6px 0 0', letterSpacing: '0.04em' }}>
                  ≈ {validAmount ? fmtToken(payNum, 'ETH') : '0'} ETH · live rate
                </p>
              )}
              {overMax && (
                <p style={{ ...SKR, fontSize: 'var(--fs-9)', color: 'var(--ink-100)', margin: '8px 0 0', lineHeight: 1.4 }}>
                  {sellToken === 'ETH'
            ...
            <div style={panel}>
              <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 8 }}>
                <span style={{ ...SKB, fontSize: 'var(--fs-8)', color: 'rgb(var(--ink-rgb) / 0.35)', textTransform: 'uppercase', letterSpacing: '0.14em' }}>YOU RECEIVE</span>
              ...
              <p style={{ ...SKB, fontSize: 24, color: GREEN, margin: 0, fontVariantNumeric: 'tabular-nums' }}>
                ...
                <span style={{ fontSize: 'var(--fs-12)', color: 'rgb(var(--ink-rgb) / 0.7)' }}>{buyToken}</span>
              </p>
            </div>

            {/* Info line */}
            <p style={{ ...SKR, fontSize: 'var(--fs-8)', color: 'rgb(var(--ink-rgb) / 0.3)', margin: '10px 0 0', letterSpacing: '0.06em', textTransform: 'uppercase' }}>
            ...
            {phase === 'swapping' ? (
              <div style={{ width: '100%', border: '1px solid rgb(var(--ink-rgb) / 0.55)', padding: '13px 0', marginTop: 14, display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 10, minHeight: 46 }}>
                <ScopeLoader size="sm" />
              ...
              <button
                ...
                style={{ width: '100%', background: !validAmount || overMax || quoteOut == null ? 'rgb(var(--ink-rgb) / 0.4)' : 'var(--ink-100)', border: 'none', cursor: !validAmount || overMax || quoteOut == null ? 'default' : 'pointer', padding: '14px 0', marginTop: 14 }}
              >
                <span style={{ ...SKB, fontSize: 'var(--fs-12)', color: (!validAmount || overMax || quoteOut == null) ? 'var(--ink-100)' : 'var(--on-ink)', textTransform: 'uppercase', letterSpacing: '0.08em' }}>
