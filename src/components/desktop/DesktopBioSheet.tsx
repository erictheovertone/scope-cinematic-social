  ...
  return (
    <div style={{ display: 'flex', gap: 44, padding: '44px 0', borderTop: `1px solid ${HAIR}` }}>
      <div style={{ width: 250, flexShrink: 0 }}>
        ...
        <p style={{ ...SKB, fontSize: 'calc(32px * var(--type-scale))', lineHeight: 1, color: 'var(--ink-100)', textTransform: 'uppercase', letterSpacing: '-0.01em', margin: 0 }}>{label}</p>
        <div style={{ width: 32, height: 2, background: RED, margin: '13px 0 0' }} />
        {sub && <p style={{ ...SKR, fontSize: 11, color: 'rgb(var(--ink-rgb) / 0.45)', textTransform: 'uppercase', letterSpacing: '0.12em', margin: '13px 0 0' }}>{sub}</p>}
...
export default function DesktopBioSheet({ profile, isOwn, links, badges, posts, followers, following, collectors, totalPosts, decks, firstCutCount, onClose, onViewBadges, onMessage }: Props) {
  ...
  const stat = (label: string, value: string | number) => (
    <div style={{ display: 'flex', alignItems: 'baseline', justifyContent: 'space-between', gap: 16, padding: '7px 0' }}>
      <span style={{ ...SKB, fontSize: 10, color: 'rgb(var(--ink-rgb) / 0.5)', textTransform: 'uppercase', letterSpacing: '0.12em' }}>{label}</span>
      ...
      <div style={{ maxWidth: 'var(--shell-narrow)', margin: '0 auto', padding: '0 40px 80px' }}>{/* Brief R1 — reading/detail width = --shell-narrow (1180), intentionally tighter than the grid --shell-max */}
        ...
        <div style={{ position: 'relative', height: BANNER_H, margin: '0 -40px', overflow: 'hidden', background: 'var(--black)' }}>
          ...
          <div style={{ position: 'absolute', left: 40, top: 0, bottom: 0, width: 440, display: 'flex', flexDirection: 'column', justifyContent: 'center' }}>
            ...
            <h1 style={{ ...SKB, fontSize: 'calc(46px * var(--type-scale))', lineHeight: 1, letterSpacing: '-0.02em', color: 'var(--ink-100)', textTransform: 'uppercase', margin: 0 }}>{name}</h1>
            {handle && <p style={{ ...SKR, fontSize: 14, color: 'rgb(var(--ink-rgb) / 0.55)', textTransform: 'uppercase', letterSpacing: '0.08em', margin: '10px 0 0' }}>@{handle}</p>}
            {shortBio && <p style={{ ...SKR, fontSize: 14, color: 'rgb(var(--ink-rgb) / 0.7)', lineHeight: 1.6, margin: '18px 0 0', maxWidth: 400 }}>{shortBio}</p>}
            <button onClick={onMessage} style={{ ...SKB, alignSelf: 'flex-start', fontSize: 11, color: 'var(--ink-100)', textTransform: 'uppercase', letterSpacing: '0.1em', background: 'transparent', border: '1px solid rgb(var(--ink-rgb) / 0.4)', cursor: 'pointer', padding: '12px 22px', margin: '26px 0 0' }}>MESSAGE {name.split(' ')[0]}</button>
          </div>
        ...
        {/* ═══ 2. BANDS ═══ */}
        {badges.length > 0 && (
          <Band label="BADGES" sub={`${badges.length} / ${TOTAL_BADGES} UNLOCKED`} action={<button onClick={onViewBadges} style={{ ...SKB, fontSize: 10, color: 'rgb(var(--ink-rgb) / 0.6)', textTransform: 'uppercase', letterSpacing: '0.1em', background: 'transparent', border: 'none', cursor: 'pointer', padding: 0 }}>VIEW ALL BADGES →</button>}>
            ...
            <style>{`
              ...
              .d12-badge:hover,.d12-badge:focus-visible{opacity:1;border-color:rgb(var(--ink-rgb) / 0.18);background:rgb(var(--ink-rgb) / 0.03);outline:none}
            `}</style>
        ...
        {longBio && (
          <Band label="BIO">
            <div style={{ position: 'relative' }}>
              <span style={{ position: 'absolute', top: -18, right: 0, ...SKB, fontSize: 'calc(70px * var(--type-scale))', color: 'rgb(var(--ink-rgb) / 0.08)', lineHeight: 1 }}>&rdquo;</span>
              <p style={{ ...SKR, fontSize: 15, color: 'rgb(var(--ink-rgb) / 0.72)', lineHeight: 1.7, margin: 0, maxWidth: 620 }}>{longBio}</p>
            </div>
          </Band>
        )}
          ...
          <Band label="MUSIC">
            <div>
              {tracks.map((t, i) => (
                ...
                >
                  <span style={{ ...SKR, fontSize: 14, color: 'rgb(var(--ink-rgb) / 0.8)', letterSpacing: '0.01em', minWidth: 0, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{t.title}</span>
                  <span style={{ ...SKR, fontSize: 12, color: 'rgb(var(--ink-rgb) / 0.4)', flexShrink: 0, fontVariantNumeric: 'tabular-nums' }}>{fmtDur(t.duration_seconds)}</span>
                </button>
              ))}
              <button
                onClick={() => { if (handle) { onClose(); router.push(`/composer/${handle}`); } }}
                style={{ ...SKB, display: 'block', marginTop: 18, fontSize: 10, color: 'rgb(var(--ink-rgb) / 0.6)', textTransform: 'uppercase', letterSpacing: '0.1em', background: 'transparent', border: 'none', cursor: 'pointer', padding: 0 }}
        ...
        {links.length > 0 && (
          <Band label="LINKS" action={<span style={{ ...SKB, fontSize: 10, color: 'rgb(var(--ink-rgb) / 0.6)', textTransform: 'uppercase', letterSpacing: '0.1em' }}>VIEW ALL LINKS →</span>}>
            <div style={{ display: 'flex', gap: 16, flexWrap: 'wrap' }}>
                  ...
                  <button key={l.id} onClick={() => window.open(l.url, '_blank')} style={{ width: 236, textAlign: 'left', background: 'var(--canvas)', border: `1px solid ${HAIR}`, cursor: 'pointer', padding: 0, overflow: 'hidden' }}>
                    ...
                    <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 8, padding: '11px 13px' }}>
                      <div style={{ minWidth: 0 }}>
                        <p style={{ ...SKB, fontSize: 11, color: 'var(--ink-100)', textTransform: 'uppercase', letterSpacing: '0.04em', margin: 0, whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>{l.title || l.url}</p>
                        <p style={{ ...SKR, fontSize: 9.5, color: 'rgb(var(--ink-rgb) / 0.4)', margin: '3px 0 0', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>{l.url.replace(/^https?:\/\//, '')}</p>
                      </div>
                      <span style={{ ...SKB, fontSize: 13, color: 'rgb(var(--ink-rgb) / 0.5)', flexShrink: 0 }}>→</span>
                    </div>
                  </button>
                );
              })}
        ...
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 14, padding: '48px 0 0', borderTop: `1px solid ${HAIR}`, marginTop: 12 }}>
          ...
          <span style={{ ...SKB, fontSize: 11, color: 'rgb(var(--ink-rgb) / 0.6)', textTransform: 'uppercase', letterSpacing: '0.3em' }}>CREATE. CONNECT. COLLECT.</span>
          <span style={{ ...SKB, fontSize: 20, color: RED, lineHeight: 1 }}>]</span>
        </div>
      </div>
    </div>,
