...
export default function DesktopDeck({ deckId }: { deckId: string }) {
  ...
  const menuItem = (label: string, fn: () => void, danger?: boolean) => (
    <button onClick={() => { setMenuOpen(false); fn(); }} style={{ ...SKB, display: 'block', width: '100%', textAlign: 'left', fontSize: 11, color: danger ? RED : 'var(--ink-100)', textTransform: 'uppercase', letterSpacing: '0.1em', background: 'transparent', border: 'none', borderBottom: `1px solid ${HAIR}`, cursor: 'pointer', padding: '12px 16px' }}>{label}</button>
  );
  ...
  return (
    <div className="bg-black" style={{ position: 'fixed', inset: 0, left: 'var(--rail-w)', background: '#000', overflowY: 'auto', WebkitOverflowScrolling: 'touch' }}>
      <DesktopShell width="fluid" padding="28px 48px 96px">{/* Brief R1a — media surface: fills the window, deck grid grows columns */}
        ...
        <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', gap: 16, marginBottom: 26 }}>
          <div style={{ minWidth: 0 }}>
            <button onClick={() => router.back()} style={{ ...SKR, fontSize: 12, color: 'rgb(var(--ink-rgb) / 0.5)', textTransform: 'uppercase', letterSpacing: '0.1em', background: 'transparent', border: 'none', cursor: 'pointer', padding: '0 0 12px' }}>← BACK</button>
            <h1 style={{ ...SKB, fontSize: 'calc(34px * var(--type-scale))', lineHeight: 1, letterSpacing: '-0.02em', color: 'var(--ink-100)', textTransform: 'uppercase', margin: 0 }}>{loading ? '' : (deck?.title ?? 'DECK NOT FOUND')}</h1>
            {deck && <p style={{ ...SKR, fontSize: 12, color: 'rgb(var(--ink-rgb) / 0.45)', margin: '8px 0 0' }}>{deck.item_count} {deck.item_count === 1 ? 'POST' : 'POSTS'}{deck.description ? ` · ${deck.description}` : ''}</p>}
          </div>
          {isOwn && deck && (
          ...
          <div ref={gridRef} style={{ display: 'grid', gridTemplateColumns: `repeat(${gridCols}, 1fr)`, gap: 6 }}>
            {isOwn && (
              <button onClick={() => fileRef.current?.click()} disabled={busy} style={{ aspectRatio: `${aspect}`, border: `1px dashed ${HAIR}`, background: 'transparent', cursor: 'pointer', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: 8 }}>
                <svg width="30" height="30" viewBox="0 0 34 34" fill="none"><path d="M17 6v22M6 17h22" stroke="rgb(var(--ink-rgb) / 0.7)" strokeWidth="1" /></svg>
                <span style={{ ...SKB, fontSize: 10, color: 'rgb(var(--ink-rgb) / 0.7)', textTransform: 'uppercase', letterSpacing: '0.12em' }}>{busy ? 'ADDING…' : 'ADD POSTS'}</span>
              </button>
            )}
            {deck.items.map((it) => {
            ...
            {deck.items.length === 0 && !isOwn && <p style={{ ...SKR, gridColumn: `span ${count}`, fontSize: 12, color: 'rgb(var(--ink-rgb) / 0.4)', textTransform: 'uppercase', textAlign: 'center', padding: '60px 0' }}>EMPTY DECK</p>}
          </div>
        )}
      ...
      <style>{`.dk-deck-cell:hover { outline: 1px solid rgb(var(--ink-rgb) / 0.25); }`}</style>
    </div>
  );
}
