  ...
  return createPortal(
    <div data-swipe-exclude style={{ position: 'fixed', inset: 0, zIndex: 700, background: 'var(--black)', overflowY: 'auto' }}>
      ...
      <div style={{ position: 'absolute', top: 22, left: 26, display: 'flex', gap: 22 }}>
        ...
        {step !== 'aspect' && (
          <button onClick={() => setStep(step === 'commit' ? 'count' : 'aspect')} style={{ ...SKR, fontSize: 12, color: 'rgb(var(--ink-rgb) / 0.55)', textTransform: 'uppercase', letterSpacing: '0.1em', background: 'transparent', border: 'none', cursor: 'pointer', padding: 0 }}>← BACK</button>
        )}
      </div>
      {echo && (
      ...
      <div style={{ maxWidth: 900, margin: '0 auto', padding: '64px 40px 80px', textAlign: 'center' }}/* R1: bespoke width — flagged */>
        <CornerBrackets>
          <h1 style={{ ...SKB, fontSize: 'calc(72px * var(--type-scale))', color: 'var(--ink-100)', textTransform: 'uppercase', letterSpacing: '-0.01em', margin: 0, lineHeight: 1 }}>{title}</h1>
        </CornerBrackets>
        ...
        {step === 'aspect' && (
          <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 26, marginTop: 48 }}>
            {DESKTOP_ASPECTS.map((c) => (
              ...
              >
                <span style={{ display: 'block', width: 300, aspectRatio: `${c.ratio}`, border: `1px solid ${aspect === c.id ? RED : 'rgb(var(--ink-rgb) / 0.35)'}`, transition: 'border-color 140ms ease' }} />
                <span style={{ ...SKB, fontSize: 12, color: aspect === c.id ? RED : 'rgb(var(--ink-rgb) / 0.65)', textTransform: 'uppercase', letterSpacing: '0.12em', display: 'block', marginTop: 8 }}>
              ...
              >
                <span style={{ display: 'grid', gridTemplateColumns: `repeat(${n}, 1fr)`, gap: 5 }}>
                  {Array.from({ length: n }).map((_, i) => (
                    <span key={i} style={{ display: 'block', aspectRatio: `${chip.ratio}`, border: `1px solid ${count === n ? RED : 'rgb(var(--ink-rgb) / 0.35)'}`, transition: 'border-color 140ms ease' }} />
                  ))}
                </span>
                <span style={{ ...SKB, fontSize: 12, color: count === n ? RED : 'rgb(var(--ink-rgb) / 0.65)', textTransform: 'uppercase', letterSpacing: '0.12em', display: 'block', marginTop: 8 }}>
                  {n}-ACROSS
                </span>
              </button>
            ))}
        ...
        {step === 'commit' && (
          <div style={{ marginTop: 48 }}>
            ...
            <p style={{ ...SKR, fontSize: 11, color: 'rgb(var(--ink-rgb) / 0.45)', textTransform: 'uppercase', letterSpacing: '0.1em', margin: '34px 0 0' }}>
            ...
            <button
              onClick={confirm}
              disabled={saving}
              style={{ ...SKB, fontSize: 13, color: 'var(--black)', textTransform: 'uppercase', letterSpacing: '0.12em', background: 'var(--ink-100)', border: 'none', cursor: 'pointer', padding: '14px 54px', marginTop: 22, opacity: saving ? 0.6 : 1 }}
            >
