...
const HAIR = 'rgb(var(--ink-rgb) / 0.12)';
...
const LABEL: React.CSSProperties = { ...SKB, fontSize: 11, color: 'rgb(var(--ink-rgb) / 0.45)', textTransform: 'uppercase', letterSpacing: '0.14em', display: 'block', margin: '0 0 7px' };
const INPUT: React.CSSProperties = { ...SKR, fontSize: 13, color: 'var(--ink-100)', background: 'rgb(var(--ink-rgb) / 0.04)', border: `1px solid ${HAIR}`, outline: 'none', padding: '10px 12px', width: '100%', boxSizing: 'border-box' };
...
export default function DesktopSettings() {
  ...
  const row = (label: string, action: () => void, key?: string) => (
    <button
      ...
      style={{ display: 'block', width: '100%', textAlign: 'left', cursor: 'pointer', background: hovered === (key ?? label) ? 'rgb(var(--ink-rgb) / 0.045)' : 'transparent', border: 'none', borderBottom: `1px solid ${HAIR}`, padding: '15px 10px', transition: 'background 120ms ease' }}
    >
      <span style={{ ...SKB, fontSize: 12.5, color: 'var(--ink-100)', textTransform: 'uppercase', letterSpacing: '0.06em' }}>{label}</span>
    </button>
  );
          ...
          <div style={{ maxWidth: 520 }}>
            ...
            <div style={{ marginBottom: 20 }}>
              ...
              <div style={{ display: 'flex', alignItems: 'baseline', gap: 8 }}>
                ...
                <span style={{ ...SKR, fontSize: 10, color: 'rgb(var(--ink-rgb) / 0.4)', fontVariantNumeric: 'tabular-nums', flexShrink: 0 }}>{shortBio.length}/80</span>
              </div>
            </div>
            ...
            {tierFlags.isFoundingMember && (
              <div style={{ marginBottom: 22 }}>
                ...
                <button onClick={() => setHoloBanner((h) => !h)} style={{ ...SKB, fontSize: 10, color: holoBanner ? 'var(--ink-100)' : 'rgb(var(--ink-rgb) / 0.5)', textTransform: 'uppercase', letterSpacing: '0.08em', background: 'transparent', border: `1px solid ${HAIR}`, cursor: 'pointer', padding: '6px 12px' }}>
            ...
            <div style={{ marginBottom: 22 }}>
              ...
              <div style={{ display: 'flex', gap: 8 }}>
                ...
                <button onClick={() => setContactPublic((p) => !p)} style={{ ...SKB, fontSize: 10, color: contactPublic ? 'var(--ink-100)' : 'rgb(var(--ink-rgb) / 0.5)', textTransform: 'uppercase', letterSpacing: '0.06em', background: 'transparent', border: `1px solid ${HAIR}`, cursor: 'pointer', padding: '0 12px', flexShrink: 0 }}>
            ...
            <div style={{ marginBottom: 26 }}>
              ...
              {links.map((l) => (
                <div key={l.id} style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '8px 0', borderBottom: `1px solid ${HAIR}` }}>
                  <span style={{ ...SKR, fontSize: 12, color: 'rgb(var(--ink-rgb) / 0.7)', flex: 1, minWidth: 0, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{l.title || l.url}</span>
                  <button
                    onClick={async () => { if (user && await setPrimaryLink(user.id, l.id)) setLinks((prev) => prev.map((x) => ({ ...x, is_primary: x.id === l.id } as ProfileLink))); }}
                    style={{ ...SKB, fontSize: 9, color: (l as { is_primary?: boolean }).is_primary ? RED : 'rgb(var(--ink-rgb) / 0.4)', background: 'transparent', border: 'none', cursor: 'pointer', textTransform: 'uppercase', letterSpacing: '0.06em', flexShrink: 0 }}
                  >
                    {(l as { is_primary?: boolean }).is_primary ? 'PRIMARY' : 'SET PRIMARY'}
                  </button>
                  <button onClick={async () => { await deleteProfileLink(l.id); setLinks((prev) => prev.filter((x) => x.id !== l.id)); }} style={{ ...SKB, fontSize: 13, color: 'rgb(var(--ink-rgb) / 0.35)', background: 'transparent', border: 'none', cursor: 'pointer', flexShrink: 0 }}>✕</button>
                </div>
              ))}
              <div style={{ display: 'flex', gap: 8, marginTop: 10 }}>
                ...
                <button onClick={addLink} style={{ ...SKB, fontSize: 11, color: 'var(--ink-100)', textTransform: 'uppercase', background: 'transparent', border: `1px solid ${HAIR}`, cursor: 'pointer', padding: '0 14px' }}>ADD</button>
              </div>
            </div>
            ...
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 16, padding: '16px 18px', border: `1px solid ${HAIR}`, marginBottom: membership.isPaid ? 18 : 0 }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: 12, minWidth: 0 }}>
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: 3, minWidth: 0 }}>
                  ...
                  <span style={{ ...SKB, fontSize: 13, color: 'var(--ink-100)', textTransform: 'uppercase', letterSpacing: '0.06em', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>
                    {membershipBarLabel(membership)}
                  </span>
                </div>
              </div>
              {!membership.isPaid && (
                <button onClick={() => goPro()} style={{ ...SKB, fontSize: 11, color: 'var(--ink-100)', textTransform: 'uppercase', letterSpacing: '0.08em', background: RED, border: 'none', cursor: 'pointer', padding: '10px 20px', flexShrink: 0 }}>GET PRO →</button>
              )}
            </div>
            ...
            {membership.isPaid && (
              <div>
                {membership.cancelsAt ? (
                  <div>
                    <p style={{ ...SKR, fontSize: 12, color: 'rgb(var(--ink-rgb) / 0.5)', lineHeight: 1.6, margin: '0 0 12px' }}>
                      Your membership is set to cancel — Pro stays active until then.
                    </p>
                    <button onClick={memResume} disabled={memBusy === 'resume'} style={{ ...SKB, fontSize: 11, color: 'var(--ink-100)', textTransform: 'uppercase', letterSpacing: '0.08em', background: RED, border: 'none', cursor: memBusy ? 'default' : 'pointer', padding: '12px 24px', width: '100%' }}>
                      {memBusy === 'resume' ? 'RESUMING…' : 'RESUME MEMBERSHIP'}
                    </button>
                  </div>
                ) : !memConfirmCancel ? (
                  <button onClick={() => setMemConfirmCancel(true)} style={{ ...SKB, fontSize: 11, color: 'rgb(var(--ink-rgb) / 0.45)', textTransform: 'uppercase', letterSpacing: '0.08em', background: 'transparent', border: `1px solid ${HAIR}`, cursor: 'pointer', padding: '12px 24px', width: '100%' }}>
                    CANCEL MEMBERSHIP
                  </button>
                ) : (
                  <div style={{ border: '1px solid rgb(var(--ink-rgb) / 0.3)', padding: 20 }}>
                    <p style={{ ...SKB, fontSize: 13, color: 'var(--ink-100)', textTransform: 'uppercase', margin: '0 0 8px' }}>ARE YOU SURE?</p>
                    <p style={{ ...SKR, fontSize: 12, color: 'rgb(var(--ink-rgb) / 0.5)', lineHeight: 1.6, margin: '0 0 18px' }}>
                      You&apos;ll keep Pro until the end of your billing period, then revert to free.
                    </p>
                    <div style={{ display: 'flex', gap: 10 }}>
                      <button onClick={() => setMemConfirmCancel(false)} style={{ ...SKB, flex: 1, fontSize: 10, color: 'rgb(var(--ink-rgb) / 0.6)', textTransform: 'uppercase', background: 'transparent', border: `1px solid ${HAIR}`, cursor: 'pointer', padding: '11px 0' }}>KEEP PRO</button>
                      <button onClick={memCancel} disabled={memBusy === 'cancel'} style={{ ...SKB, flex: 1, fontSize: 10, color: RED, textTransform: 'uppercase', background: 'rgb(var(--ink-rgb) / 0.15)', border: '1px solid rgb(var(--ink-rgb) / 0.4)', cursor: memBusy ? 'default' : 'pointer', padding: '11px 0' }}>
    ...
    >
      <div style={{ display: 'flex', height: '100%', maxWidth: 1100, margin: '0 auto' }}>
        ...
        <div style={{ width: 250, flexShrink: 0, borderRight: `1px solid ${HAIR}`, padding: '44px 0 40px', display: 'flex', flexDirection: 'column', height: '100%', boxSizing: 'border-box' }}>
          <p style={{ ...SKB, fontSize: 11, color: 'rgb(var(--ink-rgb) / 0.38)', textTransform: 'uppercase', letterSpacing: '0.22em', margin: '0 0 18px', paddingLeft: 18 }}>SETTINGS</p>
          {SECTIONS.map((s) => {
            ...
            return (
              ...
              >
                ...
                <span style={{ ...SKB, fontSize: 12.5, textTransform: 'uppercase', letterSpacing: '0.08em', color: active ? RED : hovered === `cat-${s}` ? 'var(--ink-100)' : 'rgb(var(--ink-rgb) / 0.55)', transition: 'color 120ms ease' }}>
                  {SECTION_LABELS[s]}
                </span>
              </button>
            );
        ...
        <div ref={panelRef} style={{ flex: 1, padding: '44px 36px 80px', opacity: fade ? 0 : 1, transition: 'opacity 120ms ease', height: '100%', boxSizing: 'border-box', overflowY: 'auto', WebkitOverflowScrolling: 'touch' }}>
          <h2 style={{ ...SKB, fontSize: 16, color: 'var(--ink-100)', textTransform: 'uppercase', letterSpacing: '0.1em', margin: '0 0 26px' }}>{SECTION_LABELS[section]}</h2>
          {panel()}
        </div>
      </div>
      ...
      {moreFromOpen && createPortal(
        <div data-swipe-exclude style={{ position: 'fixed', inset: 0, zIndex: 680, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
          ...
          <div style={{ position: 'relative', width: 760, maxHeight: '82vh', display: 'flex', flexDirection: 'column', background: 'var(--black)', border: '1px solid var(--surface-3)', padding: '32px 34px' }}>
            ...
            <p style={{ ...SKR, fontSize: 12, color: 'rgb(var(--ink-rgb) / 0.5)', lineHeight: 1.6, margin: '0 0 20px' }}>
            ...
            <div style={{ flex: 1, overflowY: 'auto', display: 'grid', gridTemplateColumns: 'repeat(4, minmax(0, 1fr))', gap: 12 }}>
              {myPosts.length === 0 ? (
                <p style={{ ...SKR, gridColumn: 'span 4', fontSize: 12, color: 'rgb(var(--ink-rgb) / 0.4)', textAlign: 'center', padding: '40px 0' }}>NO POSTS YET</p>
              ) : myPosts.map((p) => {
