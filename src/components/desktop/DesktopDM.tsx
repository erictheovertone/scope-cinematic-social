...
export default function DesktopDM({ initialUsername }: { initialUsername?: string }) {
  ...
  return (
    <div style={{ position: 'fixed', inset: 0, left: 'var(--rail-w)', background: 'var(--canvas)' }}>
      ...
      <div style={{ width: 340, flexShrink: 0, borderRight: `1px solid ${HAIR}`, overflowY: 'auto', display: 'flex', flexDirection: 'column' }}>
        <div style={{ padding: '22px 20px 14px' }}>
          <h1 style={{ ...SKB, fontSize: 15, color: 'var(--ink-100)', textTransform: 'uppercase', letterSpacing: '0.16em', margin: 0 }}>Messages</h1>
        </div>
        {convs.length === 0 ? (
          <div style={{ padding: '26px 20px' }}>
            <span style={{ ...SKR, fontSize: 12, color: 'rgb(var(--ink-rgb) / 0.35)', textTransform: 'uppercase', letterSpacing: '0.08em', lineHeight: 1.6 }}>
          ...
          return (
            <button
              ...
              style={{
                ...
                background: sel ? 'rgb(var(--ink-rgb) / 0.05)' : 'transparent',
                border: 'none', borderBottom: `1px solid ${HAIR}`,
                borderLeft: sel ? '2px solid var(--ink-100)' : '2px solid transparent',
            ...
            >
              ...
              <div style={{ flex: 1, minWidth: 0 }}>
                <span style={{ ...SKB, fontSize: 12, color: 'var(--ink-100)', textTransform: 'uppercase', letterSpacing: '0.02em', display: 'block' }}>@{c.otherHandle ?? 'unknown'}</span>
                <span style={{ ...(unread ? SKB : SKR), fontSize: 11, color: unread ? 'rgb(var(--ink-rgb) / 0.9)' : 'rgb(var(--ink-rgb) / 0.45)', display: 'block', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis', marginTop: 3 }}>{c.lastMessagePreview ?? ''}</span>
              </div>
              <div style={{ flexShrink: 0, display: 'flex', flexDirection: 'column', alignItems: 'flex-end', gap: 6 }}>
                <span style={{ ...SKR, fontSize: 9, color: 'rgb(var(--ink-rgb) / 0.4)', fontVariantNumeric: 'tabular-nums' }}>{dmTimeAgo(c.lastMessageAt)}</span>
                {unread && <span style={{ width: 8, height: 8, borderRadius: '50%', background: 'var(--ink-100)', display: 'block' }} />}
              </div>
            </button>
          );
        })}
      ...
      <div style={{ flex: 1, minWidth: 0, display: 'flex', flexDirection: 'column' }}>
        {active && viewerUuid && user?.id ? (
          ...
          <div style={{ margin: 'auto' }}>
            <span style={{ ...SKR, fontSize: 13, color: 'rgb(var(--ink-rgb) / 0.3)', textTransform: 'uppercase', letterSpacing: '0.1em' }}>Select a conversation</span>
          </div>
        )}
      </div>
      </DesktopShell>
    ...
    <div style={{ display: 'flex', flexDirection: 'column', height: '100%' }}>
      ...
      <div style={{ flexShrink: 0, display: 'flex', alignItems: 'center', gap: 11, padding: '16px 22px', borderBottom: `1px solid ${HAIR}` }}>
        <button onClick={onOpenProfile} style={{ display: 'flex', alignItems: 'center', gap: 11, background: 'transparent', border: 'none', cursor: 'pointer', padding: 0, minWidth: 0 }}>
          ...
          <span style={{ ...SKB, fontSize: 13, color: 'var(--ink-100)', textTransform: 'uppercase', letterSpacing: '0.04em' }}>@{other.handle}</span>
        </button>
      </div>
      ...
      <div ref={scrollRef} onScroll={onScroll} style={{ flex: 1, overflowY: 'auto', padding: '18px 22px 10px', display: 'flex', flexDirection: 'column' }}>
        {loading ? (
          <div style={{ margin: 'auto' }}><span style={{ ...SKB, fontSize: 12, color: 'rgb(var(--ink-rgb) / 0.4)', textTransform: 'uppercase', letterSpacing: '0.08em' }}>LOADING…</span></div>
        ) : items.length === 0 ? (
          <div style={{ margin: 'auto' }}><span style={{ ...SKR, fontSize: 12, color: 'rgb(var(--ink-rgb) / 0.35)', textTransform: 'uppercase', letterSpacing: '0.08em' }}>No messages yet — say hello.</span></div>
        ) : items.map((it, i) => {
            ...
            <div key={it.key}>
              {showTime && (
                <div style={{ textAlign: 'center', margin: '12px 0 8px' }}>
                  <span style={{ ...SKR, fontSize: 9, color: 'rgb(var(--ink-rgb) / 0.3)', textTransform: 'uppercase', letterSpacing: '0.1em' }}>{dmTimeAgo(it.iso)}</span>
                </div>
              )}
              <div style={{ display: 'flex', justifyContent: it.mine ? 'flex-end' : 'flex-start', marginBottom: 5 }}>
                ...
                >
                  <span style={{ ...SKR, fontSize: 13, color: 'var(--ink-100)', lineHeight: 1.45, whiteSpace: 'pre-wrap', wordBreak: 'break-word', display: 'block' }}>{it.body}</span>
                  {it.status === 'failed' && (
                    <span style={{ ...SKB, fontSize: 9, color: 'var(--ink-100)', textTransform: 'uppercase', letterSpacing: '0.08em', display: 'block', marginTop: 4 }}>Failed — click to retry</span>
                  )}
                </div>
              </div>
            </div>
      ...
      <div style={{ flexShrink: 0, display: 'flex', alignItems: 'flex-end', gap: 10, padding: '14px 22px', borderTop: `1px solid ${HAIR}` }}>
        <textarea
          ...
          maxLength={2000}
          autoFocus
          style={{ flex: 1, resize: 'none', maxHeight: 140, ...SKR, fontSize: 14, color: 'var(--ink-100)', background: 'rgb(var(--ink-rgb) / 0.04)', border: `1px solid ${HAIR}`, borderRadius: 2, outline: 'none', padding: '10px 12px', lineHeight: 1.4 }}
        />
        <button
          onClick={send}
          disabled={!draft.trim()}
          style={{ ...SKB, fontSize: 12, color: draft.trim() ? 'var(--ink-100)' : 'rgb(var(--ink-rgb) / 0.3)', background: 'transparent', border: 'none', cursor: draft.trim() ? 'pointer' : 'default', textTransform: 'uppercase', letterSpacing: '0.1em', padding: '10px 6px', flexShrink: 0 }}
        >
          Send
        </button>
      </div>
