...
const ARROWS: Record<string, React.ReactNode> = {
  DEPOSIT: <svg width="28" height="28" viewBox="0 0 22 22" fill="none" stroke="rgb(var(--ink-rgb) / 0.82)" strokeWidth="1.3" strokeLinecap="round" strokeLinejoin="round"><line x1="11" y1="3" x2="11" y2="18" /><path d="M5 12 L11 18 L17 12" /></svg>,
  SWAP: <svg width="33" height="23" viewBox="0 0 26 18" fill="none" stroke="rgb(var(--ink-rgb) / 0.82)" strokeWidth="1.3" strokeLinecap="round" strokeLinejoin="round"><path d="M3 6 H21 M17 2 L21 6 L17 10" /><path d="M23 12 H5 M9 8 L5 12 L9 16" /></svg>,
  SEND: <svg width="25" height="25" viewBox="0 0 20 20" fill="none" stroke="rgb(var(--ink-rgb) / 0.82)" strokeWidth="1.3" strokeLinecap="round" strokeLinejoin="round"><line x1="4" y1="16" x2="15" y2="5" /><path d="M6 5 H15 V14" /></svg>,
};
...
const Fiat = ({ n, o = 0.9 }: { n: number | null; o?: number }) => (
  <span style={{ display: 'inline-flex', alignItems: 'baseline', flexShrink: 0 }}>
    <span style={{ ...FM, fontSize: 13, color: `rgb(var(--ink-rgb) / ${o})`, marginRight: '0.14em' }}>$</span>
    <span style={{ ...FD, fontSize: 13, color: `rgb(var(--ink-rgb) / ${o})`, fontVariantNumeric: 'tabular-nums' }}>{n != null ? n.toFixed(2) : '—'}</span>
  </span>
);
      ...
      <div style={{ maxWidth: 'var(--shell-narrow)', margin: '0 auto', padding: '40px 24px 80px' }}/* Brief R1a §2 — capped reading surface (--shell-narrow) */>
        ...
        <PageTitle title="Wallet" paddingBottom={20}>
          {walletAddress && (
            ...
            >
              <span style={{ ...FM, fontSize: 11, letterSpacing: 'var(--track-body)', color: copied ? GREEN : 'rgb(var(--ink-rgb) / 0.5)', fontVariantNumeric: 'tabular-nums' }}>
        ...
        <div style={{ display: 'flex', gap: 40, marginTop: 28 }}>
          ...
          <div style={{ width: 420, flexShrink: 0 }}>
            ...
            <LedgerCard variant="border" radius={10} style={{ padding: '18px 24px 22px' }}>
              <div style={{ textAlign: 'center' }}>
                <p style={{ ...FD, fontSize: 14, color: 'rgb(var(--ink-rgb) / 0.6)', letterSpacing: 'var(--track-body)', margin: 0 }}>Total Balance</p>
                ...
                {([
                  ...
                  <button key={label} onClick={onClick} style={{ display: 'flex', alignItems: 'center', gap: 8, background: 'none', border: 'none', padding: 0, cursor: 'pointer', width: '100%' }}>
                    <span style={{ display: 'inline-flex', alignItems: 'baseline', gap: 4, flexShrink: 0 }}>
                      <span style={{ ...FM, fontSize: 11.8, color: 'rgb(var(--ink-rgb) / 0.75)', letterSpacing: 'var(--track-body)', whiteSpace: 'nowrap' }}>{label}</span>
                      {info && <span style={{ display: 'inline-flex', alignItems: 'center', justifyContent: 'center', width: 8, height: 8, border: '0.5px solid rgb(var(--ink-rgb) / 0.5)', transform: 'translateY(-2px)', ...FM, fontSize: 6, color: 'rgb(var(--ink-rgb) / 0.5)' }}>i</span>}
                    </span>
                    <DottedLeader />
                    <span style={{ display: 'inline-flex', alignItems: 'baseline', flexShrink: 0 }}>
                      <span style={{ ...FM, fontSize: 13, color: 'rgb(var(--ink-rgb) / 0.79)', marginRight: '0.14em' }}>$</span>
                      <span style={{ ...FD, fontSize: 13, color: 'rgb(var(--ink-rgb) / 0.79)', fontVariantNumeric: 'tabular-nums' }}>{value != null ? value.toFixed(2) : '—'}</span>
                    </span>
                  </button>
                ))}
              </div>
            ...
            <div style={{ display: 'flex', gap: 10, marginTop: 16 }}>
              {([
                ...
                <LedgerCard key={card.label} variant="gradient" radius={6} role="button" tabIndex={0} onClick={card.onClick} style={{ flex: 1, aspectRatio: '112 / 105', cursor: 'pointer', display: 'flex', flexDirection: 'column', padding: '12px 11px', boxSizing: 'border-box', overflow: 'hidden' }}>
                  <span style={{ ...FD, fontSize: 14, letterSpacing: 'var(--track-display)', color: 'rgb(var(--ink-rgb) / 0.67)', textTransform: 'uppercase' }}>{card.label}</span>
                  <span style={{ ...FM, fontSize: 9.5, color: 'rgb(var(--ink-rgb) / 0.43)', letterSpacing: 'var(--track-body)', marginTop: 3, lineHeight: 1.2 }}>{card.sub}</span>
          ...
          <div style={{ flex: 1, minWidth: 0 }}>
            ...
            <div style={{ background: 'rgb(var(--ink-rgb) / 0.035)', borderRadius: 6, padding: '0 10px', display: 'flex', alignItems: 'center', gap: 26 }}>
              {(['balances', 'holdings', 'earnings', 'activity'] as Tab[]).map((t) => (
                <button key={t} onClick={() => setTab(t)} style={{ background: 'none', border: 'none', cursor: 'pointer', padding: '10px 0', display: 'inline-flex', alignItems: 'center', gap: 5 }}>
                  ...
                  <span style={{ ...FM, fontSize: 12, letterSpacing: 'var(--track-body)', color: tab === t ? 'var(--ink-100)' : 'rgb(var(--ink-rgb) / 0.67)' }}>{t.charAt(0).toUpperCase() + t.slice(1)}</span>
                </button>
              ))}
            </div>
            ...
            {tab === 'balances' && (
              <>
                ...
                <button className="ledger-row" onClick={() => setShowImport(true)} style={{ display: 'flex', alignItems: 'center', width: '100%', height: 62, padding: '0 14px', boxSizing: 'border-box', background: 'transparent', border: 'none', cursor: 'pointer', textAlign: 'left' }}>
                  <span style={{ ...FM, fontSize: 10, color: 'rgb(var(--ink-rgb) / 0.45)', letterSpacing: 'var(--track-body)', textTransform: 'uppercase' }}>+ ADD / IMPORT ASSET</span>
                </button>
              </>
            )}
            ...
            {tab === 'holdings' && (
              <div style={{ padding: '4px 14px' }}>
                {holdings === null ? (
                  <p style={{ ...FB, fontSize: 11, color: 'rgb(var(--ink-rgb) / 0.4)', textTransform: 'uppercase', padding: '18px 0' }}>LOADING…</p>
                ) : holdings.length === 0 ? (
                  <p style={{ ...FB, fontSize: 11, color: 'rgb(var(--ink-rgb) / 0.4)', textTransform: 'uppercase', padding: '18px 0' }}>NO POSITIONS YET</p>
                ) : holdings.map((h) => (
                  ...
                  <button key={h.postId} onClick={() => openPostLightbox(h.postId)} className="tappable ledger-row" style={{ display: 'flex', alignItems: 'center', gap: 14, padding: '11px 0', width: '100%', textAlign: 'left', background: 'transparent', border: 'none', borderBottom: `1px solid ${HAIR}`, cursor: 'pointer' }}>
                    ...
                    <span style={{ ...FD, fontSize: 13.5, color: 'var(--ink-100)', fontVariantNumeric: 'tabular-nums' }}>{usd(h.valueUsd)}</span>
                  </button>
                ))}
              </div>
            )}
                ...
                {([
                  ...
                  <div key={key}>
                    <button onClick={() => setOpenCat(openCat === key ? null : key)} style={{ display: 'flex', width: '100%', alignItems: 'baseline', justifyContent: 'space-between', background: 'transparent', border: 'none', cursor: 'pointer', padding: '15px 2px 12px' }}>
                      <span style={{ ...FD, fontSize: 12, color: 'var(--ink-100)', textTransform: 'uppercase', letterSpacing: '0.06em' }}>{label}</span>
                      <span style={{ display: 'flex', alignItems: 'baseline', gap: 10 }}>
                        <span style={{ ...FB, fontSize: 9.5, color: 'rgb(var(--ink-rgb) / 0.45)', textTransform: 'uppercase', letterSpacing: '0.04em' }}>{sub}</span>
                        <span style={{ ...FD, fontSize: 14, color: GREEN, fontVariantNumeric: 'tabular-nums' }}>{total != null ? `$${total.toFixed(2)}` : '…'}</span>
                        <span style={{ ...FB, fontSize: 11, color: 'rgb(var(--ink-rgb) / 0.4)' }}>{openCat === key ? '−' : '+'}</span>
                      </span>
                    </button>
                    ...
                    {openCat === key && key === 'collected' && (
                      !fcRewards || fcRewards.posts.length === 0
                        ? <p style={{ ...FB, fontSize: 10.5, color: 'rgb(var(--ink-rgb) / 0.4)', textTransform: 'uppercase', padding: '12px 2px' }}>{fcRewards ? 'NO FIRST CUT REWARDS YET' : 'LOADING…'}</p>
                        : (
                          <>
                            {fcRewards.unpaidUsd > 0.005 && (
                              <p style={{ ...FB, fontSize: 9.5, color: 'rgb(var(--ink-rgb) / 0.45)', textTransform: 'uppercase', letterSpacing: '0.06em', padding: '10px 2px 0' }}>
                              ...
                              return (
                                <div key={p.postId} style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '9px 2px', borderBottom: `1px solid rgb(var(--ink-rgb) / 0.06)` }}>
                                  ...
                                  <span style={{ textAlign: 'right' }}>
                                    <span style={{ ...FD, fontSize: 12.5, color: p.accruedUsd - p.unpaidUsd > 0.005 ? GREEN : 'rgb(var(--ink-rgb) / 0.75)', fontVariantNumeric: 'tabular-nums', display: 'block' }}>${p.accruedUsd.toFixed(2)}</span>
                                    {p.unpaidUsd > 0.005 && <span style={{ ...FB, fontSize: 9, color: 'rgb(var(--ink-rgb) / 0.4)', textTransform: 'uppercase', fontVariantNumeric: 'tabular-nums' }}>· ${p.unpaidUsd.toFixed(2)} PENDING</span>}
                                  </span>
                                </div>
                              );
                            })}
            ...
            {tab === 'activity' && (
              <div style={{ padding: '4px 14px' }}>
                {activityFailed ? (
                  ...
                  <p style={{ ...FB, fontSize: 11, color: 'rgb(var(--ink-rgb) / 0.4)', textTransform: 'uppercase', padding: '18px 0' }}>NO ACTIVITY YET</p>
                ) : activity.map((row) => {
                  ...
                  return (
                    <div key={row.hash} style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '11px 0', borderBottom: `1px solid ${HAIR}` }}>
                      <span style={{ flex: 1, minWidth: 0 }}>
                        <span style={{ ...FD, fontSize: 11.5, color: 'var(--ink-100)', textTransform: 'uppercase', display: 'block' }}>{title}</span>
                        <span style={{ ...FB, fontSize: 9.5, color: 'rgb(var(--ink-rgb) / 0.45)', display: 'block', marginTop: 2 }}>{sub}</span>
                      </span>
                      <span style={{ ...FD, fontSize: 12.5, color: row.kind === 'receive' || row.kind === 'sell' ? GREEN : 'var(--ink-100)', fontVariantNumeric: 'tabular-nums' }}>{amount}</span>
                    </div>
                  );
                })}
              </div>
  ...
  return createPortal(
    <div data-swipe-exclude style={{ position: 'fixed', inset: 0, zIndex: 620 }}>
      ...
      <div style={{ position: 'absolute', left: '50%', top: '50%', transform: 'translate(-50%, -50%)', width: 480, background: '#080808', border: '1px solid rgb(var(--ink-rgb) / 0.14)', padding: 24 }}>
        ...
        <input value={to} onChange={(e) => setTo(e.target.value.trim())} placeholder="0x RECIPIENT ADDRESS" style={{ ...SKR, fontSize: 12, color: validAddr || !to ? 'var(--ink-100)' : RED, background: 'rgb(var(--ink-rgb) / 0.04)', border: `1px solid ${to && !validAddr ? 'rgb(var(--ink-rgb) / 0.5)' : 'rgb(var(--ink-rgb) / 0.12)'}`, outline: 'none', padding: '10px 12px', width: '100%', boxSizing: 'border-box', marginBottom: 10 }} />
        <input value={amount} onChange={(e) => setAmount(e.target.value.replace(/[^0-9.]/g, ''))} placeholder={token === 'ETH' ? 'AMOUNT (ETH)' : 'AMOUNT (USDC)'} style={{ ...SKR, fontSize: 12, color: 'var(--ink-100)', background: 'rgb(var(--ink-rgb) / 0.04)', border: '1px solid rgb(var(--ink-rgb) / 0.12)', outline: 'none', padding: '10px 12px', width: '100%', boxSizing: 'border-box' }} />
        {token === 'ETH' && rate != null && isFinite(amt) && amt > 0 && (
          <p style={{ ...SKR, fontSize: 9.5, color: 'rgb(var(--ink-rgb) / 0.45)', margin: '6px 0 0', fontVariantNumeric: 'tabular-nums' }}>≈ ${(amt * rate).toFixed(2)}</p>
        )}
