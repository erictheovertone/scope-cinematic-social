    ...
    <div data-swipe-exclude style={{ position: 'fixed', inset: 0, zIndex: 200 }}>
      ...
      <div style={{ position: 'absolute', top: 0, left: RAIL_W, right: 0, bottom: 0, overflowY: 'auto' }}>
        ...
        <div style={{ maxWidth: 1120, margin: '0 auto', padding: '72px 56px 96px', opacity: mounted ? 1 : 0, transform: mounted ? 'translateY(0)' : 'translateY(10px)', transition: reduced ? 'none' : 'opacity 320ms ease, transform 320ms cubic-bezier(0.16,0.84,0.3,1)' }}>
          ...
          <h1 style={{ ...SKB, fontSize: 54, lineHeight: 0.98, letterSpacing: '-0.02em', color: '#E5E1DB', textTransform: 'uppercase', margin: '0 0 22px' }}>Choose Your<br />Perspective</h1>
          <p style={{ ...SKR, fontSize: 15, color: 'rgba(229,225,219,0.6)', lineHeight: 1.7, margin: 0, maxWidth: 620 }}>
          ...
          <div style={{ margin: '48px 0 0' }}>
            <p style={{ ...SKB, fontSize: 11, color: 'rgba(229,225,219,0.5)', textTransform: 'uppercase', letterSpacing: '0.22em', margin: '0 0 20px' }}>MODES ENGINEERED FOR</p>
            <div style={{ display: 'flex', gap: 40, flexWrap: 'wrap' }}>
              {PILLARS.map((p) => (
                <div key={p.label} style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
                  <span style={{ display: 'flex', flexShrink: 0 }}>{p.glyph}</span>
                  <span style={{ ...SKB, fontSize: 12, color: 'rgba(229,225,219,0.85)', textTransform: 'uppercase', letterSpacing: '0.12em' }}>{p.label}</span>
                </div>
              ))}
            </div>
          </div>
          ...
          <p style={{ ...SKB, fontSize: 11, color: 'rgba(229,225,219,0.5)', textTransform: 'uppercase', letterSpacing: '0.22em', margin: '0 0 22px' }}>SELECT A FORMAT</p>
              ...
              return (
                ...
                >
                  {card.baked ? (
                    ...
                    <div style={{ position: 'absolute', inset: 0, display: 'flex', flexDirection: 'column', justifyContent: 'space-between', padding: '18px 20px', background: isSel ? 'linear-gradient(135deg, rgba(229,225,219,0.16), rgba(0,0,0,0))' : '#0a0a0a' }}>
                      <svg width="30" height="30" viewBox="0 0 24 24" fill="none"><rect x="4.5" y="4.5" width="15" height="15" rx="1" stroke={isSel ? RED : 'rgba(229,225,219,0.7)'} strokeWidth="1.3"/><path d="M9 9l6 6M15 9l-6 6" stroke={isSel ? RED : 'rgba(229,225,219,0.35)'} strokeWidth="1.1"/></svg>
                      <div>
                        <p style={{ ...SKB, fontSize: 16, color: '#E5E1DB', textTransform: 'uppercase', letterSpacing: '0.04em', margin: '0 0 3px' }}>Lightbox</p>
                        <p style={{ ...SKR, fontSize: 10.5, color: isSel ? RED : 'rgba(229,225,219,0.45)', textTransform: 'uppercase', letterSpacing: '0.12em', margin: 0 }}>{isSel ? 'SELECTED' : card.sub}</p>
                      </div>
                    </div>
                  )}
                  ...
                  {card.coming ? (
                    <span style={{ position: 'absolute', top: 10, right: 12, ...SKB, fontSize: 9, color: 'rgba(229,225,219,0.75)', textTransform: 'uppercase', letterSpacing: '0.14em', background: 'rgba(0,0,0,0.6)', padding: '3px 7px', borderRadius: 1 }}>COMING</span>
                  ) : card.baked && isSel && (
