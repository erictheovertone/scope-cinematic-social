...
const INPUT: React.CSSProperties = { ...SKB, fontSize: 13, color: '#E5E1DB', background: 'rgba(229,225,219,0.04)', border: `1px solid ${HAIR}`, outline: 'none', padding: '10px 12px', width: '100%', boxSizing: 'border-box', letterSpacing: '0.02em' };
  ...
  return (
    <div className="bg-black" style={{ position: 'fixed', inset: 0, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
      <div style={{ position: 'relative', width: 820, minHeight: 520, background: '#000', border: '1px solid #1a1a1a', boxSizing: 'border-box', padding: '54px 60px' }}>
          ...
          <div style={{ flexShrink: 0 }}>
            <button onClick={() => fileRef.current?.click()} style={{ width: 120, height: 120, borderRadius: '50%', overflow: 'hidden', background: '#141414', border: `1px solid ${HAIR}`, cursor: 'pointer', padding: 0, position: 'relative' }}>
              {pfp ? <img src={feedImage(pfp, 240)} alt="" style={{ width: '100%', height: '100%', objectFit: 'cover', display: 'block' }} /> : (
                <span style={{ position: 'absolute', inset: 0, display: 'flex', alignItems: 'center', justifyContent: 'center', ...SKB, fontSize: 10, color: 'rgba(229,225,219,0.4)', textTransform: 'uppercase', letterSpacing: '0.08em' }}>{uploading ? '…' : 'ADD PHOTO'}</span>
              )}
            </button>
        ...
        <button onClick={submit} disabled={saving} style={{ position: 'absolute', right: 60, bottom: 44, ...SKB, fontSize: 12, color: '#000', textTransform: 'uppercase', letterSpacing: '0.1em', background: '#E5E1DB', border: 'none', cursor: 'pointer', width: 160, height: 48, opacity: saving ? 0.6 : 1 }}>
