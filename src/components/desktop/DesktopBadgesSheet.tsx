      ...
      <div style={{ position: 'relative', width: 720, maxHeight: '82vh', background: 'var(--black)', border: '1px solid var(--surface-3)', boxSizing: 'border-box', display: 'flex', flexDirection: 'column' }}>
            ...
            <div style={{ padding: '4px 0 8px' }}>
              ...
              {d.sections.map((sec, i) => (
                <div key={i} style={{ marginBottom: 24 }}>
                  ...
                  <p style={{ ...SKR, fontSize: 13, color: 'rgb(var(--ink-rgb) / 0.75)', lineHeight: 1.5, margin: 0 }}>{sec.body}</p>
                </div>
              ))}
              ...
              {(detailKey === 'pro' || detailKey === 'free') && badgeState('pro', flags) !== 'held' && isOwn && (
                <button onClick={() => { onClose(); goPro(); }} style={{ ...SKB, width: '100%', fontSize: 12, color: 'var(--on-ink)', textTransform: 'uppercase', letterSpacing: '0.08em', background: 'var(--ink-100)', border: 'none', cursor: 'pointer', padding: '13px 0', marginTop: 8 }}>
                  GET PRO →
                </button>
              )}
            </div>
        ...
        {membership && (
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 16, padding: '14px 0 18px', borderBottom: `1px solid ${HAIR}`, marginBottom: 8 }}>
            ...
            <button onClick={() => { onClose(); membership.isPaid ? router.push('/profile/preferences?section=membership') : goPro(); }} style={{ ...SKB, fontSize: 11, color: membership.isPaid ? 'rgb(var(--ink-rgb) / 0.5)' : 'var(--ink-100)', textTransform: 'uppercase', letterSpacing: '0.1em', background: 'transparent', border: 'none', cursor: 'pointer', padding: 0, flexShrink: 0 }}>
            ...
            <div key={k} style={{ display: 'flex', alignItems: 'flex-start', gap: 16, padding: '16px 0', borderBottom: `1px solid ${HAIR}`, opacity: state === 'held' ? 1 : 0.72, animation: reduced ? 'none' : `badgeRippleIn 300ms ease-out ${ri * 45}ms forwards` }}>
              ...
              <div style={{ flex: 1, minWidth: 0 }}>
                <div style={{ display: 'flex', alignItems: 'baseline', gap: 10 }}>
                  <span style={{ ...SKB, fontSize: 13, color: 'var(--ink-100)', textTransform: 'uppercase', letterSpacing: '0.06em' }}>{k === 'top1k' ? 'COLLECTOR' : b.title}</span>
                  <span style={{ ...SKB, fontSize: 9, color: chip.c, textTransform: 'uppercase', letterSpacing: '0.1em' }}>{chip.t}</span>
                </div>
                {/* descriptor — ALWAYS readable */}
                <p style={{ ...SKR, fontSize: 12, color: 'rgb(var(--ink-rgb) / 0.55)', lineHeight: 1.5, margin: '6px 0 0' }}>{BADGE_BLURBS[k]}</p>
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: 18, marginTop: 10 }}>
                  ...
                  {k === 'composer' && state === 'held' && composerHandle ? (
                    <button onClick={() => { onClose(); router.push(`/composer/${composerHandle}`); }} style={{ ...SKB, fontSize: 11, color: 'rgb(var(--ink-rgb) / 0.75)', textTransform: 'uppercase', letterSpacing: '0.1em', background: 'transparent', border: 'none', cursor: 'pointer', padding: 0 }}>
                      DISCOGRAPHY →
                    </button>
                  ) : (
                    <button onClick={() => setDetailKey(k)} style={{ ...SKB, fontSize: 11, color: 'rgb(var(--ink-rgb) / 0.75)', textTransform: 'uppercase', letterSpacing: '0.1em', background: 'transparent', border: 'none', cursor: 'pointer', padding: 0 }}>
                      LEARN MORE →
                    </button>
                  )}
                  {state === 'buyable' && isOwn && (
                    <button onClick={(e) => { e.stopPropagation(); onClose(); goPro(); }} style={{ ...SKB, fontSize: 11, color: 'var(--ink-100)', textTransform: 'uppercase', letterSpacing: '0.1em', background: 'transparent', border: 'none', cursor: 'pointer', padding: 0 }}>
                      GET PRO →
                    </button>
                  )}
                </div>
