...
const ICON = (d: React.ReactNode, size = 16, color = 'rgb(var(--ink-rgb) / 0.8)'): React.ReactNode => (
...
const Chevron = ICON(<path d="M9 6l6 6l-6 6" />, 16, 'rgb(var(--ink-rgb) / 0.4)');
...
export default function WhileYouWereAwaySheet({ visible, recap, username, onClose }: Props) {
  ...
  const stat = (icon: React.ReactNode, n: number, label: string) => (
    ...
    <div style={{ flex: 1, position: 'relative', padding: '14px 8px 12px', minWidth: 0, textAlign: 'center' }}>
      ...
      <p style={{ ...SKB, fontSize: 28, color: 'var(--ink-100)', margin: '14px 0 0', lineHeight: 1, fontVariantNumeric: 'tabular-nums' }}>+{Math.round(n * e)}</p>
    ...
    >
      ...
      >
        ...
        <div style={{ position: 'relative', overflow: 'hidden', border: '0.5px solid #1f1f1f', borderRadius: 2, height: 127, boxSizing: 'border-box', marginBottom: 28, flexShrink: 0 }}>
          ...
          <p style={{ position: 'absolute', top: 39, left: 16, zIndex: 1, ...SKB, fontSize: 48, color: 'var(--ink-100)', margin: 0, lineHeight: 1, fontVariantNumeric: 'tabular-nums' }}>{fmtMoney(hero.earned * e)}</p>
        ...
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 10, flexShrink: 0 }}>
          <span style={{ ...SKB, fontSize: 'var(--fs-10)', color: 'var(--ink-100)', textTransform: 'uppercase', letterSpacing: '0.1em', display: 'flex', alignItems: 'center', gap: 7 }}>
        ...
        >
          <span style={{ ...SKB, fontSize: 'var(--fs-12)', color: 'var(--ink-100)', textTransform: 'uppercase', letterSpacing: '0.14em' }}>ENTER</span>
        </button>

        {/* FOOTER */}
        <p style={{ ...SKR, fontSize: 'var(--fs-8)', color: 'rgb(var(--ink-rgb) / 0.35)', textAlign: 'center', margin: '12px 0 0', flexShrink: 0, textTransform: 'uppercase', letterSpacing: '0.08em' }}>
