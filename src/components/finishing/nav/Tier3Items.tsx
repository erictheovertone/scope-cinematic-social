            ...
            return (
              <button
                ...
                style={{
                  ...
                  minWidth: 54, color: touched ? RED : 'var(--ink-100)',
                }}
              >
                ...
                <span style={{ ...SKB, fontSize: 'var(--fs-8)', color: touched ? RED : 'var(--ink-100)', textTransform: 'uppercase', letterSpacing: '0.06em', whiteSpace: 'nowrap' }}>{t.label}</span>
              </button>
            );
          }
          ...
          return (
            <button
              ...
              style={{
                ...
                border: `1px solid ${touched ? RED : 'rgb(var(--ink-rgb) / 0.18)'}`,
                cursor: enabled ? 'pointer' : 'default', padding: '10px 12px 9px',
                opacity: enabled ? 1 : 0.4,
                display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 6,
                minWidth: 72, color: touched ? RED : 'var(--ink-100)',
              }}
            >
              ...
              <span style={{ ...SKB, fontSize: 'var(--fs-9)', color: touched ? RED : 'var(--ink-100)', textTransform: 'uppercase', letterSpacing: '0.06em', whiteSpace: 'nowrap' }}>{t.label}</span>
              <span style={{
                ...SKB, fontSize: 'var(--fs-7)', textTransform: 'uppercase', letterSpacing: '0.1em', whiteSpace: 'nowrap',
                color: !enabled ? RED : 'rgb(var(--ink-rgb) / 0.3)',
              }}>{sub}</span>
            </button>
          );
        })}
    ...
    <div style={ROW}>
      {Array.from({ length: 4 }).map((_, i) => (
        <div key={i} style={{ flexShrink: 0, display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 6, opacity: 0.4 }}>
          <div style={{ width: 64, height: 64, border: '1px solid rgb(var(--ink-rgb) / 0.18)', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
            <span style={{ ...SKB, fontSize: 'var(--fs-7)', color: RED, textTransform: 'uppercase', letterSpacing: '0.1em' }}>SOON</span>
          </div>
          <span style={{ ...SKB, fontSize: 'var(--fs-8)', color: 'rgb(var(--ink-rgb) / 0.4)', textTransform: 'uppercase', letterSpacing: '0.06em' }}>—</span>
        </div>
      ))}
    </div>
  );
...
function EmptyNote() {
  return (
    <span style={{ ...SKB, fontSize: 'var(--fs-9)', color: 'rgb(var(--ink-rgb) / 0.3)', textTransform: 'uppercase', letterSpacing: '0.1em', padding: '18px 0' }}>
      NOTHING HERE YET
    </span>
  );
}
