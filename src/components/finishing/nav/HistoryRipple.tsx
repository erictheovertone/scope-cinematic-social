  ...
  return (
    <div style={{ maxHeight: 200, overflowY: 'auto', padding: '8px 18px 12px' }}>
      {rows.map((row, i) => {
        ...
        const nodeColor = current ? RED : 'rgb(var(--ink-rgb) / 0.5)';
        return (
          <div
          ...
          >
            ...
            {row.kind === 'origin' ? (
              <div style={{ display: 'flex', alignItems: 'center' }}>
                <span style={{ ...SKB, fontSize: 'var(--fs-9)', color: 'rgb(var(--ink-rgb) / 0.45)', textTransform: 'uppercase', letterSpacing: '0.1em' }}>ORIGINAL</span>
              </div>
            ) : (
              <div style={{ display: 'flex', alignItems: 'center', gap: 8, minWidth: 0 }}>
                <span style={{ color: current ? RED : 'var(--ink-100)', lineHeight: 0, flexShrink: 0 }}>
                  <ToolIcon toolKey={row.e.toolKey as IconKey} size={16.5} />
                </span>
                <span style={{ ...SKB, fontSize: 'var(--fs-9)', color: current ? RED : 'var(--ink-100)', textTransform: 'uppercase', letterSpacing: '0.06em', whiteSpace: 'nowrap' }}>{row.e.label}</span>
                <span style={{ ...SKB, fontSize: 'var(--fs-9)', color: 'rgb(var(--ink-rgb) / 0.4)', letterSpacing: '0.04em', whiteSpace: 'nowrap', fontVariantNumeric: 'tabular-nums' }}>{row.e.value}</span>
              </div>
            )}
          </div>
        );
