...
export default function CurvesPanel({ curves, onChange, isPro, onUpsell }: CurvesPanelProps) {
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
        ...
        {CHANNELS.map((c) => {
          ...
          return (
            <button key={c.key} onClick={() => tapTab(c.key)} style={{ position: 'relative', background: 'transparent', border: 'none', cursor: 'pointer', padding: '2px 6px', display: 'inline-flex', alignItems: 'center', gap: 3 }}>
              <span style={{ ...SKB, fontSize: 10, color: on ? 'white' : 'rgba(255,255,255,0.4)', textTransform: 'uppercase', letterSpacing: '0.06em' }}>{c.label}</span>
              {locked && (
      ...
      >
        <span style={{ ...SKB, fontSize: 8, color: 'rgba(255,255,255,0.6)', textTransform: 'uppercase', letterSpacing: '0.1em' }}>RESET {cfg.label}</span>
      </button>
    </div>
  );
}
