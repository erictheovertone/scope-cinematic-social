  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
        ...
        {CHANNELS.map((c) => {
          ...
          return (
            <button key={c.key} onClick={() => tapTab(c.key)} style={{ position: 'relative', background: 'transparent', border: 'none', cursor: 'pointer', padding: '2px 6px', display: 'inline-flex', alignItems: 'center', gap: 3 }}>
              <span style={{ ...SKB, fontSize: 'var(--fs-10)', color: on ? 'var(--ink-100)' : 'rgb(var(--ink-rgb) / 0.4)', textTransform: 'uppercase', letterSpacing: '0.06em' }}>{c.label}</span>
              {locked && (
      ...
      <button
        ...
        style={{ alignSelf: 'flex-end', background: 'transparent', border: '1px solid rgb(var(--ink-rgb) / 0.2)', cursor: 'pointer', padding: '6px 12px' }}
      >
        <span style={{ ...SKB, fontSize: 'var(--fs-8)', color: 'rgb(var(--ink-rgb) / 0.6)', textTransform: 'uppercase', letterSpacing: '0.1em' }}>RESET {cfg.label}</span>
      </button>
    </div>
  );
}
