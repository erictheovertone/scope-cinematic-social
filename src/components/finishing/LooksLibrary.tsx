  ...
  return (
    <div style={{ maxHeight: 230, overflowY: 'auto', padding: '10px 14px 14px', display: 'flex', flexDirection: 'column', gap: 14 }}>
      ...
      {LOOK_BUCKETS.map((bucket, bi) => {
        ...
        return (
          <div key={bucket}>
            <span style={{ ...SKB, fontSize: 'var(--fs-8)', color: 'rgb(var(--ink-rgb) / 0.45)', textTransform: 'uppercase', letterSpacing: '0.12em' }}>{bucket}</span>
...
function LookTile({ name, url, selected, locked, onTap, dust = null, dustDelay = 0 }: { name: string; url: string | null; selected: boolean; locked: boolean; onTap: () => void; dust?: 'hold' | 'play' | null; dustDelay?: number }) {
  return (
    <button onClick={onTap} style={{ flexShrink: 0, display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 5, background: 'transparent', border: 'none', cursor: 'pointer', padding: 0 }}>
      <div style={{ position: 'relative', width: 60, height: 60, overflow: 'hidden', border: `1px solid ${selected ? RED : 'rgb(var(--ink-rgb) / 0.18)'}`, background: '#111' }}>
      ...
      <span style={{ ...SKB, fontSize: 'var(--fs-7)', color: selected ? RED : 'rgb(var(--ink-rgb) / 0.6)', textTransform: 'uppercase', letterSpacing: '0.06em', maxWidth: 60, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{name}</span>
    </button>
  );
}
