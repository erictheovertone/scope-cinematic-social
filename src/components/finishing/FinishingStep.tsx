...
export default function FinishingStep({
  ...
  if (loadError) {
    return (
      <div style={{ position: 'fixed', inset: 0, background: 'var(--black)', zIndex: 2, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: 18, padding: '0 40px' }}>
        <p style={{ fontFamily: "'SK-Modernist', sans-serif", fontWeight: 400, fontSize: 14, color: 'rgb(var(--ink-rgb) / 0.7)', lineHeight: 1.5, textAlign: 'center', margin: 0 }}>Something went wrong loading your photo.</p>
        <button onClick={() => { setSource(null); setLoadError(false); retriedRef.current = false; setRetryKey((k) => k + 1); }} style={{ fontFamily: "'SK-Modernist', sans-serif", fontWeight: 700, fontSize: 12, color: 'var(--ink-100)', textTransform: 'uppercase', letterSpacing: '0.1em', background: 'transparent', border: '1px solid rgb(var(--ink-rgb) / 0.5)', cursor: 'pointer', padding: '13px 26px' }}>RETAP TO CONTINUE</button>
      </div>
    );
  }
