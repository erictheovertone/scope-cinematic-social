...
export default function CurveGraph({ points, onChange, config }: CurveGraphProps) {
  ...
  return (
    ...
    >
      <rect x={P} y={P} width={plotW} height={plotH} fill="none" stroke="rgb(var(--ink-rgb) / 0.18)" strokeWidth={1} />
      {[1 / 3, 2 / 3].map((f) => (
        <g key={f}>
          <line x1={toPx(f)} y1={P} x2={toPx(f)} y2={GH - P} stroke="rgb(var(--ink-rgb) / 0.10)" strokeWidth={1} />
          <line x1={P} y1={toPy(f)} x2={GW - P} y2={toPy(f)} stroke="rgb(var(--ink-rgb) / 0.10)" strokeWidth={1} />
        </g>
      ))}
