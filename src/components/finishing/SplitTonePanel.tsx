        ...
        {(['shadows', 'highlights'] as const).map((r) => {
          ...
          return (
            <button key={r} onClick={() => setRegion(r)} style={{ background: 'transparent', border: 'none', cursor: 'pointer', padding: '2px 0', position: 'relative' }}>
              <span style={{ ...SKB, fontSize: 'var(--fs-10)', color: on ? 'var(--ink-100)' : 'rgb(var(--ink-rgb) / 0.4)', textTransform: 'uppercase', letterSpacing: '0.08em' }}>{r === 'shadows' ? 'SHADOWS TINT' : 'HIGHLIGHTS TINT'}</span>
          ...
          return (
            <button
              ...
              style={{
                ...
                border: selected ? `2px solid ${RED}` : '1px solid rgb(var(--ink-rgb) / 0.25)',
