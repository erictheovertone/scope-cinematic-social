 ...
 * caller drives colour (white at rest, var(--ink-100) when the tool is active/touched)
...
const PATHS: Partial<Record<IconKey, React.ReactNode>> = {
  ...
  curve: ( // red S-curve inside a faint graph frame (resembles the curve tool)
    <>
      <rect x="3" y="3" width="18" height="18" stroke="rgb(var(--ink-rgb) / 0.5)" />
      <path d="M3 21 21 3" stroke="rgb(var(--ink-rgb) / 0.22)" />
      <path d="M4 20C9 20 8 13 12 12s3-8 8-8" stroke="var(--ink-100)" />
    </>
  ),
  exposure: (
    <>
    ...
    <>
      <defs>
        <radialGradient id="iconBlurGlow" cx="50%" cy="50%" r="66%">
          ...
          <stop offset="100%" stopColor="var(--ink-100)" stopOpacity="0" />
        </radialGradient>
      </defs>
    ...
    <>
      <defs>
        <radialGradient id="iconBloomGlow" cx="38%" cy="38%" r="72%">
          <stop offset="0%" stopColor="var(--ink-100)" stopOpacity="0.95" />
          <stop offset="55%" stopColor="var(--ink-100)" stopOpacity="0.32" />
          <stop offset="100%" stopColor="var(--ink-100)" stopOpacity="0" />
        </radialGradient>
      </defs>
