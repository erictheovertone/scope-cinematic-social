  ...
  return (
    <div style={{ display: "flex", alignItems: "center", gap: 10, padding: compact ? "6px 14px" : "8px 18px", background: "#000" }}>
      ...
      <button onClick={togglePlay} aria-label={playing ? "Pause" : "Play"} style={{ background: "transparent", border: "none", cursor: "pointer", padding: 0, lineHeight: 0, flexShrink: 0 }}>
        {playing ? (
          <svg width="15.5" height="15.5" viewBox="0 0 12 12" fill="none"><rect x="2" y="1.5" width="2.6" height="9" fill="var(--ink-100)" /><rect x="7.4" y="1.5" width="2.6" height="9" fill="var(--ink-100)" /></svg>
        ) : (
          <svg width="15.5" height="15.5" viewBox="0 0 12 12" fill="none"><path d="M2.5 1.5l8 4.5-8 4.5z" fill="var(--ink-100)" /></svg>
        )}
      </button>
      ...
      >
        <div style={{ position: "absolute", left: 0, right: 0, height: 1.5, background: "rgb(var(--ink-rgb) / 0.2)" }} />
      ...
      {/* Timecode */}
      <span style={{ ...REG, fontSize: 'var(--fs-8)', color: "rgb(var(--ink-rgb) / 0.55)", letterSpacing: "0.04em", flexShrink: 0, minWidth: 56, textAlign: "right" }}>
      ...
      <button onClick={toggleMute} aria-label={muted ? "Unmute" : "Mute"} style={{ background: "transparent", border: "none", cursor: "pointer", padding: 0, lineHeight: 0, flexShrink: 0 }}>
        {muted ? (
          <svg width="16.5" height="16.5" viewBox="0 0 24 24" fill="rgb(var(--ink-rgb) / 0.7)">
            <path d="M3.5 9v6h4l5 5V4l-5 5h-4z" />
            <path d="M15.5 8.5l5 7M20.5 8.5l-5 7" stroke="rgb(var(--ink-rgb) / 0.7)" strokeWidth="1.6" strokeLinecap="round" />
          </svg>
        ) : (
