 ...
 *
 * Design system: pure black, var(--ink-100) for active fill / active thumb / centre
...
export default function ToolSlider({ type, value, onChange, label, trackGradient, inline = false }: ToolSliderProps) {
  ...
  const track = (
    ...
    >
      ...
      <div style={{ position: 'absolute', left: 0, right: 0, height: grad ? 1 : 2, background: trackGradient ?? 'rgb(var(--ink-rgb) / 0.18)' }} />
      ...
      {/* centre tick for bi */}
      {type === 'bi' && (
        <div style={{ position: 'absolute', left: '50%', transform: 'translateX(-50%)', width: 1.5, height: 10, background: !grad && value === 0 ? RED : 'rgb(var(--ink-rgb) / 0.45)' }} />
      )}
      ...
      <div style={{
        ...
        background: grad ? 'var(--ink-100)' : (value !== 0 || dragging ? RED : 'var(--ink-100)'),
      }} />
    </div>
  );
  ...
  if (inline) {
    return (
      <div style={{ width: '100%', userSelect: 'none', display: 'flex', alignItems: 'center', gap: 12 }}>
        <span style={{ ...SKB, fontSize: 'var(--fs-9)', color: 'var(--ink-100)', textTransform: 'uppercase', letterSpacing: '0.1em', whiteSpace: 'nowrap' }}>{label}</span>
        {track}
        <span style={{
          ...SKB, fontSize: 'var(--fs-11)', color: value !== 0 ? RED : 'rgb(var(--ink-rgb) / 0.4)',
  ...
  return (
    <div style={{ width: '100%', userSelect: 'none' }}>
      ...
      <div style={{ display: 'flex', alignItems: 'baseline', justifyContent: 'space-between', marginBottom: 14 }}>
        <span style={{ ...SKB, fontSize: 'var(--fs-10)', color: 'var(--ink-100)', textTransform: 'uppercase', letterSpacing: '0.1em' }}>{label}</span>
        <span style={{
          ...SKB, fontSize: 'var(--fs-11)', color: value !== 0 ? RED : 'rgb(var(--ink-rgb) / 0.4)',
