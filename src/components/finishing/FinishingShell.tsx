...
export default function FinishingShell({
  ...
  const menuItemsContent = (
    <>
      <button onClick={toggleZoom} style={{ ...menuItem, color: zoom.active ? RED : 'var(--ink-100)' }}>
        <span style={menuLabel}>ZOOM</span>
        {zoom.active && <span style={menuTag}>ON</span>}
      </button>
      <button onClick={() => { setTheatreToggle((o) => !o); setMenuOpen(false); }} style={{ ...menuItem, color: theatre ? RED : 'var(--ink-100)', borderTop: '1px solid rgb(var(--ink-rgb) / 0.1)' }}>
  ...
  const stageInner = (
    <>
      {source && geomPreview ? (
        ...
        <span style={{ ...SKB, fontSize: 'var(--fs-10)', color: 'rgb(var(--ink-rgb) / 0.3)', textTransform: 'uppercase', letterSpacing: '0.1em' }}>LOADING…</span>
      )}
      {showingOriginal && (
        <div style={{ position: 'absolute', top: 10, left: 10, background: RED, padding: '3px 7px' }}>
          <span style={{ ...SKB, fontSize: 'var(--fs-9)', color: 'var(--black)', textTransform: 'uppercase', letterSpacing: '0.1em' }}>BEFORE</span>
        </div>
      )}
    </>
  );
  ...
  const renderBandBody = (lean: boolean) => (
    activeMode === 'history' ? (
      ...
      <div style={{ padding: '12px 16px 14px', display: 'flex', flexDirection: 'column', gap: 12 }}>
        <AddToPalette isPro={isPro} onUpsell={() => showUpsell('edit')} onSave={saveLook} />
        {savedLooks.length === 0 ? (
          <span style={{ ...SKB, fontSize: 'var(--fs-9)', color: 'rgb(var(--ink-rgb) / 0.3)', textTransform: 'uppercase', letterSpacing: '0.08em', lineHeight: 1.6 }}>NO SAVED LOOKS YET · ADD TO PALETTE SAVES YOUR CURRENT EDIT</span>
        ) : (
  ...
  const adjustingBar = activeTool && (
    <div style={{ borderTop: '1px solid rgb(var(--ink-rgb) / 0.08)', background: 'var(--black)', padding: activeTool.kind === 'slider' ? '7px 14px 9px' : '9px 14px 14px' }}>
      {activeTool.kind === 'slider' ? (
        ...
        <>
          <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 10 }}>
            {backToBrowsing}
            <span style={{ ...SKB, fontSize: 'var(--fs-10)', color: 'rgb(var(--ink-rgb) / 0.6)', textTransform: 'uppercase', letterSpacing: '0.1em' }}>{activeTool.label}</span>
  ...
  return (
    <div style={{ position: 'fixed', inset: 0, background: 'var(--black)', display: 'flex', flexDirection: 'column',
      ...
      {theatre ? (
        <div key="topbar" style={{ flexShrink: 0, height: `calc(${compactRail ? 30 : 44}px + env(safe-area-inset-top, 0px))`, paddingTop: 'env(safe-area-inset-top, 0px)', display: 'flex', alignItems: 'center', justifyContent: 'space-between', paddingLeft: 14, paddingRight: 14, borderBottom: '1px solid rgb(var(--ink-rgb) / 0.08)' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
            {onBack && (
              <button onClick={onBack} aria-label="Back" style={{ background: 'transparent', border: 'none', cursor: 'pointer', padding: 4, lineHeight: 0 }}>
                <svg width="17.5" height="17.5" viewBox="0 0 16 16" fill="none"><path d="M10 3L5 8l5 5" stroke="var(--ink-100)" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round" /></svg>
              </button>
            )}
            <span style={{ ...SKB, fontSize: compactRail ? 10 : 12, color: 'var(--ink-100)', textTransform: 'uppercase', letterSpacing: '0.06em' }}>FINISHING · THEATRE</span>
          </div>
          <div style={{ display: 'flex', alignItems: 'center', gap: 16 }}>
            {/* "+" viewing menu (inline in the bar) */}
            <div style={{ position: 'relative' }}>
              <button onClick={() => setMenuOpen((o) => !o)} aria-label="Viewing options" style={{ width: 24, height: 24, background: 'transparent', cursor: 'pointer', lineHeight: 0, display: 'flex', alignItems: 'center', justifyContent: 'center', border: `1px solid ${menuOpen ? RED : 'rgb(var(--ink-rgb) / 0.25)'}`, color: menuOpen || zoom.active ? RED : 'var(--ink-100)', transition: `border-color 0.3s ${SNAP}, color 0.3s ${SNAP}` }}>
              ...
              <div style={{ position: 'absolute', top: 'calc(100% + 6px)', right: 0, zIndex: 30, minWidth: 150, background: 'var(--black)', border: '1px solid rgb(var(--ink-rgb) / 0.18)', transformOrigin: 'top right', transform: menuOpen ? 'scale(1)' : 'scale(0.7)', opacity: menuOpen ? 1 : 0, pointerEvents: menuOpen ? 'auto' : 'none', transition: `transform ${menuOpen ? 0.3 : 0.2}s ${SNAP}, opacity ${menuOpen ? 0.3 : 0.2}s ${SNAP}` }}>
                {menuItemsContent}
              </div>
            </div>
        ...
        <div key="topbar" style={{ flexShrink: 0, height: 'calc(50px + env(safe-area-inset-top, 0px))', paddingTop: 'env(safe-area-inset-top, 0px)', display: 'flex', alignItems: 'center', justifyContent: 'space-between', paddingLeft: 18, paddingRight: 18, borderBottom: '1px solid rgb(var(--ink-rgb) / 0.08)' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
            {onBack && (
              <button onClick={onBack} aria-label="Back" style={{ background: 'transparent', border: 'none', cursor: 'pointer', padding: 4, lineHeight: 0 }}>
                <svg width="19.5" height="19.5" viewBox="0 0 16 16" fill="none"><path d="M10 3L5 8l5 5" stroke="var(--ink-100)" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round" /></svg>
              </button>
            )}
            <span style={{ ...SKB, fontSize: 'var(--fs-12)', color: 'var(--ink-100)', textTransform: 'uppercase', letterSpacing: '0.06em' }}>FINISHING</span>
          </div>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
            {!theatre && (
              <div style={{ position: 'relative' }}>
                <button onClick={() => setMenuOpen((o) => !o)} aria-label="Viewing options" className="tappable" style={{ width: 44, height: 44, background: 'transparent', border: 'none', cursor: 'pointer', lineHeight: 0, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 0, color: menuOpen || zoom.active ? RED : 'var(--ink-100)', transition: `color 0.3s ${SNAP}` }}>
                ...
                <div style={{ position: 'absolute', top: 'calc(100% + 2px)', right: 0, zIndex: 30, minWidth: 150, background: 'var(--black)', border: '1px solid rgb(var(--ink-rgb) / 0.18)', transformOrigin: 'top right', transform: menuOpen ? 'scale(1)' : 'scale(0.7)', opacity: menuOpen ? 1 : 0, pointerEvents: menuOpen ? 'auto' : 'none', transition: `transform ${menuOpen ? 0.3 : 0.2}s ${SNAP}, opacity ${menuOpen ? 0.3 : 0.2}s ${SNAP}` }}>
                  {menuItemsContent}
                </div>
              </div>
            )}
      ...
      <div key="main" style={{ flex: 1, display: 'flex', flexDirection: 'row', minHeight: 0 }}>
        <div
          ...
          style={{ flex: 1, position: 'relative', overflow: 'hidden', display: 'flex', alignItems: 'center', justifyContent: 'center', background: 'var(--black)', cursor: zoom.active ? 'grab' : 'default', touchAction: zoom.active ? 'none' : 'auto' }}
        >
          {stageInner}
          ...
          {theatre && (
            <div style={{ position: 'absolute', bottom: 8, left: 0, right: 0, display: 'flex', justifyContent: 'center', pointerEvents: 'none' }}>
              <span style={{ ...SKB, fontSize: 'var(--fs-8)', color: 'rgb(var(--ink-rgb) / 0.4)', textTransform: 'uppercase', letterSpacing: '0.12em' }}>{arChip.ratioLabel}{showingOriginal ? ' · BEFORE' : ''}</span>
            </div>
          )}
        </div>
        {theatre && (
      ...
      {zoom.active && (
        <div key="zoom" style={{ flexShrink: 0, padding: theatre ? '8px 18px' : '10px 18px', borderTop: '1px solid rgb(var(--ink-rgb) / 0.08)', background: 'var(--black)', display: 'flex', alignItems: 'center', gap: 12 }}>
          ...
          />
          <button onClick={() => setZoom({ active: false, level: 2, panX: 0.5, panY: 0.5 })} style={{ background: 'transparent', border: '1px solid rgb(var(--ink-rgb) / 0.2)', cursor: 'pointer', padding: '4px 8px' }}>
            <span style={{ ...SKB, fontSize: 'var(--fs-8)', color: 'rgb(var(--ink-rgb) / 0.6)', textTransform: 'uppercase', letterSpacing: '0.1em' }}>EXIT</span>
          </button>
        </div>
      )}
      ...
      {theatre ? (
        <div key="bottom" style={{ flexShrink: 0, position: 'relative', borderTop: '1px solid rgb(var(--ink-rgb) / 0.08)', background: 'var(--black)', maxHeight: '46vh', overflowY: 'auto' }}>
        ...
        <div key="bottom" style={{ flexShrink: 0, borderTop: '1px solid rgb(var(--ink-rgb) / 0.08)', background: 'var(--black)' }}>
      ...
      {!theatre && (
        <div
          ...
          style={{
            ...
            background: 'var(--surface-1)', borderTop: `1px solid rgb(var(--ink-rgb) / 0.1)`,
        ...
        >
          ...
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 22 }}>
            <button onClick={cancelTool} style={{ background: 'transparent', border: 'none', cursor: 'pointer', padding: 4, lineHeight: 0 }}>
              <svg width="17.5" height="17.5" viewBox="0 0 14 14" fill="none"><path d="M3 3l8 8M11 3l-8 8" stroke="var(--ink-100)" strokeWidth="1.4" strokeLinecap="round" /></svg>
            </button>
      ...
      {theatre && compactRail && !adjusting && (activeMode === 'edit' || activeMode === 'fx') && (
        <div key="ml-subcat" style={{ position: 'fixed', left: 10, bottom: 10, zIndex: 31, display: 'flex', flexDirection: 'column-reverse', alignItems: 'flex-start', gap: 6, pointerEvents: 'none' }}>
          <button onClick={() => setSubcatMenuOpen((o) => !o)} aria-label="Filter" style={{ width: 26, height: 26, background: 'var(--black)', cursor: 'pointer', lineHeight: 0, display: 'flex', alignItems: 'center', justifyContent: 'center', pointerEvents: 'auto', border: `1px solid ${subcatMenuOpen ? RED : 'rgb(var(--ink-rgb) / 0.25)'}`, color: subcatMenuOpen ? RED : 'var(--ink-100)', transition: `border-color 0.3s ${SNAP}, color 0.3s ${SNAP}` }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 2, background: 'var(--black)', border: subcatMenuOpen ? '1px solid rgb(var(--ink-rgb) / 0.18)' : '1px solid transparent', transformOrigin: 'bottom left', transform: subcatMenuOpen ? 'scale(1)' : 'scale(0.7)', opacity: subcatMenuOpen ? 1 : 0, pointerEvents: subcatMenuOpen ? 'auto' : 'none', transition: `transform ${subcatMenuOpen ? 0.3 : 0.2}s ${SNAP}, opacity ${subcatMenuOpen ? 0.3 : 0.2}s ${SNAP}` }}>
            {modeDef(activeMode).subcats.map((s) => (
              <button key={s.key} onClick={() => { setActiveSubcat(s.key); setSubcatMenuOpen(false); }} style={{ background: 'transparent', border: 'none', cursor: 'pointer', padding: '8px 14px', textAlign: 'left' }}>
                <span style={{ ...SKB, fontSize: 'var(--fs-9)', color: s.key === activeSubcat ? RED : 'var(--ink-100)', textTransform: 'uppercase', letterSpacing: '0.08em' }}>{s.label}</span>
              </button>
            ))}
          </div>
        </div>
