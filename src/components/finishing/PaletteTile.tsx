...
export default function PaletteTile({ look, selected = false, onTap }: { look: SavedLook; selected?: boolean; onTap: () => void }) {
  ...
  return (
    ...
    >
      ...
      <span style={{ ...SKB, fontSize: 'var(--fs-7)', color: selected ? 'var(--ink-100)' : 'rgb(var(--ink-rgb) / 0.6)', textTransform: 'uppercase', letterSpacing: '0.06em', maxWidth: 60, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{look.name}</span>
    </button>
  );
}
