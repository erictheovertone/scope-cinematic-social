...
export default function CropTool({
  ...
  return (
    <div style={{ position: "fixed", inset: 0, zIndex: 200, background: "#000", display: "flex", flexDirection: "column" }}>
      ...
      {/* ── Sub-tabs ── */}
      <div style={{ flexShrink: 0, display: "flex", borderTop: "1px solid rgba(255,255,255,0.06)" }}>
        {([
      ...
      {/* ── Tab content ── */}
      <div style={{ flexShrink: 0, minHeight: 84, padding: "14px 16px 22px", borderTop: "1px solid rgba(255,255,255,0.06)" }}>
        {tab === "crop" && (
          <div>
                ...
                {Array.from({ length: 19 }).map((_, i) => {
                  ...
                  return (
                    <div key={i} style={{ position: "absolute", left: `calc(50% + ${offset}px)`, top: "50%", transform: "translate(-50%,-50%)", display: "flex", flexDirection: "column", alignItems: "center" }}>
                      <div style={{ width: 1, height: deg % 15 === 0 ? 14 : 8, background: deg === 0 ? RED : "rgba(255,255,255,0.3)" }} />
                    </div>
                  );
                })}
              </div>
