  ...
  return (
    <div style={{ position: "fixed", inset: 0, zIndex: 200, background: "var(--black)", display: "flex", flexDirection: "column" }}>
      ...
      <div style={{ flexShrink: 0, height: "calc(48px + env(safe-area-inset-top, 0px))", paddingTop: "env(safe-area-inset-top, 0px)", display: "flex", alignItems: "center", justifyContent: "space-between", paddingLeft: 16, paddingRight: 16, borderBottom: "1px solid rgb(var(--ink-rgb) / 0.08)" }}>
        ...
        <span style={{ ...SKB, fontSize: 'var(--fs-11)', color: "var(--ink-100)", textTransform: "uppercase", letterSpacing: "0.1em" }}>CROP</span>
          ...
          {orientedAr > 0 && (
            ...
            >
              ...
              <div style={{ position: "absolute", inset: 0, pointerEvents: "none" }}>
                <div style={{ position: "absolute", left: "33.33%", top: 0, bottom: 0, width: 1, background: "rgb(var(--ink-rgb) / 0.25)" }} />
                <div style={{ position: "absolute", left: "66.66%", top: 0, bottom: 0, width: 1, background: "rgb(var(--ink-rgb) / 0.25)" }} />
                <div style={{ position: "absolute", top: "33.33%", left: 0, right: 0, height: 1, background: "rgb(var(--ink-rgb) / 0.25)" }} />
                <div style={{ position: "absolute", top: "66.66%", left: 0, right: 0, height: 1, background: "rgb(var(--ink-rgb) / 0.25)" }} />
              </div>
      ...
      <div style={{ flexShrink: 0, padding: "10px 12px 6px", borderTop: "1px solid rgb(var(--ink-rgb) / 0.06)" }}>
        {!allowArChoice && (
          <p style={{ ...SKB, fontSize: 'var(--fs-8)', color: "rgb(var(--ink-rgb) / 0.3)", textTransform: "uppercase", letterSpacing: "0.1em", margin: "0 0 8px", textAlign: "center" }}>
            ...
            return (
              ...
              >
                <div style={{ height: 28, display: "flex", alignItems: "center", justifyContent: "center" }}>
                  <div style={{ width: boxW, height: Math.max(boxH, 6), border: `1.5px solid ${active ? RED : "rgb(var(--ink-rgb) / 0.4)"}` }} />
                </div>
                <span style={{ ...SKB, fontSize: 'var(--fs-8)', color: active ? RED : "rgb(var(--ink-rgb) / 0.55)", textTransform: "uppercase", letterSpacing: "0.04em", textAlign: "center", lineHeight: 1.3 }}>
                  {c.label}
                </span>
                <span style={{ ...SKR, fontSize: 'var(--fs-7)', color: active ? RED : "rgb(var(--ink-rgb) / 0.3)" }}>{c.ratioLabel}</span>
              </button>
            );
          })}
        </div>
      ...
      {/* ── Sub-tabs ── */}
      <div style={{ flexShrink: 0, display: "flex", borderTop: "1px solid rgb(var(--ink-rgb) / 0.06)" }}>
        {([
          ...
          >
            <span style={{ ...SKB, fontSize: 'var(--fs-8)', color: tab === t.id ? "var(--ink-100)" : "rgb(var(--ink-rgb) / 0.4)", textTransform: "uppercase", letterSpacing: "0.06em" }}>
              {t.label}
            </span>
          </button>
        ))}
      ...
      {/* ── Tab content ── */}
      <div style={{ flexShrink: 0, minHeight: 84, padding: "14px 16px 22px", borderTop: "1px solid rgb(var(--ink-rgb) / 0.06)" }}>
        {tab === "crop" && (
          <div>
            <div style={{ display: "flex", justifyContent: "center", marginBottom: 10 }}>
              <span style={{ ...SKB, fontSize: 'var(--fs-13)', color: straighten === 0 ? "rgb(var(--ink-rgb) / 0.5)" : RED }}>
                ...
                {Array.from({ length: 19 }).map((_, i) => {
                  ...
                  return (
                    <div key={i} style={{ position: "absolute", left: `calc(50% + ${offset}px)`, top: "50%", transform: "translate(-50%,-50%)", display: "flex", flexDirection: "column", alignItems: "center" }}>
                      <div style={{ width: 1, height: deg % 15 === 0 ? 14 : 8, background: deg === 0 ? RED : "rgb(var(--ink-rgb) / 0.3)" }} />
                    </div>
                  );
                })}
              </div>
            ...
            <p style={{ ...SKR, fontSize: 'var(--fs-7)', color: "rgb(var(--ink-rgb) / 0.25)", textAlign: "center", textTransform: "uppercase", letterSpacing: "0.08em", margin: "8px 0 0" }}>
        ...
        {tab === "rotate" && (
          <div style={{ display: "flex", alignItems: "center", justifyContent: "center", gap: 24 }}>
            <button onClick={() => setRotate((r) => (r - 90 + 360) % 360)} style={{ background: "transparent", border: "1px solid rgb(var(--ink-rgb) / 0.2)", cursor: "pointer", padding: "10px 14px" }}>
              <span style={{ ...SKB, fontSize: 'var(--fs-10)', color: "var(--ink-100)", textTransform: "uppercase", letterSpacing: "0.06em" }}>↺ 90°</span>
            </button>
            <span style={{ ...SKB, fontSize: 'var(--fs-12)', color: "rgb(var(--ink-rgb) / 0.5)" }}>{rotate}°</span>
            <button onClick={() => setRotate((r) => (r + 90) % 360)} style={{ background: "transparent", border: "1px solid rgb(var(--ink-rgb) / 0.2)", cursor: "pointer", padding: "10px 14px" }}>
              <span style={{ ...SKB, fontSize: 'var(--fs-10)', color: "var(--ink-100)", textTransform: "uppercase", letterSpacing: "0.06em" }}>↻ 90°</span>
            </button>
          </div>
        )}
        ...
        {tab === "skew" && (
          ...
          <div style={{ display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", gap: 6, opacity: 0.5 }}>
            <span style={{ ...SKB, fontSize: 'var(--fs-10)', color: "rgb(var(--ink-rgb) / 0.5)", textTransform: "uppercase", letterSpacing: "0.08em" }}>SKEW</span>
