  ...
  return (
    ...
    >
      <span style={{ position: "relative", width: W, height: H, overflow: "hidden", display: "block", filter: "drop-shadow(0 1px 2px rgb(var(--black-rgb) / 0.7))" }}>
        ...
        >
          <path d={buildWave(W * 2)} fill="none" stroke="var(--ink-100)" strokeWidth={1.4} strokeLinecap="round" opacity={playing ? 1 : 0.85} />
        </svg>
        {!playing && (
          <svg width={W} height={H} viewBox={`0 0 ${W} ${H}`} style={{ position: "absolute", left: 0, top: 0 }}>
            <line x1={1} y1={H - 1} x2={W - 1} y2={1} stroke="var(--ink-100)" strokeWidth={1.4} strokeLinecap="round" />
          </svg>
        )}
      </span>
    </button>
