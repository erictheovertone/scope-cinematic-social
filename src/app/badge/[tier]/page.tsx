  ...
  return (
    <div style={{ backgroundColor: 'var(--black)', minHeight: '100dvh', padding: '0 0 80px' }}>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', padding: '40px 20px 32px' }}>
        ...
        <p style={{ ...REG, fontSize: 'var(--fs-13)', color: 'rgb(var(--ink-rgb) / 0.6)', textAlign: 'center', lineHeight: 1.6, margin: 0, maxWidth: 280 }}>
          {detail.tagline}
        </p>
      </div>

      {/* Divider */}
      <div style={{ height: 1, backgroundColor: 'rgb(var(--ink-rgb) / 0.06)', margin: '0 20px 32px' }} />
      ...
      {detail.sections.map((section, i) => (
        <div key={i} style={{ padding: '0 20px 32px' }}>
          ...
          <p style={{ ...REG, fontSize: 'var(--fs-13)', color: 'rgb(var(--ink-rgb) / 0.75)', lineHeight: 1.4, margin: 0 }}>
            {section.body}
          </p>
          {i < detail.sections.length - 1 && (
            <div style={{ height: 1, backgroundColor: 'rgb(var(--ink-rgb) / 0.05)', marginTop: 28 }} />
          )}
        </div>
      ))}
      ...
      {(tier === 'free' || tier === 'pro') && (
        <div style={{ padding: '0 20px' }}>
          <button
            ...
            style={{ width: '100%', background: 'var(--ink-100)', border: 'none', cursor: 'pointer', padding: '14px 0' }}
          >
