...
//
// Catches throws in a PAGE / its children (below the root layout + providers).
// The providers stay mounted, so this is the graceful per-route recovery: the
// rest of the app (nav, narrator) keeps working while one screen shows its
// error. Root-layout / provider throws are handled one level up by
// global-error.tsx. Surfaces message + digest so a prod-only crash is legible.
...
export default function Error({
  ...
  useEffect(() => {
    console.error('[route-error] screen crashed:', error?.message, '| digest:', error?.digest, error);
  }, [error]);
