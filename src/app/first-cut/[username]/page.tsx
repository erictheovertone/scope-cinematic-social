...
export default function FirstCutPage() {
  ...
  return (
    <div style={{ minHeight: '100dvh', background: '#000', maxWidth: '30rem', margin: '0 auto', padding: '20px 18px 60px' }}>
      <div style={{ ...SKB, fontSize: 'var(--fs-7)', letterSpacing: '0.2em', color: '#E5E1DB', textTransform: 'uppercase', marginBottom: 16 }}>
      ...
      <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', gap: 14, marginBottom: 24, borderBottom: '1px solid #E5E1DB', paddingBottom: 18 }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 12, minWidth: 0 }}>
          ...
          <div style={{ minWidth: 0 }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
              <ApertureMark size={17.5} />
              <span style={{ ...SKB, fontSize: 'var(--fs-15)', color: '#E5E1DB', textTransform: 'uppercase', letterSpacing: '0.06em' }}>FIRST CUT</span>
            </div>
            <p style={{ ...SKB, fontSize: 'var(--fs-9)', color: 'rgba(229,225,219,0.45)', margin: '6px 0 0', textTransform: 'uppercase', letterSpacing: '0.08em' }}>
              @{username} · {data ? holding : 0} HOLDING
            </p>
          </div>
        </div>
        <div style={{ textAlign: 'right' as const, flexShrink: 0 }}>
          <p style={{ ...SKB, fontSize: 'var(--fs-26)', color: '#E5E1DB', margin: 0, letterSpacing: '-0.5px', fontVariantNumeric: 'tabular-nums' }}>
            {data ? usd(data.totalEarnedUsd) : '—'}
          </p>
          <p style={{ ...SKB, fontSize: 'var(--fs-7)', color: 'rgba(229,225,219,0.4)', textTransform: 'uppercase', letterSpacing: '0.16em', margin: '4px 0 0' }}>EARNED</p>
        </div>
      </div>

      {/* Rows */}
      {loaded && positions.length === 0 && (
        <p style={{ ...SKR, fontSize: 'var(--fs-11)', color: 'rgba(229,225,219,0.4)', textTransform: 'uppercase', letterSpacing: '0.08em' }}>
      ...
      {positions
        ...
        .map((p) => (
          <div key={p.postId} style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '12px 0', borderBottom: '1px solid rgba(229,225,219,0.07)', opacity: p.active ? 1 : 0.45 }}>
            <span style={{ ...SKB, fontSize: 'var(--fs-10)', width: 20, color: p.active ? '#E5E1DB' : 'rgba(229,225,219,0.35)', fontVariantNumeric: 'tabular-nums' }}>
            ...
            <div style={{ flex: 1, minWidth: 0 }}>
              <div style={{ display: 'flex', alignItems: 'baseline', gap: 7, margin: '0 0 3px', minWidth: 0 }}>
                <p style={{ ...SKB, fontSize: 'var(--fs-11)', color: '#E5E1DB', textTransform: 'uppercase', letterSpacing: '0.03em', margin: 0, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
                  {p.postTitle}
                </p>
                {p.ticker && <TickerMark ticker={p.ticker} size={11.5} />}
              </div>
              <p style={{ ...SKR, fontSize: 'var(--fs-9)', color: 'rgba(229,225,219,0.45)', margin: 0, textTransform: 'uppercase', letterSpacing: '0.04em' }}>
                @{p.creatorHandle} · holding {p.holdingDays} days
              </p>
            </div>
            <div style={{ textAlign: 'right' as const }}>
              <p style={{ ...SKB, fontSize: 'var(--fs-13)', color: '#E5E1DB', margin: '0 0 2px', fontVariantNumeric: 'tabular-nums' }}>{usd(p.earnedUsd)}</p>
              <p style={{ ...SKB, fontSize: 'var(--fs-6_5)', color: p.active ? '#E5E1DB' : 'rgba(229,225,219,0.3)', textTransform: 'uppercase', letterSpacing: '0.1em', margin: 0 }}>
      ...
      <button onClick={() => router.back()} style={{ ...SKB, fontSize: 'var(--fs-10)', color: 'rgba(229,225,219,0.5)', background: 'transparent', border: 'none', textTransform: 'uppercase', letterSpacing: '0.1em', cursor: 'pointer', marginTop: 28 }}>
        ← BACK
      </button>
    </div>
  );
