...
export default function FirstCutPage() {
  ...
  if (!economyPreviewEnabled()) {
    return (
      <div style={{ minHeight: '100dvh', background: 'var(--canvas)', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
  ...
  return (
    <div style={{ minHeight: '100dvh', background: 'var(--canvas)', maxWidth: '30rem', margin: '0 auto', padding: '20px 18px 60px' }}>
      ...
      <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', gap: 14, marginBottom: 24, borderBottom: '1px solid var(--ink-100)', paddingBottom: 18 }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 12, minWidth: 0 }}>
          ...
          <div style={{ minWidth: 0 }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
              ...
              <span style={{ ...SKB, fontSize: 'var(--fs-15)', color: 'var(--ink-100)', textTransform: 'uppercase', letterSpacing: '0.06em' }}>FIRST CUT</span>
            </div>
            <p style={{ ...SKB, fontSize: 'var(--fs-9)', color: 'rgb(var(--ink-rgb) / 0.45)', margin: '6px 0 0', textTransform: 'uppercase', letterSpacing: '0.08em' }}>
              @{username} · {data ? holding : 0} HOLDING
            </p>
          </div>
        </div>
        <div style={{ textAlign: 'right' as const, flexShrink: 0 }}>
          <p style={{ ...SKB, fontSize: 'var(--fs-26)', color: 'var(--ink-100)', margin: 0, letterSpacing: '-0.5px', fontVariantNumeric: 'tabular-nums' }}>
            {data ? usd(data.totalEarnedUsd) : '—'}
          </p>
          <p style={{ ...SKB, fontSize: 'var(--fs-7)', color: 'rgb(var(--ink-rgb) / 0.4)', textTransform: 'uppercase', letterSpacing: '0.16em', margin: '4px 0 0' }}>EARNED</p>
        </div>
      </div>

      {/* Rows */}
      {loaded && positions.length === 0 && (
        <p style={{ ...SKR, fontSize: 'var(--fs-11)', color: 'rgb(var(--ink-rgb) / 0.4)', textTransform: 'uppercase', letterSpacing: '0.08em' }}>
      ...
      {positions
        ...
        .map((p) => (
          <div key={p.postId} style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '12px 0', borderBottom: '1px solid rgb(var(--ink-rgb) / 0.07)', opacity: p.active ? 1 : 0.45 }}>
            <span style={{ ...SKB, fontSize: 'var(--fs-10)', width: 20, color: p.active ? 'var(--ink-100)' : 'rgb(var(--ink-rgb) / 0.35)', fontVariantNumeric: 'tabular-nums' }}>
            ...
            <div style={{ flex: 1, minWidth: 0 }}>
              ...
              <p style={{ ...SKR, fontSize: 'var(--fs-9)', color: 'rgb(var(--ink-rgb) / 0.45)', margin: 0, textTransform: 'uppercase', letterSpacing: '0.04em' }}>
                @{p.creatorHandle} · holding {p.holdingDays} days
              </p>
            </div>
            <div style={{ textAlign: 'right' as const }}>
              <p style={{ ...SKB, fontSize: 'var(--fs-13)', color: 'var(--ink-100)', margin: '0 0 2px', fontVariantNumeric: 'tabular-nums' }}>{usd(p.earnedUsd)}</p>
              <p style={{ ...SKB, fontSize: 'var(--fs-6_5)', color: p.active ? 'var(--ink-100)' : 'rgb(var(--ink-rgb) / 0.3)', textTransform: 'uppercase', letterSpacing: '0.1em', margin: 0 }}>
      ...
      <button onClick={() => router.back()} style={{ ...SKB, fontSize: 'var(--fs-10)', color: 'rgb(var(--ink-rgb) / 0.5)', background: 'transparent', border: 'none', textTransform: 'uppercase', letterSpacing: '0.1em', cursor: 'pointer', marginTop: 28 }}>
        ← BACK
      </button>
    </div>
  );
