...
function MobileDMInbox() {
  ...
  return (
    <main style={{ minHeight: '100dvh', background: 'var(--canvas)', paddingBottom: 'calc(72px + env(safe-area-inset-bottom, 0px))' }}>
      ...
      {convs === null ? (
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', minHeight: '40vh' }}>
          <span style={{ ...SKB, fontSize: 'var(--fs-10)', color: 'rgb(var(--ink-rgb) / 0.4)', textTransform: 'uppercase', letterSpacing: '0.08em' }}>LOADING…</span>
        </div>
      ) : convs.length === 0 ? (
        <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', minHeight: '50vh', gap: 12, padding: '0 40px', textAlign: 'center' }}>
          <span style={{ ...SKB, fontSize: 'var(--fs-11)', color: 'rgb(var(--ink-rgb) / 0.55)', textTransform: 'uppercase', letterSpacing: '0.1em' }}>No messages yet</span>
            ...
            return (
              <button
                ...
                style={{
                  ...
                  background: 'transparent', border: 'none', borderBottom: '1px solid rgb(var(--ink-rgb) / 0.06)',
              ...
              >
                ...
                <div style={{ flex: 1, minWidth: 0 }}>
                  <span style={{ ...SKB, fontSize: 'var(--fs-10)', color: 'var(--ink-100)', textTransform: 'uppercase', letterSpacing: '0.02em', display: 'block' }}>
                    @{handle ?? 'unknown'}
                  </span>
                  <span style={{
                    ...(unread ? SKB : SKR),
                    fontSize: 'var(--fs-9)',
                    color: unread ? 'rgb(var(--ink-rgb) / 0.9)' : 'rgb(var(--ink-rgb) / 0.45)',
                ...
                <div style={{ flexShrink: 0, display: 'flex', flexDirection: 'column', alignItems: 'flex-end', gap: 6 }}>
                  <span style={{ ...SKR, fontSize: 'var(--fs-7)', color: 'rgb(var(--ink-rgb) / 0.4)', fontVariantNumeric: 'tabular-nums' }}>{dmTimeAgo(c.lastMessageAt)}</span>
                  {unread && <span aria-label="unread" style={{ width: 8, height: 8, borderRadius: '50%', background: 'var(--ink-100)', display: 'block' }} />}
                </div>
              </button>
            );
          })}
