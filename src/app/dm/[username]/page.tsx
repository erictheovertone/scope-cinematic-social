    ...
    <div style={{ height: '100dvh', display: 'flex', flexDirection: 'column', background: 'var(--canvas)' }}>
      ...
      <div style={{ flexShrink: 0, display: 'flex', alignItems: 'center', gap: 10, padding: 'calc(12px + env(safe-area-inset-top, 0px)) 14px 10px', borderBottom: '1px solid rgb(var(--ink-rgb) / 0.08)' }}>
        <button onClick={() => router.back()} aria-label="Back" className="tappable" style={{ background: 'transparent', border: 'none', cursor: 'pointer', padding: 4, lineHeight: 0, flexShrink: 0 }}>
          <svg width="16.5" height="16.5" viewBox="0 0 24 24" fill="none" stroke="var(--ink-100)" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M15 18l-6-6 6-6" /></svg>
        </button>
        <button
        ...
        >
          ...
          <span style={{ ...SKB, fontSize: 'var(--fs-11)', color: 'var(--ink-100)', textTransform: 'uppercase', letterSpacing: '0.04em', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>@{other?.handle ?? username}</span>
        </button>
      </div>
      ...
      <div ref={scrollRef} onScroll={onScroll} style={{ flex: 1, overflowY: 'auto', WebkitOverflowScrolling: 'touch', padding: '14px 14px 8px', display: 'flex', flexDirection: 'column' }}>
        {loading ? (
          <div style={{ margin: 'auto' }}><span style={{ ...SKB, fontSize: 'var(--fs-10)', color: 'rgb(var(--ink-rgb) / 0.4)', textTransform: 'uppercase', letterSpacing: '0.08em' }}>LOADING…</span></div>
        ) : notFound ? (
          <div style={{ margin: 'auto', textAlign: 'center' }}><span style={{ ...SKR, fontSize: 'var(--fs-10)', color: 'rgb(var(--ink-rgb) / 0.4)', textTransform: 'uppercase', letterSpacing: '0.08em' }}>User not found</span></div>
        ) : items.length === 0 ? (
          <div style={{ margin: 'auto', textAlign: 'center', padding: '0 30px' }}>
            <span style={{ ...SKR, fontSize: 'var(--fs-9)', color: 'rgb(var(--ink-rgb) / 0.35)', textTransform: 'uppercase', letterSpacing: '0.08em', lineHeight: 1.6 }}>
              ...
              <div key={it.key}>
                {showTime && (
                  <div style={{ textAlign: 'center', margin: '10px 0 8px' }}>
                    <span style={{ ...SKR, fontSize: 'var(--fs-7)', color: 'rgb(var(--ink-rgb) / 0.3)', textTransform: 'uppercase', letterSpacing: '0.1em' }}>{dmTimeAgo(it.iso)}</span>
                  </div>
                )}
                <div style={{ display: 'flex', justifyContent: it.mine ? 'flex-end' : 'flex-start', marginBottom: 4 }}>
                  ...
                  >
                    <span style={{ ...SKR, fontSize: 'var(--fs-10)', color: 'var(--ink-100)', lineHeight: 1.4, whiteSpace: 'pre-wrap', wordBreak: 'break-word', display: 'block' }}>{it.body}</span>
                    {it.status === 'failed' && (
      ...
      <div style={{ flexShrink: 0, display: 'flex', alignItems: 'flex-end', gap: 8, padding: '10px 12px calc(10px + env(safe-area-inset-bottom, 0px))', borderTop: '1px solid rgb(var(--ink-rgb) / 0.08)', background: 'var(--canvas)' }}>
        <textarea
          value={draft}
        ...
        <button
          ...
          className="tappable"
          style={{
            ...SKB, fontSize: 'var(--fs-9)', color: draft.trim() ? 'var(--ink-100)' : 'rgb(var(--ink-rgb) / 0.3)',
