  ...
  return (
    <div style={{ minHeight: "100vh", background: "#000", color: "var(--ink-100)" }}>
      ...
      <div style={{ position: "relative", overflow: "hidden", padding: "40px 20px 22px", maxWidth: 720, margin: "0 auto" }}>
        ...
        <div style={{ position: "relative", display: "flex", gap: 16, alignItems: "flex-start" }}>
          ...
          <div style={{ minWidth: 0, flex: 1 }}>
            ...
            <p style={{ ...SKB, fontSize: 20, color: "var(--ink-100)", margin: "0 0 2px", whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis" }}>{profile?.display_name || handle}</p>
            <button onClick={() => router.push(`/profile/${handle}`)} style={{ ...SKR, fontSize: 12, color: "rgb(var(--ink-rgb) / 0.5)", background: "none", border: "none", cursor: "pointer", padding: 0 }}>@{handle}</button>
          </div>
        </div>
      ...
      <div style={{ maxWidth: 720, margin: "0 auto", padding: "4px 20px 60px" }}>
        {tracks === null && <p style={{ ...SKR, fontSize: 13, color: "rgb(var(--ink-rgb) / 0.4)", padding: "20px 0" }}>Loading…</p>}
        {empty && <p style={{ ...SKR, fontSize: 13, color: "rgb(var(--ink-rgb) / 0.4)", padding: "20px 0" }}>No discography yet.</p>}

        {tracks?.map((t) => {
          const isPlaying = playing === t.id;
          const inN = inPostsByTrack.get(t.id) ?? 0;
          return (
            <div key={t.id} style={{ display: "flex", alignItems: "center", gap: 13, padding: "14px 0", borderBottom: `1px solid rgb(var(--ink-rgb) / 0.07)` }}>
              ...
              <button onClick={() => togglePlay(t)} aria-label={isPlaying ? "Pause" : "Play"} style={{ flexShrink: 0, width: 34, height: 34, borderRadius: "50%", border: `1px solid ${HAIR}`, background: "transparent", cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center", padding: 0 }}>
                {isPlaying
                  ? <svg width="12" height="12" viewBox="0 0 24 24" fill="var(--ink-100)"><rect x="6" y="5" width="4" height="14" /><rect x="14" y="5" width="4" height="14" /></svg>
                  : <svg width="12" height="12" viewBox="0 0 24 24" fill="var(--ink-100)"><path d="M7 5v14l12-7z" /></svg>}
              </button>
              <div style={{ flex: 1, minWidth: 0 }}>
                <div style={{ display: "flex", alignItems: "baseline", gap: 8 }}>
                  <span style={{ ...SKB, fontSize: 14, color: "var(--ink-100)", whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis" }}>{t.title}</span>
                ...
                <div style={{ display: "flex", alignItems: "center", gap: 8, flexWrap: "wrap" }}>
                  {t.keywords.slice(0, 4).map((k) => (
                    <span key={k} style={{ ...SKR, fontSize: 10.5, color: "rgb(var(--ink-rgb) / 0.4)", border: `1px solid ${HAIR}`, padding: "2px 7px" }}>{k}</span>
                  ))}
                  {inN > 0 && <span style={{ ...SKR, fontSize: 10.5, color: "rgb(var(--ink-rgb) / 0.3)" }}>in {inN} {inN === 1 ? "post" : "posts"}</span>}
                </div>
              </div>
            </div>
          );
      ...
      {!empty && (
        <div style={{ maxWidth: 720, margin: "0 auto", padding: "0 20px 80px", display: "flex", flexDirection: "column", gap: 14, alignItems: "flex-start" }}>
          ...
          <button onClick={() => setShowContribute(true)} style={{ ...SKB, fontSize: 12, color: "rgb(var(--ink-rgb) / 0.55)", textTransform: "uppercase", letterSpacing: "0.16em", background: "none", border: "none", cursor: "pointer", padding: 0 }}>
...
function Stat({ label, value }: { label: string; value: string }) {
  return (
    <div>
      <p style={{ ...SKB, fontSize: 17, color: "var(--ink-100)", margin: "0 0 3px", fontVariantNumeric: "tabular-nums" }}>{value}</p>
      <p style={{ ...SKR, fontSize: 10, color: "rgb(var(--ink-rgb) / 0.4)", textTransform: "uppercase", letterSpacing: "0.1em", margin: 0 }}>{label}</p>
    </div>
  );
}
