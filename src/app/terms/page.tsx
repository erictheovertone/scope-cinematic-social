...
export default function TermsPage() {
  return (
    ...
    <div className="bg-canvas" style={{ position: "fixed", inset: 0, overflowY: "auto", WebkitOverflowScrolling: "touch" }}>
      <div style={{ background: "var(--canvas)", minHeight: "100%", padding: "calc(20px + env(safe-area-inset-top, 0px)) 20px calc(60px + env(safe-area-inset-bottom, 0px))", maxWidth: 640, margin: "0 auto" }}>
      ...
      <h1 style={{ ...SKB, fontSize: 22, color: "var(--ink-100)", textTransform: "uppercase", letterSpacing: "0.06em", margin: "22px 0 4px" }}>Terms of Service</h1>
      ...
      {SECTIONS.map((s) => (
        <section key={s.title} style={{ margin: "0 0 24px" }}>
          <h2 style={{ ...SKB, fontSize: 12, color: "rgb(var(--ink-rgb) / 0.85)", textTransform: "uppercase", letterSpacing: "0.14em", margin: "0 0 8px" }}>{s.title}</h2>
          <p style={{ ...SKR, fontSize: 13, color: "rgb(var(--ink-rgb) / 0.7)", lineHeight: 1.6, margin: 0 }}>{s.body}</p>
        </section>
      ))}
      <section id="dmca" style={{ margin: "36px 0 0", borderTop: "1px solid rgb(var(--ink-rgb) / 0.12)", paddingTop: 24 }}>
        <h2 style={{ ...SKB, fontSize: 12, color: "rgb(var(--ink-rgb) / 0.85)", textTransform: "uppercase", letterSpacing: "0.14em", margin: "0 0 8px" }}>COPYRIGHT / DMCA</h2>
        <p style={{ ...SKR, fontSize: 13, color: "rgb(var(--ink-rgb) / 0.7)", lineHeight: 1.6, margin: 0 }}>
          ...
          <a href="mailto:dmca@scopeapp.world" style={{ color: "var(--ink-100)" }}>dmca@scopeapp.world</a>. We remove infringing content and terminate repeat infringers.
        </p>
      </section>
      </div>
    </div>
