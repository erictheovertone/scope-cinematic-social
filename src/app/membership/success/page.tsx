    ...
    <div style={{ position: "fixed", inset: 0, backgroundColor: "var(--black)", display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", overflow: "hidden", opacity: exiting ? 0 : 1, transition: exiting ? "opacity 0.8s ease" : "none" }}>
      ...
      {phase !== "loading" && (
        <>
          ...
          <div style={{ position: "absolute", top: 24, right: 24, width: 40, height: 40, borderTop: "2px solid var(--ink-100)", borderRight: "2px solid var(--ink-100)", animation: "cornerReveal 0.4s ease 0.1s forwards, flicker 4s ease 1.2s infinite", opacity: 0 }} />
          <div style={{ position: "absolute", bottom: 24, left: 24, width: 40, height: 40, borderBottom: "2px solid var(--ink-100)", borderLeft: "2px solid var(--ink-100)", animation: "cornerReveal 0.4s ease 0.2s forwards, flicker 4s ease 1.4s infinite", opacity: 0 }} />
          <div style={{ position: "absolute", bottom: 24, right: 24, width: 40, height: 40, borderBottom: "2px solid var(--ink-100)", borderRight: "2px solid var(--ink-100)", animation: "cornerReveal 0.4s ease 0.3s forwards, flicker 4s ease 1.6s infinite", opacity: 0 }} />
        </>
      )}
      ...
      {phase !== "loading" && (
        <p style={{ ...BOLD, fontSize: 'var(--fs-24)', color: "var(--ink-100)", textTransform: "uppercase", letterSpacing: "-0.02em", margin: "0 0 16px", animation: "fadeUp 0.8s ease forwards" }}>IN-HOUSE CREATOR</p>
      )}
      {(phase === "text" || phase === "done") && (
        <p style={{ fontFamily: "'SK-Modernist', sans-serif", fontWeight: 700, fontSize: 'var(--fs-13)', color: "rgb(var(--ink-rgb) / 0.6)", letterSpacing: "0.1em" }}>
          {typed}<span style={{ animation: "blink 1s step-end infinite" }}>_</span>
        </p>
      )}
      {phase === "done" && (
        <p style={{ ...REG, fontSize: 'var(--fs-11)', color: "rgb(var(--ink-rgb) / 0.3)", marginTop: 12, animation: "fadeUp 0.8s ease forwards" }}>Post 10+ times monthly to keep your badge.</p>
      )}
      <Styles />
    </div>
  );
...
function Top1kCelebration({ onDone }: { onDone: () => void }) {
  ...
  return (
    <div style={{ position: "fixed", inset: 0, backgroundColor: "var(--black)", display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", overflow: "hidden", opacity: exiting ? 0 : 1, transition: exiting ? "opacity 0.8s ease" : "none" }}>
      ...
      {phase === "done" && (
        <>
          <p style={{ ...REG, fontSize: 'var(--fs-11)', color: "rgb(var(--ink-rgb) / 0.4)", margin: "4px 0 0", animation: "fadeUp 0.8s ease forwards" }}>Your collector rank on Scope</p>
  ...
  return (
    <div style={{
      ...
      <div style={{ position: "relative", zIndex: 4, textAlign: "center", padding: "0 32px" }}>
        {(phase === "reveal" || phase === "done") && (
          <>
            <p style={{ ...BOLD, fontSize: 'var(--fs-11)', color: "rgb(var(--ink-rgb) / 0.4)", textTransform: "uppercase", letterSpacing: "0.3em", margin: "0 0 8px", animation: "fadeUp 0.8s ease forwards" }}>
              FOUNDING MEMBER
            </p>
            <p style={{
            ...
            <p style={{ ...BOLD, fontSize: 'var(--fs-10)', color: "rgb(var(--ink-rgb) / 0.3)", textTransform: "uppercase", letterSpacing: "0.2em", margin: "0 0 20px", animation: "fadeUp 0.8s ease 0.2s forwards", opacity: 0 }}>
              OF 500
            </p>
          </>
        )}
        {phase === "done" && (
          <>
            <p style={{ ...REG, fontSize: 'var(--fs-13)', color: "rgb(var(--ink-rgb) / 0.6)", lineHeight: 1.6, margin: "0 0 8px", animation: "fadeUp 0.8s ease forwards" }}>
              You are one of the first 500 members of Scope.
            </p>
            <p style={{ ...REG, fontSize: 'var(--fs-11)', color: "rgb(var(--ink-rgb) / 0.3)", animation: "fadeUp 0.8s ease 0.2s forwards", opacity: 0 }}>
