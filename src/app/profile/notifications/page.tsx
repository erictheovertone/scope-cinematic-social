...
export default function NotificationsPage() {
  ...
  const renderRow = (n: AppNotification) => (
    ...
    >
      ...
      {!n.is_read && (
        <div style={{ position: "absolute", left: 5, top: "50%", transform: "translateY(-50%)", width: 4.5, height: 4.5, background: "var(--ink-100)" }} />
      )}
      ...
      <div className="flex-1 min-w-0">
        <p style={{ ...SKR, fontSize: 'var(--fs-14)', color: "var(--ink-100)", letterSpacing: "-0.16px", lineHeight: 1.4, margin: 0, wordBreak: "break-word" }}>
          <NotificationActorMessage handle={n.actor_handle ?? null} type={n.type} onNavigate={goToActor} />
        </p>
        <p style={{ ...SKR, fontSize: 'var(--fs-9)', color: "rgb(var(--ink-rgb) / 0.4)", letterSpacing: "-0.1px", margin: "2px 0 0" }}>
    ...
    <div style={{ width: '50%', flexShrink: 0 }}>
      {list.length === 0 ? (
        <div className="flex items-center justify-center mt-12">
          <p style={{ ...SKR, fontSize: 'var(--fs-9)', color: "rgb(var(--ink-rgb) / 0.4)" }}>{emptyLabel}</p>
        </div>
      ) : (
        list.map(renderRow)
      )}
  ...
  return (
    <div className="bg-black w-full app-shell screen-min mx-auto flex flex-col">
      ...
      <div className="relative flex items-center px-[4px] pb-[10px]" style={{ paddingTop: 'calc(12px + var(--safe-top))' }}>
        ...
        >
          <span style={{ ...SKR, fontSize: 'var(--fs-9)', color: "var(--ink-100)", letterSpacing: "-0.18px" }}>
            ← Back
          </span>
        </button>
        <span
          className="absolute left-1/2"
          style={{ ...SKB, fontSize: 'var(--fs-9)', color: "var(--ink-100)", letterSpacing: "-0.18px", transform: "translateX(-50%)" }}
        >
          NOTIFICATIONS
        </span>
      </div>
      ...
      <div style={{ display: "flex", borderBottom: "1px solid rgb(var(--ink-rgb) / 0.1)" }}>
        {(["social", "economic"] as const).map((t) => (
          <button
            ...
            style={{
              ...
              borderBottom: `1px solid ${tab === t ? "var(--ink-100)" : "transparent"}`,
              cursor: "pointer", padding: "9px 0",
              ...SKB, fontSize: 'var(--fs-9)', letterSpacing: "0.12em",
              color: tab === t ? "var(--ink-100)" : "rgb(var(--ink-rgb) / 0.4)", textTransform: "uppercase",
            }}
          >
      ...
      <div ref={listRef} data-swipe-exclude className="flex-1 overflow-y-auto" style={{ overflowX: 'hidden' }}>
        {loading ? (
          ...
          <div className="flex items-center justify-center mt-12">
            <p style={{ ...SKR, fontSize: 'var(--fs-9)', color: "rgb(var(--ink-rgb) / 0.4)" }}>
