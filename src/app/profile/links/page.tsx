  ...
  return (
    <div className="bg-black" style={{ position: "fixed", inset: 0, overflowY: "auto" }}>
      ...
      {isDirty && !addingLink && (
        <div style={{ position: 'fixed', top: 0, left: 0, right: 0, zIndex: 200, background: '#000', borderBottom: '1px solid rgb(var(--ink-rgb) / 0.15)', padding: 'calc(10px + var(--safe-top)) 20px 10px', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }/* Brief X3 §3 — fixed top:0 unsaved-changes bar; pad top by --safe-top so it clears the notch */}>
          ...
          <button
            ...
            disabled={saving}
            style={{ background: 'var(--ink-100)', border: 'none', cursor: saving ? 'default' : 'pointer', padding: '8px 18px' }}
          >
      ...
      <div style={{ position: "relative", display: "flex", alignItems: "center", padding: isDirty && !addingLink ? "calc(58px + var(--safe-top)) 16px 14px" : "calc(14px + var(--safe-top)) 16px 14px" }/* Brief X3 §3 — pad top by --safe-top (clears notch when it's the top chrome; tracks the safe-padded floating bar when dirty) */}>
        <button
          onClick={() => router.back()}
          style={{ ...SKB, fontSize: 'var(--fs-11)', color: "var(--ink-100)", background: "none", border: "none", cursor: "pointer", padding: 0 }}
        >
          ← Back
        </button>
        <span style={{ ...SKB, fontSize: 'var(--fs-11)', color: "var(--ink-100)", position: "absolute", left: "50%", transform: "translateX(-50%)", letterSpacing: "0.05em" }}>
          LINK MANAGER
        </span>
        {links.length > 0 && !addingLink && (
          <button
            onClick={handleSave}
            disabled={saving}
            style={{ ...SKB, fontSize: 'var(--fs-11)', color: saved ? "var(--ink-100)" : "var(--ink-100)", background: "none", border: "none", cursor: "pointer", padding: 0, marginLeft: "auto", opacity: saving ? 0.5 : 1 }}
          >
      ...
      <div style={{ height: 1, backgroundColor: "rgb(var(--ink-rgb) / 0.12)" }} />
      ...
      <div style={{ padding: "16px 20px 40px" }}>
        ...
        {links.map((link, i) => {
          ...
          return (
            <div key={i} style={{ marginBottom: 8, border: "1px solid rgb(var(--ink-rgb) / 0.08)", background: "rgb(var(--ink-rgb) / 0.04)" }}>
              {/* Summary row */}
              <div style={{ display: "flex", alignItems: "center", gap: 8, padding: "8px 10px" }}>
                ...
                <div style={{ flex: 1, overflow: "hidden", cursor: "pointer" }} onClick={() => setExpandedIdx(isExpanded ? null : i)}>
                  <input
                    ...
                    placeholder="Title"
                    style={{ ...SKR, fontSize: 'max(16px, var(--fs-10))', color: "var(--ink-100)", background: "transparent", border: "none", borderBottom: "1px solid rgb(var(--ink-rgb) / 0.15)", outline: "none", width: "100%", padding: "2px 0", marginBottom: 2 }}
                  />
                  <p style={{ ...SKR, fontSize: 'var(--fs-8)', color: "var(--ink-100)", opacity: 0.4, margin: 0, overflow: "hidden", whiteSpace: "nowrap", textOverflow: "ellipsis" }}>
                    {link.url}
                  </p>
                </div>
                <div style={{ display: "flex", flexDirection: "column", gap: 2, flexShrink: 0 }}>
                  <button
                    ...
                    style={{ ...SKB, fontSize: 'var(--fs-10)', color: i === 0 ? "rgb(var(--ink-rgb) / 0.2)" : "var(--ink-100)", background: "none", border: "none", cursor: i === 0 ? "default" : "pointer", padding: "0 2px", lineHeight: 1 }}
                  >▴</button>
                  <button
                    onClick={() => { if (i === links.length - 1) return; markDirty(prev => { const a = [...prev]; [a[i], a[i+1]] = [a[i+1], a[i]]; return a; }); }}
                    style={{ ...SKB, fontSize: 'var(--fs-10)', color: i === links.length - 1 ? "rgb(var(--ink-rgb) / 0.2)" : "var(--ink-100)", background: "none", border: "none", cursor: i === links.length - 1 ? "default" : "pointer", padding: "0 2px", lineHeight: 1 }}
                  >▾</button>
                </div>
                <button
                  onClick={() => { markDirty(prev => prev.filter((_, j) => j !== i)); if (expandedIdx === i) setExpandedIdx(null); }}
                  style={{ ...SKB, fontSize: 'var(--fs-16)', color: "rgb(var(--ink-rgb) / 0.4)", background: "none", border: "none", cursor: "pointer", padding: "0 2px", lineHeight: 1, flexShrink: 0 }}
                >×</button>
              </div>

              {/* Expanded detail */}
              {isExpanded && (
                <div style={{ padding: "0 10px 10px", borderTop: "1px solid rgb(var(--ink-rgb) / 0.06)" }}>
                  {/* Description */}
                  <div style={{ marginTop: 8 }}>
                    <p style={{ ...SKB, fontSize: 'var(--fs-9)', color: "var(--ink-100)", opacity: 0.5, margin: "0 0 4px" }}>DESCRIPTION</p>
                    <textarea
                      ...
                      rows={2}
                      style={{ ...SKR, fontSize: 'max(16px, var(--fs-9))', color: "var(--ink-100)", background: "transparent", border: "1px solid rgb(var(--ink-rgb) / 0.15)", outline: "none", width: "100%", padding: "6px 8px", resize: "none", boxSizing: "border-box", lineHeight: 1.4 }}
                    />
                    <p style={{ ...SKR, fontSize: 'var(--fs-8)', color: "var(--ink-100)", opacity: 0.3, margin: "2px 0 0", textAlign: "right" }}>
                  ...
                  <div style={{ marginTop: 10 }}>
                    <p style={{ ...SKB, fontSize: 'var(--fs-9)', color: "var(--ink-100)", opacity: 0.5, margin: "0 0 6px" }}>CUSTOM IMAGE</p>
                    <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
                      ...
                      <div style={{ flex: 1 }}>
                        ...
                        <label
                          ...
                          style={{ ...SKB, fontSize: 'var(--fs-9)', color: uploadingThumb === i ? "rgb(var(--ink-rgb) / 0.4)" : "var(--ink-100)", border: "1px solid rgb(var(--ink-rgb) / 0.2)", padding: "6px 10px", cursor: uploadingThumb === i ? "default" : "pointer", display: "inline-block" }}
                        >
                        ...
                        {link.custom_thumbnail_url && (
                          <button
                            ...
                            style={{ ...SKR, fontSize: 'var(--fs-9)', color: "rgb(var(--ink-rgb) / 0.4)", background: "none", border: "none", cursor: "pointer", marginLeft: 8, padding: 0 }}
                          >
                            Remove
                          </button>
                        )}
              ...
              <button
                onClick={() => setExpandedIdx(isExpanded ? null : i)}
                style={{ ...SKR, fontSize: 'var(--fs-8)', color: "rgb(var(--ink-rgb) / 0.3)", background: "none", border: "none", borderTop: "1px solid rgb(var(--ink-rgb) / 0.06)", cursor: "pointer", width: "100%", padding: "4px 0", textAlign: "center" }}
              >
        ...
        {/* Add link form */}
        {addingLink ? (
          <div style={{ border: "1px solid rgb(var(--ink-rgb) / 0.15)", padding: 12, marginBottom: 8 }}>
            <input
              autoFocus
              type="url"
              value={newLinkUrl}
              ...
              style={{ ...SKR, fontSize: 'max(16px, var(--fs-10))', color: "var(--ink-100)", background: "transparent", border: "none", borderBottom: "1px solid rgb(var(--ink-rgb) / 0.2)", outline: "none", width: "100%", padding: "4px 0", marginBottom: 10, boxSizing: "border-box" }}
            />
            {fetchingPreview && (
              <p style={{ ...SKR, fontSize: 'var(--fs-9)', color: "rgb(var(--ink-rgb) / 0.4)", margin: "0 0 8px" }}>Fetching preview…</p>
            )}
            {pendingLink && (
              <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 10 }}>
                ...
                <div style={{ flex: 1 }}>
                  <input
                    ...
                    placeholder="Title"
                    style={{ ...SKR, fontSize: 'max(16px, var(--fs-10))', color: "var(--ink-100)", background: "transparent", border: "none", borderBottom: "1px solid rgb(var(--ink-rgb) / 0.2)", outline: "none", width: "100%", padding: "2px 0" }}
                  />
                  {pendingLink.is_video && (
                    <p style={{ ...SKB, fontSize: 'var(--fs-8)', color: "var(--ink-100)", opacity: 0.8, margin: "3px 0 0" }}>VIDEO</p>
                  )}
                </div>
              </div>
            )}
            <div style={{ display: "flex", gap: 8 }}>
              <button
                ...
                style={{ ...SKB, fontSize: 'var(--fs-10)', color: "var(--ink-100)", background: "transparent", border: "1px solid white", cursor: pendingLink ? "pointer" : "default", padding: "6px 12px", opacity: pendingLink ? 1 : 0.3 }}
              >ADD</button>
              <button
                onClick={() => { setAddingLink(false); setNewLinkUrl(""); setPendingLink(null); }}
                style={{ ...SKR, fontSize: 'var(--fs-10)', color: "rgb(var(--ink-rgb) / 0.5)", background: "transparent", border: "1px solid rgb(var(--ink-rgb) / 0.2)", cursor: "pointer", padding: "6px 12px" }}
              >CANCEL</button>
            </div>
          </div>
        ) : links.length < 6 ? (
          <button
            ...
            style={{ ...SKB, fontSize: 'var(--fs-10)', color: "var(--ink-100)", background: "transparent", border: "1px solid rgb(var(--ink-rgb) / 0.2)", cursor: "pointer", padding: "8px 12px", width: "100%", textAlign: "left" }}
          >
            + ADD LINK
          </button>
        ) : null}
        ...
        {links.length > 0 && !addingLink && (
          <button
            onClick={handleSave}
            disabled={saving}
            style={{ ...SKB, fontSize: 'var(--fs-11)', color: "var(--on-ink)", background: "var(--ink-100)", border: "none", cursor: "pointer", padding: "12px", width: "100%", marginTop: 16, opacity: saving ? 0.6 : 1 }/* X2c — ivory fill; text → --on-ink */}
          >
