  ...
  return (
    ...
    <div className="bg-black" style={{ position: 'fixed', inset: 0, overflowY: 'auto' }}>
      ...
      <div style={{ position: 'relative', display: 'flex', alignItems: 'center', padding: '14px 16px', paddingTop: 'calc(14px + env(safe-area-inset-top, 0px))' }}>
        ...
        <span style={{ ...MONO, fontSize: 'var(--fs-15)', color: 'var(--ink-100)', position: 'absolute', left: '50%', transform: 'translateX(-50%)', letterSpacing: '0.08em', textTransform: 'uppercase' }}>
          SETTINGS
        </span>
      </div>
      ...
      {sections.map((sec) => (
        <div key={sec.title}>
          <p style={{ ...MONO, fontSize: 'var(--fs-8)', color: 'rgb(var(--ink-rgb) / 0.38)', textTransform: 'uppercase', letterSpacing: '0.2em', margin: '26px 20px 6px' }}>
            {sec.title}
          </p>
          {sec.rows.map((item) => (
            <div key={item.label}>
              ...
              >
                <span style={{ ...MONO, fontSize: 'var(--fs-11)', color: photoError && item.label === photoLabel ? 'var(--ink-100)' : 'var(--ink-100)', textTransform: 'uppercase', letterSpacing: '0.06em' }}>
                  {item.label}
                </span>
              </button>
              <div style={{ height: 1, backgroundColor: 'rgb(var(--ink-rgb) / 0.12)', margin: '0 20px' }} />
            </div>
          ))}
        </div>
      ))}
      ...
      >
        <span style={{ ...MONO, fontSize: 'var(--fs-11)', color: 'var(--ink-100)', textTransform: 'uppercase', letterSpacing: '0.06em' }}>Log Out</span>
      </button>
