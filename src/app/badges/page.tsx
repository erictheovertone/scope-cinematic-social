    ...
    <div className="bg-black" style={{ position: 'fixed', inset: 0, overflowY: 'auto' }}>
      <div style={{ maxWidth: '30rem', margin: '0 auto', padding: '16px 20px 60px' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '4px 0 20px' }}>
          ...
          <div style={{ width: 8, height: 8, borderRadius: '50%', background: 'var(--ink-100)', marginLeft: 'auto' }} />
        </div>

        <p style={{ ...SKB, fontSize: 'var(--fs-22)', color: 'var(--ink-100)', letterSpacing: '-0.02em', textTransform: 'uppercase', margin: '0 0 6px' }}>SCOPE BADGES</p>
        <p style={{ ...SKR, fontSize: 'var(--fs-11)', color: 'rgb(var(--ink-rgb) / 0.4)', lineHeight: 1.5, margin: '0 0 28px' }}>
        ...
        {RARITY_ORDER.map((key) => {
          const b = BADGES[key];
          return (
            <div key={key} style={{ display: 'flex', gap: 14, alignItems: 'flex-start', padding: '14px 0', borderTop: '1px solid rgb(var(--ink-rgb) / 0.1)' }}>
              <img src={b.bannerSrc ?? b.src} alt={b.title} style={{ width: 38, height: 38, objectFit: 'contain', flexShrink: 0 }} />
              <div style={{ flex: 1, minWidth: 0 }}>
                <p style={{ ...SKB, fontSize: 'var(--fs-11)', color: 'var(--ink-100)', letterSpacing: '0.1em', textTransform: 'uppercase', margin: '0 0 5px' }}>{b.title}</p>
                <p style={{ ...SKR, fontSize: 'var(--fs-10_5)', color: 'rgb(var(--ink-rgb) / 0.6)', lineHeight: 1.45, margin: 0 }}>{BADGE_SHORT_BLURB[key]}</p>
              </div>
            </div>
          );
        })}
