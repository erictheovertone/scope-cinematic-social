...
export default function BadgesStubPage() {
  ...
  return (
    <div className="bg-black" style={{ position: 'fixed', inset: 0, overflowY: 'auto' }}>
      <div style={{ maxWidth: 375, margin: '0 auto', padding: '16px 20px 60px' }}>
        ...
        <p style={{ ...SKB, fontSize: 22, color: 'white', letterSpacing: '-0.02em', textTransform: 'uppercase', margin: '0 0 6px' }}>SCOPE BADGES</p>
        <p style={{ ...SKR, fontSize: 11, color: 'rgba(255,255,255,0.4)', lineHeight: 1.5, margin: '0 0 28px' }}>
          ...
          return (
            <div key={key} style={{ display: 'flex', gap: 14, alignItems: 'flex-start', padding: '14px 0', borderTop: '1px solid rgba(255,255,255,0.1)' }}>
              ...
              <div style={{ flex: 1, minWidth: 0 }}>
                <p style={{ ...SKB, fontSize: 11, color: '#FF0000', letterSpacing: '0.1em', textTransform: 'uppercase', margin: '0 0 5px' }}>{b.title}</p>
                <p style={{ ...SKR, fontSize: 10.5, color: 'rgba(255,255,255,0.6)', lineHeight: 1.45, margin: 0 }}>{BADGE_SHORT_BLURB[key]}</p>
              </div>
            </div>
          );
        })}
